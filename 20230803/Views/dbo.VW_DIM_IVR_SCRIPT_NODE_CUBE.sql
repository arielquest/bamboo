...
GO
CREATE VIEW [dbo].[VW_DIM_IVR_SCRIPT_NODE_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('00004610-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        pk.[SPEECH_RECOGNITION],
        pk.[DEFLECTION_POINT],
        COALESCE(pk.[DEFLECTION_CATEGORY], '') AS [DEFLECTION_CATEGORY],
        pk.[DEFLECTION_EXCEPTION],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_IVR_SCRIPT_NODE] it
LEFT JOIN [dbo].[TB_DIM_IVR_SCRIPT_NODE_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                        AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted IVR Script Node ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted IVR Script Node ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        NULL AS [SPEECH_RECOGNITION],
        NULL AS [DEFLECTION_POINT],
        '' AS [DEFLECTION_CATEGORY],
        NULL AS [DEFLECTION_EXCEPTION],
...
WHERE   e.[ITEM_TYPE_URN] = 60;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_IVR_SCRIPT_NODE_CUBE]
	TO [portalapp_role]
GO
