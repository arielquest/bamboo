...
GO
CREATE VIEW [dbo].[VW_DIM_VECTOR_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('02d90779-bea4-48c7-ac7f-2df4bc9da230' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_VECTOR] it
LEFT JOIN [dbo].[TB_DIM_VECTOR_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                               AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Vector ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Vector ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 103;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_VECTOR_CUBE]
	TO [portalapp_role]
GO
