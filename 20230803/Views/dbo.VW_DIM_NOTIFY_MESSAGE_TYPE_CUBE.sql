...
GO
CREATE VIEW [dbo].[VW_DIM_NOTIFY_MESSAGE_TYPE_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('A1A245CD-ACCE-4D76-9839-7A589515601A' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        fl.[FOLDER_URN],
        it.[NAME],
        COALESCE(NULLIF(RTRIM(LTRIM(it.[NAME])), ''), pk.[PKEY1], 'Not Known') AS [INTERNAL_NAME],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_NOTIFY_MESSAGE_TYPE] it
LEFT JOIN [dbo].[TB_DIM_NOTIFY_MESSAGE_TYPE_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                               AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Notify Message Type ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Notify Message Type ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 128;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_NOTIFY_MESSAGE_TYPE_CUBE]
	TO [portalapp_role]
GO
