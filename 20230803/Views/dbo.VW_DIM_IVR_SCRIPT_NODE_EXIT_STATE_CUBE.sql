...
GO
CREATE VIEW [dbo].[VW_DIM_IVR_SCRIPT_NODE_EXIT_STATE_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('7b7acf0f-bf74-46de-b8f6-65b46e94f80c' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_IVR_SCRIPT_NODE_EXIT_STATE] it
LEFT JOIN [dbo].[TB_DIM_IVR_SCRIPT_NODE_EXIT_STATE_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                              AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted IVR Script Node Exit State '
...
WHERE   e.[ITEM_TYPE_URN] = 61;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_IVR_SCRIPT_NODE_EXIT_STATE_CUBE]
	TO [portalapp_role]
GO
