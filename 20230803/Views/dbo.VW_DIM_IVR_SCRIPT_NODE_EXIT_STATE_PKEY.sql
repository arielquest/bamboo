...
GO
CREATE   VIEW [dbo].[VW_DIM_IVR_SCRIPT_NODE_EXIT_STATE_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_IVR_SCRIPT_NODE_EXIT_STATE] di
    JOIN [dbo].[TB_DIM_IVR_SCRIPT_NODE_EXIT_STATE_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_IVR_SCRIPT_NODE_EXIT_STATE_PKEY]
	TO [portalapp_role]
GO
