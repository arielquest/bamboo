...
GO
CREATE VIEW [dbo].[VW_DIM_LABEL_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('00004700-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        COALESCE(pk.[LABEL_TYPE], -1) AS [LABEL_TYPE],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_LABEL] it
LEFT JOIN [dbo].[TB_DIM_LABEL_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                              AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Label ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Label ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        -1 AS [LABEL_TYPE],
...
WHERE   e.[ITEM_TYPE_URN] = 63;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_LABEL_CUBE]
	TO [portalapp_role]
GO
