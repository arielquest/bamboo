...
GO
CREATE   VIEW [dbo].[VW_DIM_PERSON_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           pk.[HOME_CLUSTER],
           pk.[ENABLE_USER],
           pk.[MEETING_INFORMATION],
           pk.[UC_SERVICE_PROFILE_BIZ_URN],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_PERSON] di
    JOIN [dbo].[TB_DIM_PERSON_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_PERSON_PKEY]
	TO [portalapp_role]
GO
