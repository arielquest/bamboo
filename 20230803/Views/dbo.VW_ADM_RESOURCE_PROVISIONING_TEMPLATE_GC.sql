SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
CREATE VIEW [dbo].[VW_ADM_RESOURCE_PROVISIONING_TEMPLATE_GC]
WITH SCHEMABINDING
AS
SELECT  datagc.[GUID_KEY] AS RESOURCE_ID,
        g.[FOLDER_ID] AS CONTAINER_ID,
        g.[RESOURCE_TYPE_ID],
        g.[NAME],
        g.[DESCRIPTION],
        g.[DELETED],
        g.[XML_DATA],
        g.[CREATION_DATE],
        g.[MODIFIED_DATE],
        g.[CREATED_BY_ID],
        g.[MODIFIED_BY_ID],
        g.[CHANGE_STAMP],
        g.[CUSTOM_XML_DATA],
        datagc.[GENERATION_COUNT],
        datagc.[CREATE_GENERATION_COUNT],
        datagc.[LAST_OPERATION]
FROM    [dbo].[TS_ADM_DATA_GENERATION_COUNT] datagc
JOIN    [dbo].[TS_ADM_TABLE_GENERATION_COUNT] tablegc ON tablegc.TABLE_GENERATION_COUNT_URN = datagc.TABLE_GENERATION_COUNT_URN
LEFT JOIN [dbo].[TB_ADM_RESOURCE_PROVISIONING_TEMPLATE] g ON g.ID = datagc.GUID_KEY
WHERE   tablegc.TABLE_NAME = 'TB_ADM_RESOURCE_PROVISIONING_TEMPLATE';
GO
GRANT SELECT
	ON [dbo].[VW_ADM_RESOURCE_PROVISIONING_TEMPLATE_GC]
	TO [portalapp_role]
GO
