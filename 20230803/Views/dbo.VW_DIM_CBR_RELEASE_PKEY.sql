...
GO
CREATE   VIEW [dbo].[VW_DIM_CBR_RELEASE_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           pk.[DESCRIPTION],
           pk.[RELEASE_STATUS],
           di.[ORIGINAL_CBR_RELEASE_BIZ_ITEM_URN],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_CBR_RELEASE] di
    JOIN [dbo].[TB_DIM_CBR_RELEASE_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CBR_RELEASE_PKEY]
	TO [portalapp_role]
GO
