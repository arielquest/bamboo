...
GO
CREATE VIEW [dbo].[VW_DIM_VA_ATTRIBUTE_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
...
LEFT JOIN [dbo].[TB_DIM_VA_ATTRIBUTE_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN] AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted VA Attribute ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted VA Attribute ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],       
...
WHERE   e.[ITEM_TYPE_URN] = 145;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_VA_ATTRIBUTE_CUBE]
	TO [portalapp_role]
GO
