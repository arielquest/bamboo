...
GO
CREATE   VIEW [dbo].[VW_DIM_NETWORK_VRU_SCRIPT_GC]
AS
    SELECT 
    ...
    FROM [dbo].[TS_DIM_NETWORK_VRU_SCRIPT_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_NETWORK_VRU_SCRIPT] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_NETWORK_VRU_SCRIPT_GC]
	TO [portalapp_role]
GO
