...
GO
CREATE   VIEW [dbo].[VW_DIM_OFFER_GC]
AS
    SELECT 
        ...
        item.[EFFECTIVE_FROM],
        item.[EFFECTIVE_TO],        
        item.[OFFER_TYPE_CODE_BIZ_URN],
        item.[SERVICE_TYPE_CODE_BIZ_URN],
        item.[MODIFIED_DATE],
        item.[MODIFIED_BY_ID],
        item.[CREATION_DATE],
        item.[CREATED_BY_ID],
    ...
    FROM [dbo].[TS_DIM_OFFER_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_OFFER] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_OFFER_GC]
	TO [portalapp_role]
GO
