...
GO
CREATE   VIEW [dbo].[VW_DIM_CHAT_REFERRER_URL_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('d12cd7df-e5b4-48c3-9129-b1d1c00325fd' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_CHAT_REFERRER_URL_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_CHAT_REFERRER_URL_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CHAT_REFERRER_URL_PKEY_MAP_GC]
	TO [portalapp_role]
GO
