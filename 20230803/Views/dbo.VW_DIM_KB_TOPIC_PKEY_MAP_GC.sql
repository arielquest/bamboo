...
GO
CREATE   VIEW [dbo].[VW_DIM_KB_TOPIC_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('d82c0660-b9ee-4da4-8b19-2861120ca90a' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_KB_TOPIC_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_KB_TOPIC_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_KB_TOPIC_PKEY_MAP_GC]
	TO [portalapp_role]
GO
