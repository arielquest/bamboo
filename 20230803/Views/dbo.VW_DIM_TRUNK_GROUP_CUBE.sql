...
GO
CREATE VIEW [dbo].[VW_DIM_TRUNK_GROUP_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('00008300-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        COALESCE(pk.[PERIPHERAL_NUMBER], '') AS [PERIPHERAL_NUMBER],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_TRUNK_GROUP] it
LEFT JOIN [dbo].[TB_DIM_TRUNK_GROUP_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                    AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Trunk Group ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Trunk Group ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        '' AS [PERIPHERAL_NUMBER],
        '' AS [PERIPHERAL_NAME],
...
WHERE   e.[ITEM_TYPE_URN] = 101;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_TRUNK_GROUP_CUBE]
	TO [portalapp_role]
GO
