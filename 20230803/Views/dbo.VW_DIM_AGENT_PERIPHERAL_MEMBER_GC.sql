...
GO
CREATE   VIEW [dbo].[VW_DIM_AGENT_PERIPHERAL_MEMBER_GC]
AS
    SELECT 
    ...
    FROM [dbo].[TS_DIM_AGENT_PERIPHERAL_MEMBER_GC] datagc 
    
    LEFT JOIN [dbo].[TB_DIM_AGENT_PERIPHERAL_MEMBER] member
    ON member.MEMBER_URN = datagc.MEMBER_URN

    WHERE datagc.[MEMBER_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_AGENT_PERIPHERAL_MEMBER_GC]
	TO [portalapp_role]
GO
