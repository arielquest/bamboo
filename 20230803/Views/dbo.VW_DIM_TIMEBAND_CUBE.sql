...
GO
CREATE VIEW [dbo].[VW_DIM_TIMEBAND_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('00007810-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_TIMEBAND] it
LEFT JOIN [dbo].[TB_DIM_TIMEBAND_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                 AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Timeband ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        '' AS [PKEY1],
        'This dimension does not exist in VIM anymore.' AS [DESCRIPTION],
        'Deleted Timeband ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        CAST(-2 AS INT) AS [PERIOD_START],
        CAST(-1 AS INT) AS [PERIOD_END],
...
WHERE   e.[ITEM_TYPE_URN] = 98;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_TIMEBAND_CUBE]
	TO [portalapp_role]
GO
