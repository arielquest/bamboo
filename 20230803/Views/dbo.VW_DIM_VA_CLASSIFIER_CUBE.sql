...
GO
CREATE VIEW [dbo].[VW_DIM_VA_CLASSIFIER_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('88FEFCA3-3FC1-4718-BC4D-8CFBC53DDFD7' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted VA Classifier ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted VA Classifier ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],       
...
WHERE   e.[ITEM_TYPE_URN] = 168;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_VA_CLASSIFIER_CUBE]
	TO [portalapp_role]
GO
