...
GO
CREATE   VIEW [dbo].[VW_DIM_PERIPHERAL_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           pk.[PERIPHERAL_NAME],
           pk.[LOCATION],
           pk.[ANSWERED_SHORT_CALLS_THRESHOLD],
           pk.[ABANDONED_CALL_WAIT_TIME],
           pk.[SERVICE_LEVEL_TYPE],
           pk.[PERIPHERAL_SERVICE_LEVEL_TYPE],
           pk.[SERVICE_LEVEL_THRESHOLD],
           pk.[AVAILABLE_HOLDOFF_DELAY],
           pk.[CLIENT_TYPE],
           pk.[SUB_SKILLGROUP_MASK],
           pk.[AUTO_CONFIGURED],
           di.[APPLICATION_USER_BIZ_URN],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_PERIPHERAL] di
    JOIN [dbo].[TB_DIM_PERIPHERAL_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_PERIPHERAL_PKEY]
	TO [portalapp_role]
GO
GRANT SELECT
	ON [dbo].[VW_DIM_PERIPHERAL_PKEY]
