...
GO
CREATE   VIEW [dbo].[VW_DIM_MEDIAFILE_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('d500e079-8a45-44b3-8068-f5dfe64297d5' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_MEDIAFILE_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_MEDIAFILE_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_MEDIAFILE_PKEY_MAP_GC]
	TO [portalapp_role]
GO
