...
GO
CREATE   VIEW [dbo].[VW_DIM_KB_FOLDER_GC]
AS
    SELECT 
        ...
        item.[EFFECTIVE_FROM],
        item.[EFFECTIVE_TO],        
        item.[KB_FOLDER_TYPE_CODE_BIZ_URN],
        item.[KB_FOLDER_TYPE],
        item.[MODIFIED_DATE],
        item.[MODIFIED_BY_ID],
        item.[CREATION_DATE],
    ...
    FROM [dbo].[TS_DIM_KB_FOLDER_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_KB_FOLDER] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_KB_FOLDER_GC]
	TO [portalapp_role]
GO
