...
GO
CREATE   VIEW [dbo].[VW_DIM_USER_GROUP_GC]
AS
    SELECT 
        ...
        item.[EFFECTIVE_FROM],
        item.[EFFECTIVE_TO],        
        item.[AGENT_TEAM_BIZ_URN],
        item.[SKILLGROUP_BIZ_URN],
        item.[MODIFIED_DATE],
        item.[MODIFIED_BY_ID],
        item.[CREATION_DATE],
        item.[CREATED_BY_ID],
    ...
    FROM [dbo].[TS_DIM_USER_GROUP_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_USER_GROUP] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_USER_GROUP_GC]
	TO [portalapp_role]
GO
