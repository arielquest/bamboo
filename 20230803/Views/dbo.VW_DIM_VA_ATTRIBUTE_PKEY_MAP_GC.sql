...
GO
CREATE   VIEW [dbo].[VW_DIM_VA_ATTRIBUTE_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('4ded5469-18cd-4817-96db-6e73b7c69656' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_VA_ATTRIBUTE_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_VA_ATTRIBUTE_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_VA_ATTRIBUTE_PKEY_MAP_GC]
	TO [portalapp_role]
GO
