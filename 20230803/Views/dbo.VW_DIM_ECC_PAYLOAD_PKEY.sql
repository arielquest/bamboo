...
GO
CREATE   VIEW [dbo].[VW_DIM_ECC_PAYLOAD_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           fl.[FOLDER_URN],
           di.[OWNER_ID],
           CAST('10fb2472-bf84-4cf8-a8bb-cc7dc9147df7' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
           di.[ITEM_SUBTYPE_ID],
           di.[NAME],    
    ...
    FROM [dbo].[TB_DIM_ECC_PAYLOAD] di
    JOIN [dbo].[TB_DIM_ECC_PAYLOAD_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_ECC_PAYLOAD_PKEY]
	TO [portalapp_role]
GO
