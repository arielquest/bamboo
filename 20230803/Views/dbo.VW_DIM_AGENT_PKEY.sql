...
GO
CREATE   VIEW [dbo].[VW_DIM_AGENT_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           pk.[PERIPHERAL_NAME],
           pk.[SUPERVISOR],
           pk.[AGENT_STATE_TRACE],
           pk.[DOMAIN_NAME],
           pk.[DOMAIN_LOGIN_NAME],
           pk.[DOMAIN_USER_NAME],
           pk.[DOMAIN_USER_GUID],
           pk.[USER_DELETABLE],
           pk.[USER_GROUP_ID],
           di.[DEPARTMENT_BIZ_URN],
           di.[MANAGER_AGENT_BIZ_URN],
           di.[USER_ID],
           pk.[ENABLED_ASSIST_SERVICE_TYPES],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_AGENT] di
    JOIN [dbo].[TB_DIM_AGENT_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_AGENT_PKEY]
	TO [portalapp_role]
GO
