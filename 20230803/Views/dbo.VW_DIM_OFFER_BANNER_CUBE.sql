...
GO
CREATE VIEW [dbo].[VW_DIM_OFFER_BANNER_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        ...
        fl.[FOLDER_URN],
        it.[NAME],
        COALESCE(NULLIF(RTRIM(LTRIM(pk.[URL])), ''), pk.[PKEY1], 'Not Known') AS [INTERNAL_NAME],
        COALESCE(pk.[PKEY1], '') AS [PKEY1],
        COALESCE(pk.[URL], '') AS [URL],
        COALESCE(pk.[AB_TEST_SHARE], -1) AS [AB_TEST_SHARE],
...
LEFT JOIN [dbo].[TB_DIM_OFFER_BANNER_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                     AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Offer Banner ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Offer Banner ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        '' AS [URL],
        -1 AS [AB_TEST_SHARE],
...
WHERE   e.[ITEM_TYPE_URN] = 119;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_OFFER_BANNER_CUBE]
	TO [portalapp_role]
GO
