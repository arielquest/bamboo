...
GO
CREATE   VIEW [dbo].[VW_DIM_CODE_GROUP_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           di.[INTERNAL_NAME],
           di.[CODE_CONSTRAINT_TYPE_ID],
           di.[CONSTRAINT_DEFINITION],
           di.[CONSTRAINT_MIN_VALUE],
           di.[CONSTRAINT_MAX_VALUE],
           di.[DEFAULT_VALUE],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_CODE_GROUP] di
    JOIN [dbo].[TB_DIM_CODE_GROUP_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CODE_GROUP_PKEY]
	TO [portalapp_role]
GO
