...
GO
CREATE VIEW [dbo].[VW_DIM_MEDIA_ROUTING_DOMAIN_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('00009600-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        COALESCE(pk.[INTERRUPTIBLE], '') AS [INTERRUPTIBLE],
        COALESCE(pk.[MAX_CALLS_IN_QUEUE], -1) AS [MAX_CALLS_IN_QUEUE],
        COALESCE(pk.[MAX_CALLS_IN_QUEUE_PER_CALLTYPE], -1) AS [MAX_CALLS_IN_QUEUE_PER_CALLTYPE],
        COALESCE(pk.[MAX_TASK_DURATION], -1) AS [MAX_TASK_DURATION],
        COALESCE(pk.[MAX_TIME_INQUEUE], -1) AS [MAX_TIME_INQUEUE],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_MEDIA_ROUTING_DOMAIN] it
LEFT JOIN [dbo].[TB_DIM_MEDIA_ROUTING_DOMAIN_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                             AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Media Routing Domain '
        + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Media Routing Domain '
        + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        '' AS [INTERRUPTIBLE],
        -1 AS [MAX_CALLS_IN_QUEUE],
        -1 AS [MAX_CALLS_IN_QUEUE_PER_CALLTYPE],
        -1 AS [MAX_TASK_DURATION],
        -1 AS [MAX_TIME_INQUEUE],
...
WHERE   e.[ITEM_TYPE_URN] = 68;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_MEDIA_ROUTING_DOMAIN_CUBE]
	TO [portalapp_role]
GO
