...
GO
CREATE   VIEW [dbo].[VW_DIM_CODE_GROUP_GC]
AS
    SELECT 
        ...
        item.[EFFECTIVE_TO],        
        item.[INTERNAL_NAME],
        item.[CODE_CONSTRAINT_TYPE_ID],
        item.[CONSTRAINT_DEFINITION],
        item.[CONSTRAINT_MIN_VALUE],
        item.[CONSTRAINT_MAX_VALUE],
        item.[DEFAULT_VALUE],
        item.[MODIFIED_DATE],
        item.[MODIFIED_BY_ID],
        item.[CREATION_DATE],
    ...
    FROM [dbo].[TS_DIM_CODE_GROUP_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_CODE_GROUP] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CODE_GROUP_GC]
	TO [portalapp_role]
GO
