...
GO
CREATE VIEW [dbo].[VW_DIM_GHE_CLUSTER_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('36FEE03E-BE37-43D3-A3E9-4B13F6CCAE5D' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_GHE_CLUSTER] it
LEFT JOIN [dbo].[TB_DIM_GHE_CLUSTER_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN] AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted GHE Cluster ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],      
...
WHERE   e.[ITEM_TYPE_URN] = 164;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_GHE_CLUSTER_CUBE]
	TO [portalapp_role]
GO
