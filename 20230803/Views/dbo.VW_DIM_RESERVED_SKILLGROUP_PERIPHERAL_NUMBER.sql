...
GO
CREATE VIEW dbo.VW_DIM_RESERVED_SKILLGROUP_PERIPHERAL_NUMBER
AS
...
INNER JOIN dbo.TB_DIM_SKILLGROUP_PERIPHERAL_MEMBER AS pMember ON pMember.PARENT_ITEM_BIZ_URN = pItem.ITEM_BIZ_URN
                                                              AND pMember.LATEST = 1
INNER JOIN dbo.TB_DIM_SKILLGROUP AS cItem ON pMember.CHILD_ITEM_BIZ_URN = cItem.ITEM_BIZ_URN
                                             AND cItem.LATEST = 1
INNER JOIN dbo.TB_DIM_SKILLGROUP_PKEY_MAP AS cItemPkey ON cItem.ITEM_BIZ_URN = cItemPkey.ITEM_BIZ_URN
...
GO
GRANT SELECT
	ON [dbo].[VW_DIM_RESERVED_SKILLGROUP_PERIPHERAL_NUMBER]
	TO [portalapp_role]
GO
