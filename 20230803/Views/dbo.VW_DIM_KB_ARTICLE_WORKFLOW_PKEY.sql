...
GO
CREATE   VIEW [dbo].[VW_DIM_KB_ARTICLE_WORKFLOW_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           fl.[FOLDER_URN],
           di.[OWNER_ID],
           CAST('140ae2b9-c3ba-42bb-8508-0240e45b95c1' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
           di.[ITEM_SUBTYPE_ID],
           di.[NAME],    
    ...
    FROM [dbo].[TB_DIM_KB_ARTICLE_WORKFLOW] di
    JOIN [dbo].[TB_DIM_KB_ARTICLE_WORKFLOW_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_KB_ARTICLE_WORKFLOW_PKEY]
	TO [portalapp_role]
GO
