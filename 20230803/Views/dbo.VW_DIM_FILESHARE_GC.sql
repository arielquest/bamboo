...
GO
CREATE   VIEW [dbo].[VW_DIM_FILESHARE_GC]
AS
    SELECT 
    ...
    FROM [dbo].[TS_DIM_FILESHARE_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_FILESHARE] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_FILESHARE_GC]
	TO [portalapp_role]
GO
