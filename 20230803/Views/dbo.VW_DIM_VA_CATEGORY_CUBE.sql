...
GO
CREATE VIEW [dbo].[VW_DIM_VA_CATEGORY_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('533f3ee4-334c-4d8b-a04c-a88efc120b55' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_VA_CATEGORY] it
LEFT JOIN [dbo].[TB_DIM_VA_CATEGORY_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN] AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted VA Category ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted VA Category ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],       
...
WHERE   e.[ITEM_TYPE_URN] = 149;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_VA_CATEGORY_CUBE]
	TO [portalapp_role]
GO
