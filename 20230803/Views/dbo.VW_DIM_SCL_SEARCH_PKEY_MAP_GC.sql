...
GO
CREATE   VIEW [dbo].[VW_DIM_SCL_SEARCH_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('b2a289a1-f613-4d0d-8f57-f83e04521155' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_SCL_SEARCH_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_SCL_SEARCH_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_SCL_SEARCH_PKEY_MAP_GC]
	TO [portalapp_role]
GO
