...
GO
CREATE   VIEW [dbo].[VW_DIM_SCL_ADAPTER_GC]
AS
    SELECT 
        ...
        item.[EFFECTIVE_FROM],
        item.[EFFECTIVE_TO],        
        item.[ADAPTER_TYPE_CODE_BIZ_URN],
        item.[ADAPTER_TYPE_SUB_CODE_BIZ_URN],
        item.[MODIFIED_DATE],
        item.[MODIFIED_BY_ID],
        item.[CREATION_DATE],
        item.[CREATED_BY_ID],
    ...
    FROM [dbo].[TS_DIM_SCL_ADAPTER_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_SCL_ADAPTER] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_SCL_ADAPTER_GC]
	TO [portalapp_role]
GO
