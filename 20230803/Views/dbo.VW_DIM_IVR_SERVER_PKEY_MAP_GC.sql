...
GO
CREATE   VIEW [dbo].[VW_DIM_IVR_SERVER_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('6368bd5b-e59d-4c8f-875b-82a117e8b7fc' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_IVR_SERVER_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_IVR_SERVER_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_IVR_SERVER_PKEY_MAP_GC]
	TO [portalapp_role]
GO
