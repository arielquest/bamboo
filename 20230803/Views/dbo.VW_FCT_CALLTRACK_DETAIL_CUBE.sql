...
GO
CREATE VIEW [dbo].[VW_FCT_CALLTRACK_DETAIL_CUBE]
AS
SELECT  [PARTITION_URN],
        ...
        CAST([CALL_DIRECTION] AS BIGINT) AS [Call Direction],
        ...
        CAST([CONFIRMATIONS_SENT_CNTR] AS BIGINT) AS [Confirmations Sent Counter],
        CAST([COMPLETED_MET_SLA_CNTR] AS BIGINT) AS [Completed Met Sla Counter],
        CAST([HANDLE_TIME] AS BIGINT) AS [Handle Time],
        [PARTITION_ID],
        [Row Count]
FROM    [dbo].[VW_FCT_CALLTRACK_DETAIL] x
...
GO
GRANT SELECT
	ON [dbo].[VW_FCT_CALLTRACK_DETAIL_CUBE]
	TO [portalapp_role]
GO
