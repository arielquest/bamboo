...
GO
CREATE VIEW [dbo].[VW_DIM_SCL_SEARCH_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        ...
        COALESCE(REPLACE(REPLACE(REPLACE(it.[SEARCH_PHRASE], CHAR(9), ''),
                                 CHAR(13), ''), CHAR(10), ''), '') COLLATE Latin1_General_100_CI_AS_SC AS [SEARCH_PHRASE],
        it.[EFFECTIVE_FROM],
        it.[EFFECTIVE_TO],
        it.[TIME_ZONE_URN],
        it.[TENANT_BIZ_URN],
...
LEFT JOIN [dbo].[TB_DIM_SCL_SEARCH_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN] AND pk.[LATEST] = 1
JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = it.[FOLDER_ID]
JOIN [dbo].[TB_DIM_SCL_ADAPTER] sca ON it.[SCL_ADAPTER_BIZ_URN] = sca.[ITEM_BIZ_URN]
UNION ALL
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        -1 AS [SCL_ADAPTER_BIZ_URN],     
        'Deleted Social Search ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Social Search ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 143;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_SCL_SEARCH_CUBE]
	TO [portalapp_role]
GO
