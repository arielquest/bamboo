...
GO
CREATE   VIEW [dbo].[VW_DIM_GHE_QUESTION_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_GHE_QUESTION] di
    JOIN [dbo].[TB_DIM_GHE_QUESTION_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_GHE_QUESTION_PKEY]
	TO [portalapp_role]
GO
