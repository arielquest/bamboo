...
GO
CREATE   VIEW [dbo].[VW_DIM_VXMLAPPLICATION_SERVER_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('e8df41b0-854c-421d-9c6d-1eaa8fdc9dce' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_VXMLAPPLICATION_SERVER_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_VXMLAPPLICATION_SERVER_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_VXMLAPPLICATION_SERVER_PKEY_MAP_GC]
	TO [portalapp_role]
GO
