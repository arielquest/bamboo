...
GO
CREATE   VIEW [dbo].[VW_DIM_MEDIA_ROUTING_DOMAIN_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           pk.[DESCRIPTION],
           pk.[INTERRUPTIBLE],
           pk.[MAX_CALLS_IN_QUEUE],
           pk.[MAX_CALLS_IN_QUEUE_PER_CALLTYPE],
    ...
    FROM [dbo].[TB_DIM_MEDIA_ROUTING_DOMAIN] di
    JOIN [dbo].[TB_DIM_MEDIA_ROUTING_DOMAIN_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_MEDIA_ROUTING_DOMAIN_PKEY]
	TO [portalapp_role]
GO
GRANT SELECT
	ON [dbo].[VW_DIM_MEDIA_ROUTING_DOMAIN_PKEY]
