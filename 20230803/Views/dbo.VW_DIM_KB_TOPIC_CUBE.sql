...
GO
CREATE VIEW [dbo].[VW_DIM_KB_TOPIC_CUBE]
WITH SCHEMABINDING
AS
SELECT it.[ITEM_URN],
       it.[ITEM_BIZ_URN],
       CAST('d82c0660-b9ee-4da4-8b19-2861120ca90a' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
       it.[ITEM_SUBTYPE_ID],
       it.[OWNER_ID],
       it.[FOLDER_ID],
       ...
       it.[LATEST],
       it.[CHANGE_STAMP]
FROM [dbo].[TB_DIM_KB_TOPIC] it
LEFT JOIN [dbo].[TB_DIM_KB_TOPIC_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                          AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted KB Topic ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted KB Topic ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 122;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_KB_TOPIC_CUBE]
	TO [portalapp_role]
GO
