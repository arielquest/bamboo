...
GO
CREATE   VIEW [dbo].[VW_DIM_ROUTING_CLIENT_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           fl.[FOLDER_URN],
           di.[OWNER_ID],
           CAST('00007300-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
           di.[ITEM_SUBTYPE_ID],
           di.[NAME],    
           ...
           pk.[DESCRIPTION],
           pk.[CLIENT_TYPE],
           pk.[TIMEOUT_THRESHOLD],
           pk.[TIMEOUT_LIMIT],
           pk.[LATE_THRESHOLD],
           pk.[DIALED_NUMBER_LABEL_MAP_PRESENT],
           pk.[NETWORK_ROUTING_CLIENT],
           pk.[NETWORK_TRANSFER_PREFERED],
           di.[LOGICAL_INTERFACE_CONTROLLER_BIZ_URN],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_ROUTING_CLIENT] di
    JOIN [dbo].[TB_DIM_ROUTING_CLIENT_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_ROUTING_CLIENT_PKEY]
	TO [portalapp_role]
GO
GRANT SELECT
	ON [dbo].[VW_DIM_ROUTING_CLIENT_PKEY]
