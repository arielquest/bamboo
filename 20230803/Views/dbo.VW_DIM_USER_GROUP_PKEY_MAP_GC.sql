...
GO
CREATE   VIEW [dbo].[VW_DIM_USER_GROUP_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('c95a5f46-9dec-4cf3-80bc-f6943f3cf0c0' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_USER_GROUP_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_USER_GROUP_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_USER_GROUP_PKEY_MAP_GC]
	TO [portalapp_role]
GO
