...
GO
CREATE   VIEW [dbo].[VW_DIM_VXMLAPPLICATION_SERVER_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           fl.[FOLDER_URN],
           di.[OWNER_ID],
           CAST('e8df41b0-854c-421d-9c6d-1eaa8fdc9dce' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
           di.[ITEM_SUBTYPE_ID],
           di.[NAME],    
    ...
    FROM [dbo].[TB_DIM_VXMLAPPLICATION_SERVER] di
    JOIN [dbo].[TB_DIM_VXMLAPPLICATION_SERVER_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_VXMLAPPLICATION_SERVER_PKEY]
	TO [portalapp_role]
GO
