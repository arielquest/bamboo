...
GO
CREATE VIEW [dbo].[VW_DIM_SUB_CODE_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        ...
        fl.[FOLDER_URN],
        it.[NAME],
        COALESCE(NULLIF(RTRIM(LTRIM(it.[INTERNAL_NAME])), ''), pk.[PKEY1],
...
LEFT JOIN [dbo].[TB_DIM_SUB_CODE_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                 AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Sub Code ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Sub Code ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 117;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_SUB_CODE_CUBE]
	TO [portalapp_role]
GO
