...
GO
CREATE VIEW [dbo].[VW_DIM_SKILLGROUP_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_SKILLGROUP] it
LEFT JOIN [dbo].[TB_DIM_SKILLGROUP_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                   AND pk.[LATEST] = 1
JOIN    [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = it.[FOLDER_ID]
UNION ALL
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        e.[ITEM_BIZ_URN] AS [ITEM_BIZ_URN],
        'Deleted SkillGroup ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        -1 AS [PERIPHERAL_NUMBER],
        '' AS [PERIPHERAL_NAME],
        ...
        -1 AS [FOLDER_URN],
        'Deleted SkillGroup ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
...
WHERE   e.[ITEM_TYPE_URN] = 95;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_SKILLGROUP_CUBE]
	TO [portalapp_role]
GO
