...
GO
CREATE VIEW [dbo].[VW_DIM_KB_PORTAL_CUBE]
WITH SCHEMABINDING
AS
SELECT it.[ITEM_URN],
       it.[ITEM_BIZ_URN],
       CAST('81e33d5a-42f6-4a6d-9753-561cefe6136f' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
       it.[ITEM_SUBTYPE_ID],
       it.[OWNER_ID],
       it.[FOLDER_ID],
       ...
       it.[LATEST],
       it.[CHANGE_STAMP]
FROM [dbo].[TB_DIM_KB_PORTAL] it
LEFT JOIN [dbo].[TB_DIM_KB_PORTAL_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                          AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted KB Portal ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted KB Portal ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 121;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_KB_PORTAL_CUBE]
	TO [portalapp_role]
GO
