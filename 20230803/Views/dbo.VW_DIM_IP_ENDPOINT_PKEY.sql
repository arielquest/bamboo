...
GO
CREATE   VIEW [dbo].[VW_DIM_IP_ENDPOINT_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           di.[OWNER_PERSON_BIZ_URN],
           di.[RESOURCE_TEMPLATE_ID],
           di.[USER_CONTROLS_DEVICE],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_IP_ENDPOINT] di
    JOIN [dbo].[TB_DIM_IP_ENDPOINT_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_IP_ENDPOINT_PKEY]
	TO [portalapp_role]
GO
