...
GO
CREATE   VIEW [dbo].[VW_DIM_CUSTOM_ATTRIBUTE3_GC]
AS
    SELECT 
    ...
    FROM [dbo].[TS_DIM_CUSTOM_ATTRIBUTE3_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_CUSTOM_ATTRIBUTE3] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CUSTOM_ATTRIBUTE3_GC]
	TO [portalapp_role]
GO
