...
GO
CREATE   VIEW [dbo].[VW_DIM_ORIGINATOR_TYPE_CODE_PKEY]
WITH SCHEMABINDING
AS
    SELECT fl.[FOLDER_ID],
    ...
    WHERE (cg.[INTERNAL_NAME] = 'CG_ORIGINATOR_TYPE' OR it.ITEM_URN IN (-1, -2))
    AND cg.[SYSTEM] = 1
    AND it.[LATEST] = 1;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_ORIGINATOR_TYPE_CODE_PKEY]
	TO [portalapp_role]
GO
