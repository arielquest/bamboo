...
GO
CREATE   VIEW [dbo].[VW_DIM_AGENT_TEAM_DIALED_NUMBER_MEMBER_GC]
AS
    SELECT 
        ...
        CAST('00001200-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
    ...
    FROM [dbo].[TS_DIM_AGENT_TEAM_DIALED_NUMBER_MEMBER_GC] datagc 
    
    LEFT JOIN [dbo].[TB_DIM_AGENT_TEAM_DIALED_NUMBER_MEMBER] member
    ON member.MEMBER_URN = datagc.MEMBER_URN

    WHERE datagc.[MEMBER_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_AGENT_TEAM_DIALED_NUMBER_MEMBER_GC]
	TO [portalapp_role]
GO
