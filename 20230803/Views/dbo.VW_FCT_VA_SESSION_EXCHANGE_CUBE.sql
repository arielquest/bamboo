...
GO
CREATE VIEW [dbo].[VW_FCT_VA_SESSION_EXCHANGE_CUBE]
AS
    SELECT  [x].[PARTITION_URN],
            ...
            CAST([x].[CLARIFICATIONS] AS BIGINT) AS [Clarifications],
            CAST([x].[EXCHANGES] AS BIGINT) AS [Exchanges],
            CAST([x].[FAILED] AS BIGINT) AS [Failed],
            CAST([x].[OUT_OF_DOMAIN] AS BIGINT) AS [Out Of Domain],
            CAST([x].[RELEVANT_EXCHANGES] AS BIGINT) AS [Relevant Exchanges],
            CAST([x].[URL_PUSHED] AS BIGINT) AS [URL Pushed],

            [x].[PARTITION_ID],
            [x].[Row Count]
    FROM    [dbo].[VW_FCT_VA_SESSION_EXCHANGE] [x]
...
GO
GRANT SELECT
	ON [dbo].[VW_FCT_VA_SESSION_EXCHANGE_CUBE]
	TO [portalapp_role]
GO
