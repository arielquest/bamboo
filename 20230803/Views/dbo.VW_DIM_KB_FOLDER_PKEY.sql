...
GO
CREATE   VIEW [dbo].[VW_DIM_KB_FOLDER_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           pk.[INTERNAL_NAME],
           pk.[DESCRIPTION],
           di.[KB_FOLDER_TYPE_CODE_BIZ_URN],
           di.[KB_FOLDER_TYPE],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_KB_FOLDER] di
    JOIN [dbo].[TB_DIM_KB_FOLDER_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_KB_FOLDER_PKEY]
	TO [portalapp_role]
GO
