...
GO
CREATE   VIEW [dbo].[VW_DIM_USER_GROUP_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           fl.[FOLDER_URN],
           di.[OWNER_ID],
           CAST('c95a5f46-9dec-4cf3-80bc-f6943f3cf0c0' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
           di.[ITEM_SUBTYPE_ID],
           di.[NAME],    
           ...
           di.[AGENT_TEAM_BIZ_URN],
           di.[SKILLGROUP_BIZ_URN],
           pk.[INTERNAL_NAME],
           pk.[DESCRIPTION],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_USER_GROUP] di
    JOIN [dbo].[TB_DIM_USER_GROUP_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_USER_GROUP_PKEY]
	TO [portalapp_role]
GO
