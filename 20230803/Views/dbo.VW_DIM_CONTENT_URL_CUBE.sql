...
GO
CREATE VIEW [dbo].[VW_DIM_CONTENT_URL_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('12F8F3FD-B2D4-4D48-AEE6-52ED6C359AC9' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_CONTENT_URL] it
LEFT JOIN [dbo].[TB_DIM_CONTENT_URL_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                    AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Content Url ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Content Url ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        '' AS [LABEL],
        '' AS [URL],
...
WHERE   e.[ITEM_TYPE_URN] = 120;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CONTENT_URL_CUBE]
	TO [portalapp_role]
GO
