...
GO
CREATE   VIEW [dbo].[VW_DIM_QUEUE_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           di.[DIALED_NUMBER_BIZ_URN],
           di.[SKILLGROUP_BIZ_URN],
           pk.[INTERNAL_NAME],
           pk.[DESCRIPTION],
           pk.[STATE],
           pk.[EMAIL_ROUTING_TYPE],
           pk.[CHAT_ROUTING_TYPE],
           pk.[SOCIAL_ROUTING_TYPE],
           di.[TIME_ZONE_URN],
           di.[TENANT_BIZ_URN],
    ...
    FROM [dbo].[TB_DIM_QUEUE] di
    JOIN [dbo].[TB_DIM_QUEUE_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_QUEUE_PKEY]
	TO [portalapp_role]
GO
