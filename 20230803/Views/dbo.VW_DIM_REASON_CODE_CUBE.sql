...
GO
CREATE VIEW [dbo].[VW_DIM_REASON_CODE_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('00009800-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        LEFT(COALESCE(pk.[REASON_TEXT_VALUE], COALESCE(it.[NAME], '')), 50) AS [REASON_TEXT_VALUE],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_REASON_CODE] it
LEFT JOIN [dbo].[TB_DIM_REASON_CODE_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                    AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Reason Code ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Reason Code ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        '' AS [REASON_TEXT_VALUE],
        'This dimension does not exist in VIM anymore.' AS [DESCRIPTION],
        CAST(0 AS BIT) AS [IS_PRODUCTIVE],
...
WHERE   e.[ITEM_TYPE_URN] = 84;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_REASON_CODE_CUBE]
	TO [portalapp_role]
GO
