...
GO
CREATE VIEW [dbo].[VW_FCT_CVP_USAGE_CUBE]
AS
SELECT  [PARTITION_URN],
        ...
        tzd.[OLAP_DATETIME_TIMEZONE_19] AS [TIME_ZONE_19_URN],
            ...
            -- Dimensions
        [MACHINE_URN],
        [MACHINE_RESOURCE_URN],
        [CLUSTER_RESOURCE_URN],
        [THRESHOLD_REACHED],
            
            -- Measures
        CAST([RESOURCE_USED] AS BIGINT) AS [RESOURCE_USED],
        CAST([RESOURCE_MAX] AS BIGINT) AS [RESOURCE_MAX],
...
FROM    [dbo].[VW_FCT_CVP_USAGE] x
JOIN    [dbo].[VW_DIM_TIME_STD_CUBE] t ON x.[EVENT_TIME_UTC_URN] = t.[TIME_URN]
JOIN    [dbo].[TS_ADM_TIME_ZONE_DATA] tzd ON tzd.[DATE_UTC_URN] = x.[EVENT_DATE_UTC_URN]
                                             AND tzd.[TIME_UTC_URN] = t.[QUARTER_HOUR_URN];
GO
GRANT SELECT
	ON [dbo].[VW_FCT_CVP_USAGE_CUBE]
	TO [portalapp_role]
GO
