...
GO
CREATE VIEW [dbo].[VW_DIM_KB_COMPLIANCE_POLICY_CUBE]
WITH SCHEMABINDING
AS
SELECT it.[ITEM_URN],
       it.[ITEM_BIZ_URN],
       CAST('555644c0-afa2-498d-b7fa-038a00f821f0' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
       it.[ITEM_SUBTYPE_ID],
       it.[OWNER_ID],
       it.[FOLDER_ID],
       ...
       it.[LATEST],
       it.[CHANGE_STAMP]
FROM [dbo].[TB_DIM_KB_COMPLIANCE_POLICY] it
LEFT JOIN [dbo].[TB_DIM_KB_COMPLIANCE_POLICY_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                          AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted KB Compliance Policy ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted KB Compliance Policy ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        'This dimension does not exist in EA anymore.' AS [DESCRIPTION],
...
WHERE   e.[ITEM_TYPE_URN] = 171;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_KB_COMPLIANCE_POLICY_CUBE]
	TO [portalapp_role]
GO
