...
GO
CREATE VIEW [dbo].[VW_DIM_GHE_QUESTION_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('95009762-FC8E-4303-8096-86FB24EC0CB3' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        fl.[FOLDER_URN],
        it.[NAME],
        COALESCE(NULLIF(RTRIM(LTRIM(pk.[QUESTION_TEXT])), ''), pk.[PKEY1],
                 'Not Known') COLLATE Latin1_General_100_CI_AS_SC AS [QUESTION_TEXT],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_GHE_QUESTION] it
LEFT JOIN [dbo].[TB_DIM_GHE_QUESTION_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                               AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted GHE Question ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted GHE Question ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [QUESTION_TEXT],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 129;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_GHE_QUESTION_CUBE]
	TO [portalapp_role]
GO
