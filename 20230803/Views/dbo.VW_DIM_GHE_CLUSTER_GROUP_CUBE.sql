...
GO
CREATE VIEW [dbo].[VW_DIM_GHE_CLUSTER_GROUP_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('81674C53-6785-4F4F-A6D7-27F180C520F8' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_GHE_CLUSTER_GROUP] it
LEFT JOIN [dbo].[TB_DIM_GHE_CLUSTER_GROUP_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN] AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted GHE Cluster Group ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],      
...
WHERE   e.[ITEM_TYPE_URN] = 165;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_GHE_CLUSTER_GROUP_CUBE]
	TO [portalapp_role]
GO
