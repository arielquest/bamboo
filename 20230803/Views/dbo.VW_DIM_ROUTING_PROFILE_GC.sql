...
GO
CREATE   VIEW [dbo].[VW_DIM_ROUTING_PROFILE_GC]
AS
    SELECT 
    ...
    FROM [dbo].[TS_DIM_ROUTING_PROFILE_GC] datagc 
        
    LEFT JOIN [dbo].[TB_DIM_ROUTING_PROFILE] item
    ON item.ITEM_URN = datagc.ITEM_URN

    WHERE datagc.[ITEM_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_ROUTING_PROFILE_GC]
	TO [portalapp_role]
GO
