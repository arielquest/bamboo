...
GO
CREATE   VIEW [dbo].[VW_DIM_AGENT_USER_GROUP_MEMBER_GC]
AS
    SELECT 
        ...
        CAST('c95a5f46-9dec-4cf3-80bc-f6943f3cf0c0' AS UNIQUEIDENTIFIER) AS [PARENT_RESOURCE_TYPE_ID],
    ...
    FROM [dbo].[TS_DIM_AGENT_USER_GROUP_MEMBER_GC] datagc 
    
    LEFT JOIN [dbo].[TB_DIM_AGENT_USER_GROUP_MEMBER] member
    ON member.MEMBER_URN = datagc.MEMBER_URN

    WHERE datagc.[MEMBER_URN] > 0;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_AGENT_USER_GROUP_MEMBER_GC]
	TO [portalapp_role]
GO
