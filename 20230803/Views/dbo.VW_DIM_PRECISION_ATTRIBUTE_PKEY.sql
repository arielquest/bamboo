...
GO
CREATE   VIEW [dbo].[VW_DIM_PRECISION_ATTRIBUTE_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           pk.[INTERNAL_NAME],
           pk.[DESCRIPTION],
           pk.[ATTRIBUTE_DATA_TYPE],
           pk.[MINIMUM_VALUE],
           pk.[MAXIMUM_VALUE],
           pk.[DEFAULT_VALUE],
           pk.[APPEARS_ON_DESKTOP],
           pk.[SETTABLE_BY_AGENT],
    ...
    FROM [dbo].[TB_DIM_PRECISION_ATTRIBUTE] di
    JOIN [dbo].[TB_DIM_PRECISION_ATTRIBUTE_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_PRECISION_ATTRIBUTE_PKEY]
	TO [portalapp_role]
GO
