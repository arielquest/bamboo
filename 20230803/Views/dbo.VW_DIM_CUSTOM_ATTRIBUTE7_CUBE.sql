...
GO
CREATE VIEW [dbo].[VW_DIM_CUSTOM_ATTRIBUTE7_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN] AS [CUSTOM_URN],
        it.[ITEM_BIZ_URN] AS [CUSTOM_PARENT_URN],
        CAST('33912E67-A457-4260-A0B5-82117DEBE1EF' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_CUSTOM_ATTRIBUTE7] it
JOIN    [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = it.[FOLDER_ID]
LEFT JOIN [dbo].[TB_DIM_CUSTOM_ATTRIBUTE7_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                AND pk.[LATEST] = 1;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CUSTOM_ATTRIBUTE7_CUBE]
	TO [portalapp_role]
GO
