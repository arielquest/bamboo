...
GO
CREATE VIEW [dbo].[VW_DIM_CHAT_REFERRER_URL_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
...
LEFT JOIN [dbo].[TB_DIM_CHAT_REFERRER_URL_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                               AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Chat Referrer URL ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Chat Referrer URL ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
...
WHERE   e.[ITEM_TYPE_URN] = 163;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_CHAT_REFERRER_URL_CUBE]
	TO [portalapp_role]
GO
