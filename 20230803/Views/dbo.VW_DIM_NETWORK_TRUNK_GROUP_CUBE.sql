...
GO
CREATE VIEW [dbo].[VW_DIM_NETWORK_TRUNK_GROUP_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('00005600-1000-0000-0000-000000000010' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_NETWORK_TRUNK_GROUP] it
LEFT JOIN [dbo].[TB_DIM_NETWORK_TRUNK_GROUP_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                                            AND pk.[LATEST] = 1
...
WHERE   e.[ITEM_TYPE_URN] = 69;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_NETWORK_TRUNK_GROUP_CUBE]
	TO [portalapp_role]
GO
