...
GO
CREATE   VIEW [dbo].[VW_DIM_NOTIFY_CHANNEL_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('10f7f60a-f120-49e5-9ea1-5adc5a66371f' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_NOTIFY_CHANNEL_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_NOTIFY_CHANNEL_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_NOTIFY_CHANNEL_PKEY_MAP_GC]
	TO [portalapp_role]
GO
