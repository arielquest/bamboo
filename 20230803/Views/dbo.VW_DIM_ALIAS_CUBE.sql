...
GO
CREATE VIEW [dbo].[VW_DIM_ALIAS_CUBE]
WITH SCHEMABINDING
AS
SELECT  it.[ITEM_URN],
        it.[ITEM_BIZ_URN],
        CAST('D681B841-A1D4-4BE8-8538-F75C40F524EB' AS UNIQUEIDENTIFIER) AS [ITEM_TYPE_ID],
        it.[ITEM_SUBTYPE_ID],
        it.[OWNER_ID],
        it.[FOLDER_ID],
        ...
        COALESCE(pk.[EMAIL_ADDRESS], '') AS [EMAIL_ADDRESS],
        COALESCE(pk.[IS_DEFAULT_ALIAS], 0) AS [IS_DEFAULT_ALIAS],
        ...
        it.[LATEST],
        it.[CHANGE_STAMP]
FROM    [dbo].[TB_DIM_ALIAS] it
LEFT JOIN [dbo].[TB_DIM_ALIAS_PKEY_MAP] pk ON it.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
                                              AND pk.[LATEST] = 1
...
SELECT  e.[ITEM_BIZ_URN] AS [ITEM_URN],
        ...
        -1 AS [FOLDER_URN],
        'Deleted Alias ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [NAME],
        'Deleted Alias ' + CAST(e.[ITEM_BIZ_URN] AS NVARCHAR(10)) AS [INTERNAL_NAME],
        '' AS [PKEY1],
        '' AS [EMAIL_ADDRESS],
        0 AS [IS_DEFAULT_ALIAS],
        0 AS [IS_ACTIVE],
...
WHERE   e.[ITEM_TYPE_URN] = 116;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_ALIAS_CUBE]
	TO [portalapp_role]
GO
