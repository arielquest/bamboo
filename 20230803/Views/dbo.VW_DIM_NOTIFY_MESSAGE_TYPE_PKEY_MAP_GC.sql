...
GO
CREATE   VIEW [dbo].[VW_DIM_NOTIFY_MESSAGE_TYPE_PKEY_MAP_GC]
AS
   SELECT 
        ...
        CAST('a1a245cd-acce-4d76-9839-7a589515601a' AS UNIQUEIDENTIFIER) AS [CHILD_RESOURCE_TYPE_ID],
        pkey.[STATUS],
        pkey.[CHANGE_STAMP],
        pkey.[PKEY1],
        pkey.[PKEY2],
    ...
    FROM [dbo].[TS_DIM_NOTIFY_MESSAGE_TYPE_PKEY_MAP_GC] datagc
    
    LEFT JOIN [dbo].[TB_DIM_NOTIFY_MESSAGE_TYPE_PKEY_MAP] pkey
    ON pkey.PKEY_MAP_URN = datagc.PKEY_MAP_URN;
GO
GRANT SELECT
	ON [dbo].[VW_DIM_NOTIFY_MESSAGE_TYPE_PKEY_MAP_GC]
	TO [portalapp_role]
GO
