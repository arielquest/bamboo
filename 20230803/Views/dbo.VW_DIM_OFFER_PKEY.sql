...
GO
CREATE   VIEW [dbo].[VW_DIM_OFFER_PKEY]
WITH SCHEMABINDING
AS
    SELECT di.[ITEM_URN],
           di.[ITEM_BIZ_URN],
           ...
           di.[OFFER_TYPE_CODE_BIZ_URN],
           di.[SERVICE_TYPE_CODE_BIZ_URN],
           pk.[INTERNAL_NAME],
           pk.[DESCRIPTION],
           pk.[LABEL],
           pk.[IS_FIXED],
    ...
    FROM [dbo].[TB_DIM_OFFER] di
    JOIN [dbo].[TB_DIM_OFFER_PKEY_MAP] pk ON di.[ITEM_BIZ_URN] = pk.[ITEM_BIZ_URN]
    JOIN [dbo].[TB_SEC_FOLDER] fl ON fl.[FOLDER_ID] = di.[FOLDER_ID];
GO
GRANT SELECT
	ON [dbo].[VW_DIM_OFFER_PKEY]
	TO [portalapp_role]
GO
