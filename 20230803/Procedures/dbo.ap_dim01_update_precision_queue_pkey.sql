...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_precision_queue_pkey]
(    
    ...
    @AgentOrdering           [INT] = 1, 
    @CallOrdering            [INT] = 1, 
    @ServiceLevelThreshold   [INT] = 0, 
    @ServiceLevelType        [SMALLINT] = 1, 
    @ForceExpandingQueue     [BIT] = NULL, 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_precision_queue_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00039562-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldAgentOrdering = map.[AGENT_ORDERING],
                                @OldCallOrdering = map.[CALL_ORDERING],
                                @OldServiceLevelThreshold = map.[SERVICE_LEVEL_THRESHOLD],
                                @OldServiceLevelType = map.[SERVICE_LEVEL_TYPE],
                                @OldForceExpandingQueue = map.[FORCE_EXPANDING_QUEUE], 
                        ...
                        FROM dbo.TB_DIM_PRECISION_QUEUE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_PRECISION_QUEUE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'PRECISION_QUEUE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.PRECISION_QUEUE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAgentOrdering IS NOT NULL AND @AgentOrdering IS NOT NULL AND @OldAgentOrdering <> @AgentOrdering)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldAgentOrdering AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@AgentOrdering AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_AGENT_ORDERING', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldCallOrdering IS NOT NULL AND @CallOrdering IS NOT NULL AND @OldCallOrdering <> @CallOrdering)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldCallOrdering AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@CallOrdering AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_CALL_ORDERING', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_SERVICE_LEVEL_THRESHOLD', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_SERVICE_LEVEL_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldForceExpandingQueue IS NOT NULL AND @ForceExpandingQueue IS NOT NULL AND @OldForceExpandingQueue <> @ForceExpandingQueue)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldForceExpandingQueue WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @ForceExpandingQueue WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_FORCE_EXPANDING_QUEUE', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_precisionPKEYTran;
                            BEGIN TRY;
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 78, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_precisionPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_precision_queue_pkey]
	TO [portalapp_role]
GO
