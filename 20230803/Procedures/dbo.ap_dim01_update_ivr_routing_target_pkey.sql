...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ivr_routing_target_pkey]
(    
    ...
    @RoutingMethod           [NVARCHAR](50) = NULL, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldRoutingMethod                [NVARCHAR](50), 
                @OldCallType                     [NVARCHAR](50), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_ivr_routing_target_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00011300-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldRoutingMethod = map.[ROUTING_METHOD],
                                @OldCallType = map.[CALL_TYPE], 
                        ...
                        FROM dbo.TB_DIM_IVR_ROUTING_TARGET_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_IVR_ROUTING_TARGET AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_IVR_ROUTING_TARGET_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'IVR_ROUTING_TARGET_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.IVR_ROUTING_TARGET_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ROUTING_TARGET_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ROUTING_TARGET_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldRoutingMethod IS NOT NULL AND @RoutingMethod IS NOT NULL AND @OldRoutingMethod COLLATE Latin1_General_CS_AS <> @RoutingMethod COLLATE Latin1_General_CS_AS)  
                        OR (@OldRoutingMethod IS NULL AND @RoutingMethod IS NOT NULL)
                        OR (@OldRoutingMethod IS NOT NULL AND @RoutingMethod IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldRoutingMethod AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@RoutingMethod AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ROUTING_TARGET_ROUTING_METHOD', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldCallType IS NOT NULL AND @CallType IS NOT NULL AND @OldCallType COLLATE Latin1_General_CS_AS <> @CallType COLLATE Latin1_General_CS_AS)  
                        OR (@OldCallType IS NULL AND @CallType IS NOT NULL)
                        OR (@OldCallType IS NOT NULL AND @CallType IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldCallType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@CallType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ROUTING_TARGET_CALL_TYPE', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_ivrroutingtargPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_IVR_ROUTING_TARGET_PKEY_MAP
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 58, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_ivrroutingtargPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ivr_routing_target_pkey]
	TO [portalapp_role]
GO
