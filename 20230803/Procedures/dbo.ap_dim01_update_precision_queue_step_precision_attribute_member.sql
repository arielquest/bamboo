...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_precision_queue_step_precision_attribute_member] 
(
...
AS
    ...
    @OldEffectiveFrom DATETIME, @OldEffectiveTo DATETIME, @OldXmlData XML, @OldCustomXmlData AS XML,@OldTermOrder                   [INT],
    ...
    BEGIN 
        ...
        WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER' AND MEMBER_TYPE_ID = '00039612-1000-0000-0000-000000000010' AND CUSTOM = 0;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_ADM_XML_DEFAULTS 
        WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER' AND MEMBER_TYPE_ID = '00039612-1000-0000-0000-000000000010' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        @OldEffectiveTo = member.[EFFECTIVE_TO], @OldCustomXmlData = member.[CUSTOM_XML_DATA], @OldTermOrder = member.[TERM_ORDER],
        ...
        FROM dbo.TB_DIM_PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER member
        ...
        IF EXISTS(SELECT 1 FROM dbo.TB_DIM_PRECISION_ATTRIBUTE WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF EXISTS(SELECT 1 FROM dbo.TB_DIM_PRECISION_QUEUE_STEP WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                ...
                INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                SELECT 'PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
                ...
                IF (@OldTermOrder IS NOT NULL AND @TermOrder IS NOT NULL AND @OldTermOrder <> @TermOrder) 
                BEGIN 
      
                    SET @AuditTempOld = CAST(@OldTermOrder AS NVARCHAR(250));
                    SET @AuditTempNew = CAST(@TermOrder AS NVARCHAR(250));                     INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                    VALUES('PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_TERM_ORDER', @AuditTempNew, @AuditTempOld);
                    SET @FieldChanged = 1;
                    SET @ProvisionableFieldChanged= 1;
                END;
                ...
                BEGIN 
                    -- Update the tables
                    BEGIN TRANSACTION PUPD_precisionqueuesTran;
                        ...
                        BEGIN 
                            ...
                            BEGIN TRY;
                                ...
                                IF @DoType2 = 1
                                BEGIN 
                                    EXEC @ReturnCode = dbo.ap_dim01_do_type2_precision_queue_step_precision_attribute_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @ChangeStamp, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 1;
                                    IF @ReturnCode <> 0
                                    BEGIN 
                                        RAISERROR ('ap_dim01_do_type2_precision_queue_step_precision_attribute_member returned [%d] Error raised in TRY block.', 16, 1, @ReturnCode);
                                    END;
...
,[TERM_ORDER] = @TermOrder
                                ...
                                ELSE BEGIN 
                                    ...
                                    BEGIN 
                                        COMMIT TRANSACTION PUPD_precisionqueuesTran;
                                    END;
                                    ELSE BEGIN 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_UPDATE_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 77, 79, DEFAULT, DEFAULT, 'S', 0, 'MEMBER_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_precision_queue_step_precision_attribute_member]
	TO [portalapp_role]
GO
