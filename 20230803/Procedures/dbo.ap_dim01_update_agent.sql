...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_agent]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_AGENT' AND ITEM_TYPE_ID = '00001100-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_AGENT' AND ITEM_TYPE_ID = '00001100-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldManagerAgentBizUrn = item.[MANAGER_AGENT_BIZ_URN], 
        @OldUserId = item.[USER_ID], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_AGENT item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'AGENT_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('AGENT_DEPARTMENT_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldManagerAgentBizUrn IS NOT NULL AND @ManagerAgentBizUrn IS NOT NULL AND @OldManagerAgentBizUrn <> @ManagerAgentBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldManagerAgentBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ManagerAgentBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('AGENT_MANAGER_AGENT_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldUserId IS NOT NULL AND @UserId IS NOT NULL AND @OldUserId <> @UserId) 
        OR (@OldUserId IS NULL AND @UserId IS NOT NULL)
        OR (@OldUserId IS NOT NULL AND @UserId IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldUserId AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@UserId AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('AGENT_USER_ID', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_agent @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_agent returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_agent @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_agent returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[MANAGER_AGENT_BIZ_URN] = @ManagerAgentBizUrn        
,[USER_ID] = @UserId        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 1, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
