...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_dialed_number_call_type_member] 
(
...
AS
    ...
    IF @XmlData IS NULL
    BEGIN 
        SELECT @XmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER' AND MEMBER_TYPE_ID = '00000021-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    
    -- Get Custom Default XML
    IF @CustomXmlData IS NULL
    BEGIN 
        SELECT @CustomXmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER' AND MEMBER_TYPE_ID = '00000021-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        IF @ParentItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_CALL_TYPE WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF @ChildItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_DIALED_NUMBER WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                -- Insert the base and derived table in a transaction to ensure an atomic insert.
                BEGIN TRANSACTION PADD_dialednumbercalTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Add the base data
                            INSERT INTO dbo.TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER 
                            (
                             [PARENT_ITEM_BIZ_URN], [CHILD_ITEM_BIZ_URN], [EFFECTIVE_FROM], [EFFECTIVE_TO], [XML_DATA], [SYSTEM], [MODIFIED_BY_ID], [CREATED_BY_ID], [CUSTOM_XML_DATA], [REGION_BIZ_URN], [ITEM] 
                            )
                            SELECT TOP 1 @ParentItemBusinessUrn, @ChildItemBusinessUrn, @EffectiveFrom, @EffectiveTo, @XmlData, @System, @SessionUserId, @SessionUserId, @CustomXmlData, @RegionBizUrn, @Item 
                            WHERE NOT EXISTS (SELECT TOP 1 [MEMBER_URN]
                                              ...
                                              AND [CHILD_ITEM_BIZ_URN] = @ChildItemBusinessUrn                                              AND [ITEM] = @Item 
                                              ORDER BY [MEMBER_URN]);
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_INSERT_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 12, 35, DEFAULT, DEFAULT, 'S', 0, 
                            'MEMBER_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                            IF @InitialTransactionCount = 0 
                            BEGIN 
                                COMMIT TRANSACTION PADD_dialednumbercalTran;
                            END;
                            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_dialed_number_call_type_member]
	TO [portalapp_role]
GO
