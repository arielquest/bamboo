...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_document]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_DOCUMENT' AND ITEM_TYPE_ID = 'EC51C525-F562-482D-836A-AE257D6EA373' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_DOCUMENT' AND ITEM_TYPE_ID = 'EC51C525-F562-482D-836A-AE257D6EA373' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldDocumentType = item.[DOCUMENT_TYPE], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_DOCUMENT item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'DOCUMENT_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldDocumentType IS NOT NULL AND @DocumentType IS NOT NULL AND @OldDocumentType <> @DocumentType) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldDocumentType AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@DocumentType AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('DOCUMENT_DOCUMENT_TYPE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_docTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_document @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_document returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_document @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_document returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[DOCUMENT_TYPE] = @DocumentType        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 112, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_docTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_document]
	TO [portalapp_role]
GO
