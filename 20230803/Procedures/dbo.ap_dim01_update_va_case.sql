...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_va_case]
(
    ...
    @VaAssistantBizUrn               [INT] = -1, 
    @CaseQuestion                    [NVARCHAR](max) = NULL, 
...
AS
    ...
    @OldVaAssistantBizUrn            [INT],
    @OldCaseQuestion                 [NVARCHAR](max),
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_VA_CASE' AND ITEM_TYPE_ID = 'BC92CC12-300F-442C-83DE-1A8BCD86D28F' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_VA_CASE' AND ITEM_TYPE_ID = 'BC92CC12-300F-442C-83DE-1A8BCD86D28F' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldVaAssistantBizUrn = item.[VA_ASSISTANT_BIZ_URN], 
        @OldCaseQuestion = item.[CASE_QUESTION], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_VA_CASE item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'VA_CASE_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldVaAssistantBizUrn IS NOT NULL AND @VaAssistantBizUrn IS NOT NULL AND @OldVaAssistantBizUrn <> @VaAssistantBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldVaAssistantBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@VaAssistantBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('VA_CASE_VA_ASSISTANT_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldCaseQuestion IS NOT NULL AND @CaseQuestion IS NOT NULL AND @OldCaseQuestion <> @CaseQuestion) 
        OR (@OldCaseQuestion IS NULL AND @CaseQuestion IS NOT NULL)
        OR (@OldCaseQuestion IS NOT NULL AND @CaseQuestion IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldCaseQuestion AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@CaseQuestion AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('VA_CASE_CASE_QUESTION', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_va_case @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_va_case returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_va_case @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_va_case returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[VA_ASSISTANT_BIZ_URN] = @VaAssistantBizUrn        
,[CASE_QUESTION] = @CaseQuestion        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 147, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_vTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_va_case]
	TO [portalapp_role]
GO
