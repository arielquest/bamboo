...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_offer]
(
    ...
    @OfferTypeCodeBizUrn             [INT] = -1, 
    @ServiceTypeCodeBizUrn           [INT] = -1, 
...
AS
    ...
    @OldOfferTypeCodeBizUrn          [INT],
    @OldServiceTypeCodeBizUrn        [INT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_OFFER' AND ITEM_TYPE_ID = 'DBC2A99B-996E-4C94-BE3B-AF3CE1E7341D' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_OFFER' AND ITEM_TYPE_ID = 'DBC2A99B-996E-4C94-BE3B-AF3CE1E7341D' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldOfferTypeCodeBizUrn = item.[OFFER_TYPE_CODE_BIZ_URN], 
        @OldServiceTypeCodeBizUrn = item.[SERVICE_TYPE_CODE_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_OFFER item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'OFFER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldOfferTypeCodeBizUrn IS NOT NULL AND @OfferTypeCodeBizUrn IS NOT NULL AND @OldOfferTypeCodeBizUrn <> @OfferTypeCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldOfferTypeCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@OfferTypeCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('OFFER_OFFER_TYPE_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldServiceTypeCodeBizUrn IS NOT NULL AND @ServiceTypeCodeBizUrn IS NOT NULL AND @OldServiceTypeCodeBizUrn <> @ServiceTypeCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldServiceTypeCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ServiceTypeCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('OFFER_SERVICE_TYPE_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_offer @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_offer returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_offer @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_offer returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[OFFER_TYPE_CODE_BIZ_URN] = @OfferTypeCodeBizUrn        
,[SERVICE_TYPE_CODE_BIZ_URN] = @ServiceTypeCodeBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 118, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
