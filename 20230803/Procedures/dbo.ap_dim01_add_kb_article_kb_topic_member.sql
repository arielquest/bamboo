...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_kb_article_kb_topic_member] 
(
...
AS
    ...
    IF @XmlData IS NULL
    BEGIN 
        SELECT @XmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_KB_ARTICLE_KB_TOPIC_MEMBER' AND MEMBER_TYPE_ID = '00000061-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    
    -- Get Custom Default XML
    IF @CustomXmlData IS NULL
    BEGIN 
        SELECT @CustomXmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_KB_ARTICLE_KB_TOPIC_MEMBER' AND MEMBER_TYPE_ID = '00000061-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        IF @ParentItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_KB_TOPIC WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF @ChildItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_KB_ARTICLE WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                -- Insert the base and derived table in a transaction to ensure an atomic insert.
                BEGIN TRANSACTION PADD_kbarticlekbtopiTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Add the base data
                            INSERT INTO dbo.TB_DIM_KB_ARTICLE_KB_TOPIC_MEMBER 
                            (
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_INSERT_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 122, 123, DEFAULT, DEFAULT, 'S', 0, 
                            'MEMBER_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                            IF @InitialTransactionCount = 0 
                            BEGIN 
                                COMMIT TRANSACTION PADD_kbarticlekbtopiTran;
                            END;
                            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_kb_article_kb_topic_member]
	TO [portalapp_role]
GO
