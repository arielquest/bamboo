...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_kb_article] (@SessionId UNIQUEIDENTIFIER, @ItemUrn INT, @ChangeStamp INT OUTPUT)
AS BEGIN 
    SET NOCOUNT ON;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_KB_ARTICLE WITH (UPDLOCK)
        ...
        BEGIN 
            ...
            BEGIN
                BEGIN TRANSACTION PDEL_kbarTran;
                    BEGIN TRY;
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_DELETE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 123, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_DELETED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PDEL_kbarTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_kb_article]
	TO [portalapp_role]
GO
