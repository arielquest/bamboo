...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_expanded_call_variable_pkey]
(    
    ...
    @ECCArray                [BIT] = 0, 
    @VariableEnabled         [BIT] = 1, 
    @GeotelProvided          [BIT] = 0, 
    @MaximumArraySize        [SMALLINT] = NULL, 
    @MaximumLength           [SMALLINT] = 1, 
    @Persistent              [BIT] = 0, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldECCArray                     [BIT], 
                @OldVariableEnabled              [BIT], 
                @OldGeotelProvided               [BIT], 
                @OldMaximumArraySize             [SMALLINT], 
                @OldMaximumLength                [SMALLINT], 
                @OldPersistent                   [BIT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_expanded_call_variable_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00009400-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldECCArray = map.[ECC_ARRAY],
                                @OldVariableEnabled = map.[VARIABLE_ENABLED],
                                @OldGeotelProvided = map.[GEOTEL_PROVIDED],
                                @OldMaximumArraySize = map.[MAXIMUM_ARRAY_SIZE],
                                @OldMaximumLength = map.[MAXIMUM_LENGTH],
                                @OldPersistent = map.[PERSISTENT], 
                        ...
                        FROM dbo.TB_DIM_EXPANDED_CALL_VARIABLE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_EXPANDED_CALL_VARIABLE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_EXPANDED_CALL_VARIABLE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'EXPANDED_CALL_VARIABLE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.EXPANDED_CALL_VARIABLE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldECCArray IS NOT NULL AND @ECCArray IS NOT NULL AND @OldECCArray <> @ECCArray)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldECCArray WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @ECCArray WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_ECC_ARRAY', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldVariableEnabled IS NOT NULL AND @VariableEnabled IS NOT NULL AND @OldVariableEnabled <> @VariableEnabled)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldVariableEnabled WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @VariableEnabled WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_VARIABLE_ENABLED', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldGeotelProvided IS NOT NULL AND @GeotelProvided IS NOT NULL AND @OldGeotelProvided <> @GeotelProvided)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldGeotelProvided WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @GeotelProvided WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_GEOTEL_PROVIDED', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMaximumArraySize IS NOT NULL AND @MaximumArraySize IS NOT NULL AND @OldMaximumArraySize <> @MaximumArraySize)  
                        OR (@OldMaximumArraySize IS NULL AND @MaximumArraySize IS NOT NULL)
                        OR (@OldMaximumArraySize IS NOT NULL AND @MaximumArraySize IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMaximumArraySize AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MaximumArraySize AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_MAXIMUM_ARRAY_SIZE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMaximumLength IS NOT NULL AND @MaximumLength IS NOT NULL AND @OldMaximumLength <> @MaximumLength)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMaximumLength AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MaximumLength AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_MAXIMUM_LENGTH', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldPersistent IS NOT NULL AND @Persistent IS NOT NULL AND @OldPersistent <> @Persistent)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldPersistent WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @Persistent WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('EXPANDED_CALL_VARIABLE_PERSISTENT', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_expandedcallvaPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_EXPANDED_CALL_VARIABLE_PKEY_MAP
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [GEOTEL_PROVIDED] = @GeotelProvided, 
                                    [MAXIMUM_ARRAY_SIZE] = @MaximumArraySize, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 43, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_expandedcallvaPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_expanded_call_variable_pkey]
	TO [portalapp_role]
GO
