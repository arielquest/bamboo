...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_scl_adapter]
(
...
BEGIN 
    ...
    BEGIN 
        ...
        BEGIN
            ...
            BEGIN TRANSACTION PADD_scladTran;
                WHILE @deadlockRetry < 3
                BEGIN 
                    SAVE TRANSACTION deadlockSavePoint;
                    BEGIN TRY;
                        -- Add the base data
                        INSERT INTO dbo.TB_DIM_SCL_ADAPTER
                        (
                       ...
                       , [ADAPTER_TYPE_SUB_CODE_BIZ_URN]
                        ...
                        @XmlData, @TimeZoneUrn, @TenantBizUrn, @HierarchyParentUrn, @CustomXmlData, @SessionUserId, @SessionUserId
                         , @AdapterTypeCodeBizUrn
                         , @AdapterTypeSubCodeBizUrn
                        );

                        SELECT @ItemUrn = SCOPE_IDENTITY();
            
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_INSERT_COMMITTED', DEFAULT, @ItemUrn, DEFAULT, 142, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PADD_scladTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_scl_adapter]
	TO [portalapp_role]
GO
