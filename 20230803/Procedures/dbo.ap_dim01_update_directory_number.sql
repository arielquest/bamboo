...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_directory_number]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_DIRECTORY_NUMBER' AND ITEM_TYPE_ID = '00011100-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_DIRECTORY_NUMBER' AND ITEM_TYPE_ID = '00011100-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldRoutePartitionBizUrn = item.[ROUTE_PARTITION_BIZ_URN], 
        @OldResourceTemplateId = item.[RESOURCE_TEMPLATE_ID], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_DIRECTORY_NUMBER item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'DIRECTORY_NUMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldRoutePartitionBizUrn IS NOT NULL AND @RoutePartitionBizUrn IS NOT NULL AND @OldRoutePartitionBizUrn <> @RoutePartitionBizUrn) 
        OR (@OldRoutePartitionBizUrn IS NULL AND @RoutePartitionBizUrn IS NOT NULL)
        OR (@OldRoutePartitionBizUrn IS NOT NULL AND @RoutePartitionBizUrn IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldRoutePartitionBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@RoutePartitionBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('DIRECTORY_NUMBER_ROUTE_PARTITION_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('DIRECTORY_NUMBER_RESOURCE_TEMPLATE_ID', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_directorynTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_directory_number @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_directory_number returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_directory_number @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_directory_number returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[ROUTE_PARTITION_BIZ_URN] = @RoutePartitionBizUrn        
,[RESOURCE_TEMPLATE_ID] = @ResourceTemplateId        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 38, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_directorynTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_directory_number]
	TO [portalapp_role]
GO
