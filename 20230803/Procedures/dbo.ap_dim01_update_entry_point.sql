...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_entry_point]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_ENTRY_POINT' AND ITEM_TYPE_ID = '9D660B17-9AF3-43EC-A6B8-EC8E0D74C2A0' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_ENTRY_POINT' AND ITEM_TYPE_ID = '9D660B17-9AF3-43EC-A6B8-EC8E0D74C2A0' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldQueueBizUrn = item.[QUEUE_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_ENTRY_POINT item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'ENTRY_POINT_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldQueueBizUrn IS NOT NULL AND @QueueBizUrn IS NOT NULL AND @OldQueueBizUrn <> @QueueBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldQueueBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@QueueBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('ENTRY_POINT_QUEUE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
            SET @DoStandardType2 = 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_entryTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_entry_point @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_entry_point returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_entry_point @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_entry_point returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[QUEUE_BIZ_URN] = @QueueBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 115, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_entryTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_entry_point]
	TO [portalapp_role]
GO
