...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_chargeband_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode AS [INTEGER] = 0,
                @Status AS [CHAR](1),
                @ItemTypeId AS [UNIQUEIDENTIFIER] = '00001900-1000-0000-0000-000000000010',
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    IF NOT EXISTS(SELECT 1 
                                  FROM dbo.TB_DIM_CHARGEBAND WITH(READUNCOMMITTED) 
                    ...
                    ELSE BEGIN 
                        ...
                        IF EXISTS(SELECT 1 
                                  FROM [dbo].[TB_DIM_CHARGEBAND_PKEY_MAP] WITH(READUNCOMMITTED) 
                        ...
                        ELSE BEGIN 
                            ...
                            BEGIN TRANSACTION PADD_chargPKEYTran;
                            BEGIN TRY;
                                INSERT INTO [dbo].[TB_DIM_CHARGEBAND_PKEY_MAP](
                                ITEM_BIZ_URN, 
                                CLUSTER_RESOURCE_ID, 
                                STATUS, 
                                EFFECTIVE_FROM, 
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, DEFAULT, 15, DEFAULT, @ClusterResourceId, DEFAULT, 'S', 0, 'DIMENSION_ADDED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                
                            IF @InitialTransactionCount = 0 
                            BEGIN 
                                COMMIT TRANSACTION PADD_chargPKEYTran;
                            END;
                            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_chargeband_pkey]
	TO [portalapp_role]
GO
