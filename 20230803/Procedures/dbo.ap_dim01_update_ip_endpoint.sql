...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ip_endpoint]
(
    ...
    @ProductCodeBizUrn               [INT] = -1, 
    @ProtocolCodeBizUrn              [INT] = -1, 
    @OwnerPersonBizUrn               [INT] = -1, 
    @ResourceTemplateId              [UNIQUEIDENTIFIER] = NULL, 
    @UserControlsDevice              [BIT] = 0, 
...
AS
    ...
    @OldProductCodeBizUrn            [INT],
    @OldProtocolCodeBizUrn           [INT],
    @OldOwnerPersonBizUrn            [INT],
    @OldResourceTemplateId           [UNIQUEIDENTIFIER],
    @OldUserControlsDevice           [BIT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_IP_ENDPOINT' AND ITEM_TYPE_ID = '00004500-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_IP_ENDPOINT' AND ITEM_TYPE_ID = '00004500-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldProductCodeBizUrn = item.[PRODUCT_CODE_BIZ_URN], 
        @OldProtocolCodeBizUrn = item.[PROTOCOL_CODE_BIZ_URN], 
        @OldOwnerPersonBizUrn = item.[OWNER_PERSON_BIZ_URN], 
        @OldResourceTemplateId = item.[RESOURCE_TEMPLATE_ID], 
        @OldUserControlsDevice = item.[USER_CONTROLS_DEVICE], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_IP_ENDPOINT item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'IP_ENDPOINT_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldProductCodeBizUrn IS NOT NULL AND @ProductCodeBizUrn IS NOT NULL AND @OldProductCodeBizUrn <> @ProductCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldProductCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ProductCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('IP_ENDPOINT_PRODUCT_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldProtocolCodeBizUrn IS NOT NULL AND @ProtocolCodeBizUrn IS NOT NULL AND @OldProtocolCodeBizUrn <> @ProtocolCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldProtocolCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ProtocolCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('IP_ENDPOINT_PROTOCOL_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldOwnerPersonBizUrn IS NOT NULL AND @OwnerPersonBizUrn IS NOT NULL AND @OldOwnerPersonBizUrn <> @OwnerPersonBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldOwnerPersonBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@OwnerPersonBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('IP_ENDPOINT_OWNER_PERSON_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldResourceTemplateId IS NOT NULL AND @ResourceTemplateId IS NOT NULL AND @OldResourceTemplateId <> @ResourceTemplateId) 
        OR (@OldResourceTemplateId IS NULL AND @ResourceTemplateId IS NOT NULL)
        OR (@OldResourceTemplateId IS NOT NULL AND @ResourceTemplateId IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldResourceTemplateId AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ResourceTemplateId AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('IP_ENDPOINT_RESOURCE_TEMPLATE_ID', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldUserControlsDevice IS NOT NULL AND @UserControlsDevice IS NOT NULL AND @OldUserControlsDevice <> @UserControlsDevice) 
        BEGIN      
            SET @AuditTempOld = CASE @OldUserControlsDevice WHEN 1 THEN 'Yes' ELSE 'No' END;
            SET @AuditTempNew = CASE @UserControlsDevice WHEN 1 THEN 'Yes' ELSE 'No' END;
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('IP_ENDPOINT_USER_CONTROLS_DEVICE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_ipendTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_ip_endpoint @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_ip_endpoint returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_ip_endpoint @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_ip_endpoint returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[PRODUCT_CODE_BIZ_URN] = @ProductCodeBizUrn        
,[PROTOCOL_CODE_BIZ_URN] = @ProtocolCodeBizUrn        
,[OWNER_PERSON_BIZ_URN] = @OwnerPersonBizUrn        
,[RESOURCE_TEMPLATE_ID] = @ResourceTemplateId        
,[USER_CONTROLS_DEVICE] = @UserControlsDevice        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 53, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_ipendTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ip_endpoint]
	TO [portalapp_role]
GO
