...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_dialed_number_call_type_member_pkey]
(
    ...
    @CLIWildCard             [NVARCHAR](60) = NULL,
    @CLIWildCardType         [INT] = NULL,
    @CEDWildCard             [NVARCHAR](60) = NULL,
...
BEGIN 
    ...
    DECLARE @ReturnCode                     AS [INTEGER],
            ...
            @OldCLIWildCard                  [NVARCHAR](60), 
            @OldCLIWildCardType              [INT], 
            @OldCEDWildCard                  [NVARCHAR](60), 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_update_dialed_number_call_type_member_pkey');
        END;
        ...
        BEGIN    
            ...
            SET @MemberTypeId = '00000021-1000-0000-0000-0000000000c0';
            SELECT @MemberTypeIsProvisionable = ISNULL((SELECT TOP 1 1 
                                                        FROM dbo.TE_DIM_ITEM_TYPE 
                                                        WHERE ITEM_TYPE_ID IN ('00001500-1000-0000-0000-000000000010', '00002400-1000-0000-0000-000000000010')
            ...
            SELECT  @OldEffectiveFrom = pm.[EFFECTIVE_FROM],
                    ...
                    @OldCLIWildCard = pm.[CLI_WILD_CARD], 
                    @OldCLIWildCardType = pm.[CLI_WILD_CARD_TYPE], 
                    @OldCEDWildCard = pm.[CED_WILD_CARD], 
                    @OldDescription = pm.[DESCRIPTION], 
            ...
            FROM dbo.TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER_PKEY_MAP AS pm WITH(UPDLOCK)
            INNER JOIN dbo.TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER AS m WITH(READUNCOMMITTED) ON pm.MEMBER_BIZ_URN = m.MEMBER_BIZ_URN AND m.LATEST = 1
            ...
            BEGIN 
                ...
                WHERE TABLE_NAME = 'TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER_PKEY_MAP' 
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'DIALED_NUMBER_CALL_TYPE_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'custom.DIALED_NUMBER_CALL_TYPE_MEMBER_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
            SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
            SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
            ...
            IF (@OldCLIWildCard IS NOT NULL AND @CLIWildCard IS NOT NULL AND @OldCLIWildCard <> @CLIWildCard) 
 
            OR (@OldCLIWildCard IS NULL AND @CLIWildCard IS NOT NULL)
            OR (@OldCLIWildCard IS NOT NULL AND @CLIWildCard IS NULL) 
            BEGIN  
                SET @AuditTempOld = CAST(@OldCLIWildCard AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@CLIWildCard AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('DIALED_NUMBER_CALL_TYPE_MEMBER_CLI_WILD_CARD', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            
 
            IF (@OldCLIWildCardType IS NOT NULL AND @CLIWildCardType IS NOT NULL AND @OldCLIWildCardType <> @CLIWildCardType) 
 
            BEGIN  
                SET @AuditTempOld = CAST(@OldCLIWildCardType AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@CLIWildCardType AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('DIALED_NUMBER_CALL_TYPE_MEMBER_CLI_WILD_CARD_TYPE', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            
 
            IF (@OldCEDWildCard IS NOT NULL AND @CEDWildCard IS NOT NULL AND @OldCEDWildCard <> @CEDWildCard) 
 
            OR (@OldCEDWildCard IS NULL AND @CEDWildCard IS NOT NULL)
            OR (@OldCEDWildCard IS NOT NULL AND @CEDWildCard IS NULL) 
            BEGIN  
                SET @AuditTempOld = CAST(@OldCEDWildCard AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@CEDWildCard AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('DIALED_NUMBER_CALL_TYPE_MEMBER_CED_WILD_CARD', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            
 
            IF (@OldDescription IS NOT NULL AND @Description IS NOT NULL AND @OldDescription <> @Description) 
 
            OR (@OldDescription IS NULL AND @Description IS NOT NULL)
            OR (@OldDescription IS NOT NULL AND @Description IS NULL) 
            BEGIN  
                SET @AuditTempOld = CAST(@OldDescription AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@Description AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('DIALED_NUMBER_CALL_TYPE_MEMBER_DESCRIPTION', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            ...
            BEGIN        
                ...
                BEGIN 
                    RAISERROR('ap_dim01_update_dialed_number_call_type_member_pkey: Cannot perform provisionable update on deleted or deleting pkey map [%d]', 16, 1, @PkeyMapUrn) WITH LOG;
                END;
                ...
                BEGIN TRANSACTION PUPD_dialednumbercaPKEYTran;       
                -- Update the pkey  record
                UPDATE dbo.TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER_PKEY_MAP
                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                    ...
                    [CLI_WILD_CARD] = @CLIWildCard, 
                    [CLI_WILD_CARD_TYPE] = @CLIWildCardType, 
                    [CED_WILD_CARD] = @CEDWildCard, 
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_update_dialed_number_call_type_member_pkey: Error updating member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_update_dialed_number_call_type_member_pkey: Concurrency conflict occured whilst attempting to update member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 12, 35, @ClusterResourceId, NULL, 'S', 0, 
                    'MEMBER_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
            
                IF @InitialTransactionCount = 0 
                BEGIN 
                    COMMIT TRANSACTION PUPD_dialednumbercaPKEYTran;
                END;
                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_dialed_number_call_type_member_pkey]
	TO [portalapp_role]
GO
