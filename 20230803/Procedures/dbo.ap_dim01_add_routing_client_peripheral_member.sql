...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_routing_client_peripheral_member] 
(
...
AS
    ...
    IF @XmlData IS NULL
    BEGIN 
        SELECT @XmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_ROUTING_CLIENT_PERIPHERAL_MEMBER' AND MEMBER_TYPE_ID = '00000009-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    
    -- Get Custom Default XML
    IF @CustomXmlData IS NULL
    BEGIN 
        SELECT @CustomXmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_ROUTING_CLIENT_PERIPHERAL_MEMBER' AND MEMBER_TYPE_ID = '00000009-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        IF @ParentItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_PERIPHERAL WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF @ChildItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_ROUTING_CLIENT WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                -- Insert the base and derived table in a transaction to ensure an atomic insert.
                BEGIN TRANSACTION PADD_routingclientpeTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Add the base data
                            INSERT INTO dbo.TB_DIM_ROUTING_CLIENT_PERIPHERAL_MEMBER 
                            (
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_INSERT_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 73, 88, DEFAULT, DEFAULT, 'S', 0, 
                            'MEMBER_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                            IF @InitialTransactionCount = 0 
                            BEGIN 
                                COMMIT TRANSACTION PADD_routingclientpeTran;
                            END;
                            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_routing_client_peripheral_member]
	TO [portalapp_role]
GO
