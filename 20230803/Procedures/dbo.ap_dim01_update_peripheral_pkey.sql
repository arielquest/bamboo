...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_peripheral_pkey]
(    
    ...
    @AnsweredShortCallsThreshold  [INT] = NULL, 
    @AbandonedCallWaitTime   [SMALLINT] = NULL, 
    @ServiceLevelType        [SMALLINT] = NULL, 
    @PeripheralServiceLevelType  [SMALLINT] = NULL, 
    @ServiceLevelThreshold   [SMALLINT] = NULL, 
    @AvailableHoldoffDelay   [SMALLINT] = NULL, 
    @ClientType              [SMALLINT] = NULL, 
    @SubSkillGroupMask       [NVARCHAR](64) = NULL, 
    @AutoConfigured          [BIT] = 0, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldPeripheralName               [VARCHAR](32), 
                @OldLocation                     [VARCHAR](32), 
                @OldAnsweredShortCallsThreshold  [INT], 
                @OldAbandonedCallWaitTime        [SMALLINT], 
                @OldServiceLevelType             [SMALLINT], 
                @OldPeripheralServiceLevelType   [SMALLINT], 
                @OldServiceLevelThreshold        [SMALLINT], 
                @OldAvailableHoldoffDelay        [SMALLINT], 
                @OldClientType                   [SMALLINT], 
                @OldSubSkillGroupMask            [NVARCHAR](64), 
                @OldAutoConfigured               [BIT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_peripheral_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00006200-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldLocation = map.[LOCATION],
                                @OldAnsweredShortCallsThreshold = map.[ANSWERED_SHORT_CALLS_THRESHOLD],
                                @OldAbandonedCallWaitTime = map.[ABANDONED_CALL_WAIT_TIME],
                                @OldServiceLevelType = map.[SERVICE_LEVEL_TYPE],
                                @OldPeripheralServiceLevelType = map.[PERIPHERAL_SERVICE_LEVEL_TYPE],
                                @OldServiceLevelThreshold = map.[SERVICE_LEVEL_THRESHOLD],
                                @OldAvailableHoldoffDelay = map.[AVAILABLE_HOLDOFF_DELAY],
                                @OldClientType = map.[CLIENT_TYPE],
                                @OldSubSkillGroupMask = map.[SUB_SKILLGROUP_MASK],
                                @OldAutoConfigured = map.[AUTO_CONFIGURED], 
                        ...
                        FROM dbo.TB_DIM_PERIPHERAL_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_PERIPHERAL AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_PERIPHERAL_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'PERIPHERAL_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.PERIPHERAL_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_PERIPHERAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldLocation IS NOT NULL AND @Location IS NOT NULL AND @OldLocation COLLATE Latin1_General_CS_AS <> @Location COLLATE Latin1_General_CS_AS)  
                        OR (@OldLocation IS NULL AND @Location IS NOT NULL)
                        OR (@OldLocation IS NOT NULL AND @Location IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldLocation AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Location AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_LOCATION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAnsweredShortCallsThreshold IS NOT NULL AND @AnsweredShortCallsThreshold IS NOT NULL AND @OldAnsweredShortCallsThreshold <> @AnsweredShortCallsThreshold)  
                        OR (@OldAnsweredShortCallsThreshold IS NULL AND @AnsweredShortCallsThreshold IS NOT NULL)
                        OR (@OldAnsweredShortCallsThreshold IS NOT NULL AND @AnsweredShortCallsThreshold IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldAnsweredShortCallsThreshold AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@AnsweredShortCallsThreshold AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_ANSWERED_SHORT_CALLS_THRESHOLD', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAbandonedCallWaitTime IS NOT NULL AND @AbandonedCallWaitTime IS NOT NULL AND @OldAbandonedCallWaitTime <> @AbandonedCallWaitTime)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldAbandonedCallWaitTime AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@AbandonedCallWaitTime AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_ABANDONED_CALL_WAIT_TIME', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldServiceLevelType IS NOT NULL AND @ServiceLevelType IS NOT NULL AND @OldServiceLevelType <> @ServiceLevelType)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldServiceLevelType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ServiceLevelType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_SERVICE_LEVEL_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldPeripheralServiceLevelType IS NOT NULL AND @PeripheralServiceLevelType IS NOT NULL AND @OldPeripheralServiceLevelType <> @PeripheralServiceLevelType)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldPeripheralServiceLevelType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@PeripheralServiceLevelType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_PERIPHERAL_SERVICE_LEVEL_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldServiceLevelThreshold IS NOT NULL AND @ServiceLevelThreshold IS NOT NULL AND @OldServiceLevelThreshold <> @ServiceLevelThreshold)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldServiceLevelThreshold AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ServiceLevelThreshold AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_SERVICE_LEVEL_THRESHOLD', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAvailableHoldoffDelay IS NOT NULL AND @AvailableHoldoffDelay IS NOT NULL AND @OldAvailableHoldoffDelay <> @AvailableHoldoffDelay)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldAvailableHoldoffDelay AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@AvailableHoldoffDelay AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_AVAILABLE_HOLDOFF_DELAY', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldClientType IS NOT NULL AND @ClientType IS NOT NULL AND @OldClientType <> @ClientType)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldClientType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ClientType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_CLIENT_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldSubSkillGroupMask IS NOT NULL AND @SubSkillGroupMask IS NOT NULL AND @OldSubSkillGroupMask COLLATE Latin1_General_CS_AS <> @SubSkillGroupMask COLLATE Latin1_General_CS_AS)  
                        OR (@OldSubSkillGroupMask IS NULL AND @SubSkillGroupMask IS NOT NULL)
                        OR (@OldSubSkillGroupMask IS NOT NULL AND @SubSkillGroupMask IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldSubSkillGroupMask AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@SubSkillGroupMask AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_SUB_SKILLGROUP_MASK', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAutoConfigured IS NOT NULL AND @AutoConfigured IS NOT NULL AND @OldAutoConfigured <> @AutoConfigured)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldAutoConfigured WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @AutoConfigured WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PERIPHERAL_AUTO_CONFIGURED', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_peripPKEYTran;
                            BEGIN TRY;
                                ...
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [LOCATION] = @Location, 
                                    [ANSWERED_SHORT_CALLS_THRESHOLD] = @AnsweredShortCallsThreshold, 
                                    [ABANDONED_CALL_WAIT_TIME] = @AbandonedCallWaitTime, 
                                    [SERVICE_LEVEL_TYPE] = @ServiceLevelType, 
                                    [PERIPHERAL_SERVICE_LEVEL_TYPE] = @PeripheralServiceLevelType, 
                                    [SERVICE_LEVEL_THRESHOLD] = @ServiceLevelThreshold, 
                                    [AVAILABLE_HOLDOFF_DELAY] = @AvailableHoldoffDelay, 
                                    [CLIENT_TYPE] = @ClientType, 
                                    [SUB_SKILLGROUP_MASK] = @SubSkillGroupMask, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 73, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_peripPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_peripheral_pkey]
	TO [portalapp_role]
GO
