...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_kb_article_pkey]
(    
    ...
    @ArticleDisplayID        [NVARCHAR](50) = NULL, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldArticleDisplayID             [NVARCHAR](50), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_kb_article_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = 'e349b610-402f-4a1a-b555-ba26abdc6f30';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldKeywords = map.[KEYWORDS],
                                @OldArticleDisplayID = map.[ARTICLE_DISPLAY_ID], 
                        ...
                        FROM dbo.TB_DIM_KB_ARTICLE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_KB_ARTICLE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_KB_ARTICLE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'KB_ARTICLE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.KB_ARTICLE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('KB_ARTICLE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('KB_ARTICLE_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldKeywords IS NOT NULL AND @Keywords IS NOT NULL AND @OldKeywords COLLATE Latin1_General_CS_AS <> @Keywords COLLATE Latin1_General_CS_AS)  
                        OR (@OldKeywords IS NULL AND @Keywords IS NOT NULL)
                        OR (@OldKeywords IS NOT NULL AND @Keywords IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldKeywords AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Keywords AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('KB_ARTICLE_KEYWORDS', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldArticleDisplayID IS NOT NULL AND @ArticleDisplayID IS NOT NULL AND @OldArticleDisplayID COLLATE Latin1_General_CS_AS <> @ArticleDisplayID COLLATE Latin1_General_CS_AS)  
                        OR (@OldArticleDisplayID IS NULL AND @ArticleDisplayID IS NOT NULL)
                        OR (@OldArticleDisplayID IS NOT NULL AND @ArticleDisplayID IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldArticleDisplayID AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ArticleDisplayID AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('KB_ARTICLE_ARTICLE_DISPLAY_ID', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_kbarPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_KB_ARTICLE_PKEY_MAP
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [KEYWORDS] = @Keywords, 
                                    [ARTICLE_DISPLAY_ID] = @ArticleDisplayID, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 123, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_kbarPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_kb_article_pkey]
	TO [portalapp_role]
GO
