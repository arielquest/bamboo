...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_network_trunk_group]
(
...
BEGIN 
    ...
    BEGIN 
        ...
        BEGIN
            ...
            BEGIN TRANSACTION PADD_networktrunkgroTran;
                WHILE @deadlockRetry < 3
                BEGIN 
                    SAVE TRANSACTION deadlockSavePoint;
                    BEGIN TRY;
                        -- Add the base data
                        INSERT INTO dbo.TB_DIM_NETWORK_TRUNK_GROUP
                        (
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_INSERT_COMMITTED', DEFAULT, @ItemUrn, DEFAULT, 69, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PADD_networktrunkgroTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_network_trunk_group]
	TO [portalapp_role]
GO
