...
GO
CREATE PROCEDURE [dbo].[ap_dim01_do_type2_user_group]
    ...
    BEGIN 
        ...
        BEGIN 
            ...
            BEGIN 
                ...
                BEGIN TRANSACTION PTYP_userTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Insert the new type-2 item record
                            INSERT INTO dbo.TB_DIM_USER_GROUP ([ITEM_BIZ_URN], [SKIP_LEVEL], [FOLDER_ID], [OWNER_ID], [ITEM_SUBTYPE_ID], [NAME], [DESCRIPTION],
[AGENT_TEAM_BIZ_URN],
[SKILLGROUP_BIZ_URN],
...
[DESCRIPTION], 
                                ...
                                UPDATE dbo.TB_DIM_USER_GROUP_PKEY_MAP 
                                SET STATUS = 'D', 
                                ...
                                INSERT INTO dbo.TB_DIM_USER_GROUP_PKEY_MAP  ([PKEY_MAP_BIZ_URN], [ITEM_BIZ_URN], [CLUSTER_RESOURCE_ID], [PKEY1], [PKEY2],                            
[INTERNAL_NAME], 
[DESCRIPTION], 
...
[INTERNAL_NAME], [DESCRIPTION],                                 [STATUS], [EFFECTIVE_FROM], [EFFECTIVE_TO], [CREATED_BY_ID], [MODIFIED_BY_ID], [MODIFIED_DATE], [CREATION_DATE],
                           ...
                           BEGIN 
                               ...
                               VALUES('AT_DIMENSION_TYPE_2_PERFORMED', @ItemBusinessUrn, @NewItemUrn, 114, 'DIMENSION_TYPE_2_CHANGE');
                           END;
                           ELSE BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)                
                               VALUES('AT_DIMENSION_TYPE_2_TOMBSTONE_PERFORMED', @ItemBusinessUrn, @ItemUrn, 114, 'DIMENSION_TYPE_2_CHANGE');
                           END;
            
                           IF @MaintainBusinessKey = 0
                           BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)
                               VALUES('AT_DIMENSION_TYPE_2_REVIVAL_PERFORMED', @NewItemBusinessUrn, @ItemUrn, 114, 'DIMENSION_TYPE_2_CHANGE');
                           ...
                           BEGIN 
                               COMMIT TRANSACTION PTYP_userTran;
                           END;
                           ELSE BEGIN 
    ...
    ELSE BEGIN 
        ...
        BEGIN 
            ...
            END;
        END;
        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_TYPE_2_PERFORMED', DEFAULT, @ItemBusinessUrn, DEFAULT, 114, DEFAULT, DEFAULT, DEFAULT, 'F', 0, 'DIMENSION_TYPE_2_CHANGE', NULL, DEFAULT, DEFAULT, @AuditUrn out;
        RAISERROR(@ErrorMessage, 16, 1, @ReturnCode) WITH LOG;
    END;
    RETURN @ReturnCode;
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_do_type2_user_group]
	TO [portalapp_role]
GO
