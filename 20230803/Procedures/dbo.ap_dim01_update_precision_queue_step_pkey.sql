...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_precision_queue_step_pkey]
(    
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_precision_queue_step_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00039572-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldStepOrder = map.[STEP_ORDER],
                                @OldWaitTime = map.[WAIT_TIME],
                                @OldConsiderIf = map.[CONSIDER_IF],
                                @OldNextStep = map.[NEXT_STEP], 
                        ...
                        FROM dbo.TB_DIM_PRECISION_QUEUE_STEP_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_PRECISION_QUEUE_STEP AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_STEP_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'PRECISION_QUEUE_STEP_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.PRECISION_QUEUE_STEP_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_STEP_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldStepOrder IS NOT NULL AND @StepOrder IS NOT NULL AND @OldStepOrder <> @StepOrder)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldStepOrder AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@StepOrder AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_STEP_STEP_ORDER', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldWaitTime IS NOT NULL AND @WaitTime IS NOT NULL AND @OldWaitTime <> @WaitTime)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldWaitTime AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@WaitTime AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_STEP_WAIT_TIME', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldConsiderIf IS NOT NULL AND @ConsiderIf IS NOT NULL AND @OldConsiderIf COLLATE Latin1_General_CS_AS <> @ConsiderIf COLLATE Latin1_General_CS_AS)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldConsiderIf AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ConsiderIf AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_STEP_CONSIDER_IF', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldNextStep IS NOT NULL AND @NextStep IS NOT NULL AND @OldNextStep <> @NextStep)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldNextStep WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @NextStep WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_QUEUE_STEP_NEXT_STEP', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRY;
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 79, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_precision_queue_step_pkey]
	TO [portalapp_role]
GO
