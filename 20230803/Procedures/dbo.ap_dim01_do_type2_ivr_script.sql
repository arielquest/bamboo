...
GO
CREATE PROCEDURE [dbo].[ap_dim01_do_type2_ivr_script]
    ...
    BEGIN 
        ...
        BEGIN 
            ...
            BEGIN 
                ...
                BEGIN TRANSACTION PTYP_ivrsTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Insert the new type-2 item record
                            INSERT INTO dbo.TB_DIM_IVR_SCRIPT ([ITEM_BIZ_URN], [SKIP_LEVEL], [FOLDER_ID], [OWNER_ID], [ITEM_SUBTYPE_ID], [NAME], [DESCRIPTION],
...
[DESCRIPTION], 
                                ...
                                UPDATE dbo.TB_DIM_IVR_SCRIPT_PKEY_MAP 
                                SET STATUS = 'D', 
                                ...
                                INSERT INTO dbo.TB_DIM_IVR_SCRIPT_PKEY_MAP  ([PKEY_MAP_BIZ_URN], [ITEM_BIZ_URN], [CLUSTER_RESOURCE_ID], [PKEY1], [PKEY2],                            
[SCRIPT_NAME], 
[DESCRIPTION], 
...
[SCRIPT_NAME], [DESCRIPTION],                                 [STATUS], [EFFECTIVE_FROM], [EFFECTIVE_TO], [CREATED_BY_ID], [MODIFIED_BY_ID], [MODIFIED_DATE], [CREATION_DATE],
                                [PURGED], [SYSTEM], [LATEST], [SOURCE_CHANGE_STAMP], [XML_DATA], [CUSTOM_XML_DATA]
                                FROM #PKEY_MAP_ROWS;
                           ...
                           BEGIN 
                               ...
                               VALUES('AT_DIMENSION_TYPE_2_PERFORMED', @ItemBusinessUrn, @NewItemUrn, 59, 'DIMENSION_TYPE_2_CHANGE');
                           END;
                           ELSE BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)                
                               VALUES('AT_DIMENSION_TYPE_2_TOMBSTONE_PERFORMED', @ItemBusinessUrn, @ItemUrn, 59, 'DIMENSION_TYPE_2_CHANGE');
                           END;
            
                           IF @MaintainBusinessKey = 0
                           BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)
                               VALUES('AT_DIMENSION_TYPE_2_REVIVAL_PERFORMED', @NewItemBusinessUrn, @ItemUrn, 59, 'DIMENSION_TYPE_2_CHANGE');
                               ...
                               DECLARE ivr_routing_target_ivr_script_member_cursor CURSOR FOR
                               SELECT MEMBER_URN, MEMBER_BIZ_URN, CHANGE_STAMP
                               FROM dbo.TB_DIM_IVR_ROUTING_TARGET_IVR_SCRIPT_MEMBER        
                               WHERE PARENT_ITEM_BIZ_URN = @ItemBusinessUrn 
                               AND MEMBER_BIZ_URN IN (SELECT MEMBER_BIZ_URN FROM dbo.TB_DIM_IVR_ROUTING_TARGET_IVR_SCRIPT_MEMBER_PKEY_MAP WHERE STATUS IN ('E', 'S', 'P', 'R'));
                               OPEN ivr_routing_target_ivr_script_member_cursor;
                               FETCH NEXT FROM ivr_routing_target_ivr_script_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               WHILE @@FETCH_STATUS = 0
                               BEGIN 
                                   EXEC @ReturnCode = dbo.ap_dim01_do_type2_ivr_routing_target_ivr_script_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @MemberChangeStamp, @EffectiveFrom, NULL, NULL, @MaintainBusinessKey;
                    
                                   IF @ReturnCode <> 0
                                   BEGIN 
                                       RAISERROR ('dbo.ap_dim01_do_type2_ivr_routing_target_ivr_script_member returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                   END;
                    
                                   -- Update the new member to point at the new item
                                   UPDATE dbo.TB_DIM_IVR_ROUTING_TARGET_IVR_SCRIPT_MEMBER
                                   SET PARENT_ITEM_BIZ_URN = @NewItemUrn 
                                   WHERE MEMBER_URN = @MemberUrn;
                    
                                   FETCH NEXT FROM ivr_routing_target_ivr_script_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               END;
                               CLOSE ivr_routing_target_ivr_script_member_cursor;
                               DEALLOCATE ivr_routing_target_ivr_script_member_cursor;
                           END;
                           ...
                           BEGIN 
                               COMMIT TRANSACTION PTYP_ivrsTran;
                           END;
                           ELSE BEGIN 
    ...
    ELSE BEGIN 
        ...
        BEGIN 
            ...
            END;
        END;
        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_TYPE_2_PERFORMED', DEFAULT, @ItemBusinessUrn, DEFAULT, 59, DEFAULT, DEFAULT, DEFAULT, 'F', 0, 'DIMENSION_TYPE_2_CHANGE', NULL, DEFAULT, DEFAULT, @AuditUrn out;
        RAISERROR(@ErrorMessage, 16, 1, @ReturnCode) WITH LOG;
    END;
    RETURN @ReturnCode;
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_do_type2_ivr_script]
	TO [portalapp_role]
GO
