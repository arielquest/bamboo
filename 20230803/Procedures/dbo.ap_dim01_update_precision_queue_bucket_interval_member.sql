...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_precision_queue_bucket_interval_member] 
(
...
AS
    ...
    BEGIN 
        ...
        WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_BUCKET_INTERVAL_MEMBER' AND MEMBER_TYPE_ID = '00039582-1000-0000-0000-000000000010' AND CUSTOM = 0;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_ADM_XML_DEFAULTS 
        WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_BUCKET_INTERVAL_MEMBER' AND MEMBER_TYPE_ID = '00039582-1000-0000-0000-000000000010' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_PRECISION_QUEUE_BUCKET_INTERVAL_MEMBER member
        ...
        IF EXISTS(SELECT 1 FROM dbo.TB_DIM_BUCKET_INTERVAL WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            ...
            BEGIN 
                ...
                INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                SELECT 'PRECISION_QUEUE_BUCKET_INTERVAL_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
                ...
                BEGIN 
                    -- Update the tables
                    BEGIN TRANSACTION PUPD_precisionqueuebTran;
                        ...
                        BEGIN 
                            ...
                            BEGIN TRY;
                                ...
                                IF @DoType2 = 1
                                BEGIN 
                                    EXEC @ReturnCode = dbo.ap_dim01_do_type2_precision_queue_bucket_interval_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @ChangeStamp, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 1;
                                    IF @ReturnCode <> 0
                                    BEGIN 
                                        RAISERROR ('ap_dim01_do_type2_precision_queue_bucket_interval_member returned [%d] Error raised in TRY block.', 16, 1, @ReturnCode);
                                    END;
                                ...
                                ELSE BEGIN 
                                    ...
                                    BEGIN 
                                        COMMIT TRANSACTION PUPD_precisionqueuebTran;
                                    END;
                                    ELSE BEGIN 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_UPDATE_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 8, 78, DEFAULT, DEFAULT, 'S', 0, 'MEMBER_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_precision_queue_bucket_interval_member]
	TO [portalapp_role]
GO
