...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_precision_attribute_pkey]
(    
    ...
    @AttributeDataType       [INT] = NULL, 
    @MinimumValue            [VARCHAR](255) = NULL, 
    @MaximumValue            [VARCHAR](255) = NULL, 
    @DefaultValue            [VARCHAR](255) = NULL, 
    @AppearsOnDesktop        [BIT] = NULL, 
    @SettableByAgent         [BIT] = NULL, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldAttributeDataType            [INT], 
                @OldMinimumValue                 [VARCHAR](255), 
                @OldMaximumValue                 [VARCHAR](255), 
                @OldDefaultValue                 [VARCHAR](255), 
                @OldAppearsOnDesktop             [BIT], 
                @OldSettableByAgent              [BIT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_precision_attribute_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00039602-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldAttributeDataType = map.[ATTRIBUTE_DATA_TYPE],
                                @OldMinimumValue = map.[MINIMUM_VALUE],
                                @OldMaximumValue = map.[MAXIMUM_VALUE],
                                @OldDefaultValue = map.[DEFAULT_VALUE],
                                @OldAppearsOnDesktop = map.[APPEARS_ON_DESKTOP],
                                @OldSettableByAgent = map.[SETTABLE_BY_AGENT], 
                        ...
                        FROM dbo.TB_DIM_PRECISION_ATTRIBUTE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_PRECISION_ATTRIBUTE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_PRECISION_ATTRIBUTE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'PRECISION_ATTRIBUTE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.PRECISION_ATTRIBUTE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAttributeDataType IS NOT NULL AND @AttributeDataType IS NOT NULL AND @OldAttributeDataType <> @AttributeDataType)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldAttributeDataType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@AttributeDataType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_ATTRIBUTE_DATA_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMinimumValue IS NOT NULL AND @MinimumValue IS NOT NULL AND @OldMinimumValue COLLATE Latin1_General_CS_AS <> @MinimumValue COLLATE Latin1_General_CS_AS)  
                        OR (@OldMinimumValue IS NULL AND @MinimumValue IS NOT NULL)
                        OR (@OldMinimumValue IS NOT NULL AND @MinimumValue IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMinimumValue AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MinimumValue AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_MINIMUM_VALUE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMaximumValue IS NOT NULL AND @MaximumValue IS NOT NULL AND @OldMaximumValue COLLATE Latin1_General_CS_AS <> @MaximumValue COLLATE Latin1_General_CS_AS)  
                        OR (@OldMaximumValue IS NULL AND @MaximumValue IS NOT NULL)
                        OR (@OldMaximumValue IS NOT NULL AND @MaximumValue IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMaximumValue AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MaximumValue AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_MAXIMUM_VALUE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDefaultValue IS NOT NULL AND @DefaultValue IS NOT NULL AND @OldDefaultValue COLLATE Latin1_General_CS_AS <> @DefaultValue COLLATE Latin1_General_CS_AS)  
                        OR (@OldDefaultValue IS NULL AND @DefaultValue IS NOT NULL)
                        OR (@OldDefaultValue IS NOT NULL AND @DefaultValue IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldDefaultValue AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@DefaultValue AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_DEFAULT_VALUE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAppearsOnDesktop IS NOT NULL AND @AppearsOnDesktop IS NOT NULL AND @OldAppearsOnDesktop <> @AppearsOnDesktop)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldAppearsOnDesktop WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @AppearsOnDesktop WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_APPEARS_ON_DESKTOP', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldSettableByAgent IS NOT NULL AND @SettableByAgent IS NOT NULL AND @OldSettableByAgent <> @SettableByAgent)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldSettableByAgent WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @SettableByAgent WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('PRECISION_ATTRIBUTE_SETTABLE_BY_AGENT', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_precisionattriPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_PRECISION_ATTRIBUTE_PKEY_MAP
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [APPEARS_ON_DESKTOP] = @AppearsOnDesktop, 
                                    [SETTABLE_BY_AGENT] = @SettableByAgent, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 77, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_precisionattriPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_precision_attribute_pkey]
	TO [portalapp_role]
GO
