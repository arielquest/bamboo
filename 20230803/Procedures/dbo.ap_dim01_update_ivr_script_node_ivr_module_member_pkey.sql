...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ivr_script_node_ivr_module_member_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_update_ivr_script_node_ivr_module_member_pkey');
        END;
        ...
        BEGIN    
            ...
            FROM dbo.TB_DIM_IVR_SCRIPT_NODE_IVR_MODULE_MEMBER_PKEY_MAP AS pm WITH(UPDLOCK)
            INNER JOIN dbo.TB_DIM_IVR_SCRIPT_NODE_IVR_MODULE_MEMBER AS m WITH(READUNCOMMITTED) ON pm.MEMBER_BIZ_URN = m.MEMBER_BIZ_URN AND m.LATEST = 1
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'IVR_SCRIPT_NODE_IVR_MODULE_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
            ...
            BEGIN        
                ...
                BEGIN 
                    RAISERROR('ap_dim01_update_ivr_script_node_ivr_module_member_pkey: Cannot perform provisionable update on deleted or deleting pkey map [%d]', 16, 1, @PkeyMapUrn) WITH LOG;
                END;
                ...
                BEGIN TRANSACTION PUPD_ivrscriptnodeiPKEYTran;       
                -- Update the pkey  record
                UPDATE dbo.TB_DIM_IVR_SCRIPT_NODE_IVR_MODULE_MEMBER_PKEY_MAP
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_update_ivr_script_node_ivr_module_member_pkey: Error updating member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_update_ivr_script_node_ivr_module_member_pkey: Concurrency conflict occured whilst attempting to update member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    COMMIT TRANSACTION PUPD_ivrscriptnodeiPKEYTran;
                END;
                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ivr_script_node_ivr_module_member_pkey]
	TO [portalapp_role]
GO
