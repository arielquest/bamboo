...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_service_enterprise_service_member] 
(
...
AS
    ...
    BEGIN 
        ...
        WHERE TABLE_NAME = 'TB_DIM_SERVICE_ENTERPRISE_SERVICE_MEMBER' AND MEMBER_TYPE_ID = '00000006-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_ADM_XML_DEFAULTS 
        WHERE TABLE_NAME = 'TB_DIM_SERVICE_ENTERPRISE_SERVICE_MEMBER' AND MEMBER_TYPE_ID = '00000006-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_SERVICE_ENTERPRISE_SERVICE_MEMBER member
        ...
        IF EXISTS(SELECT 1 FROM dbo.TB_DIM_ENTERPRISE_SERVICE WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            ...
            BEGIN 
                ...
                INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                SELECT 'SERVICE_ENTERPRISE_SERVICE_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
                ...
                BEGIN 
                    -- Update the tables
                    BEGIN TRANSACTION PUPD_serviceenterpriTran;
                        ...
                        BEGIN 
                            ...
                            BEGIN TRY;
                                ...
                                IF @DoType2 = 1
                                BEGIN 
                                    EXEC @ReturnCode = dbo.ap_dim01_do_type2_service_enterprise_service_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @ChangeStamp, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 1;
                                    IF @ReturnCode <> 0
                                    BEGIN 
                                        RAISERROR ('ap_dim01_do_type2_service_enterprise_service_member returned [%d] Error raised in TRY block.', 16, 1, @ReturnCode);
                                    END;
                                ...
                                ELSE BEGIN 
                                    ...
                                    BEGIN 
                                        COMMIT TRANSACTION PUPD_serviceenterpriTran;
                                    END;
                                    ELSE BEGIN 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_UPDATE_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 41, 94, DEFAULT, DEFAULT, 'S', 0, 'MEMBER_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_service_enterprise_service_member]
	TO [portalapp_role]
GO
