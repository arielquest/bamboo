...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_dialed_number_call_type_member] 
(
...
AS
    ...
    @OldEffectiveFrom DATETIME, @OldEffectiveTo DATETIME, @OldXmlData XML, @OldCustomXmlData AS XML,@OldRegionBizUrn                [INT],
    @OldItem                        [INT],
    ...
    BEGIN 
        ...
        WHERE TABLE_NAME = 'TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER' AND MEMBER_TYPE_ID = '00000021-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_ADM_XML_DEFAULTS 
        WHERE TABLE_NAME = 'TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER' AND MEMBER_TYPE_ID = '00000021-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        @OldEffectiveTo = member.[EFFECTIVE_TO], @OldCustomXmlData = member.[CUSTOM_XML_DATA], @OldRegionBizUrn = member.[REGION_BIZ_URN],
        @OldItem = member.[ITEM],
                @OldXmlData = member.[XML_DATA], @OldMemberUrn = member.[MEMBER_URN], @OldParentItemBusinessUrn = member.[PARENT_ITEM_BIZ_URN],
        @OldChildItemBusinessUrn = member.[CHILD_ITEM_BIZ_URN]
        FROM dbo.TB_DIM_DIALED_NUMBER_CALL_TYPE_MEMBER member
        ...
        IF EXISTS(SELECT 1 FROM dbo.TB_DIM_CALL_TYPE WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF EXISTS(SELECT 1 FROM dbo.TB_DIM_DIALED_NUMBER WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                ...
                INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                SELECT 'DIALED_NUMBER_CALL_TYPE_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
                ...
                IF (@OldRegionBizUrn IS NOT NULL AND @RegionBizUrn IS NOT NULL AND @OldRegionBizUrn <> @RegionBizUrn) 
                OR (@OldRegionBizUrn IS NULL AND @RegionBizUrn IS NOT NULL)
                OR (@OldRegionBizUrn IS NOT NULL AND @RegionBizUrn IS NULL)                 BEGIN 
      
                    SET @AuditTempOld = CAST(@OldRegionBizUrn AS NVARCHAR(250));
                    SET @AuditTempNew = CAST(@RegionBizUrn AS NVARCHAR(250));                     INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                    VALUES('DIALED_NUMBER_CALL_TYPE_MEMBER_REGION_BIZ_URN', @AuditTempNew, @AuditTempOld);
                    SET @FieldChanged = 1;
                    SET @ProvisionableFieldChanged= 1;
                END;
                IF (@OldItem IS NOT NULL AND @Item IS NOT NULL AND @OldItem <> @Item) 
                BEGIN 
      
                    SET @AuditTempOld = CAST(@OldItem AS NVARCHAR(250));
                    SET @AuditTempNew = CAST(@Item AS NVARCHAR(250));                     INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                    VALUES('DIALED_NUMBER_CALL_TYPE_MEMBER_ITEM', @AuditTempNew, @AuditTempOld);
                    SET @FieldChanged = 1;
                    SET @ProvisionableFieldChanged= 1;
                END;
                ...
                BEGIN 
                    -- Update the tables
                    BEGIN TRANSACTION PUPD_dialednumbercalTran;
                        ...
                        BEGIN 
                            ...
                            BEGIN TRY;
                                ...
                                IF @DoType2 = 1
                                BEGIN 
                                    EXEC @ReturnCode = dbo.ap_dim01_do_type2_dialed_number_call_type_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @ChangeStamp, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 1;
                                    IF @ReturnCode <> 0
                                    BEGIN 
                                        RAISERROR ('ap_dim01_do_type2_dialed_number_call_type_member returned [%d] Error raised in TRY block.', 16, 1, @ReturnCode);
                                    END;
...
,[ITEM] = @Item
                                ...
                                ELSE BEGIN 
                                    ...
                                    BEGIN 
                                        COMMIT TRANSACTION PUPD_dialednumbercalTran;
                                    END;
                                    ELSE BEGIN 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_UPDATE_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 12, 35, DEFAULT, DEFAULT, 'S', 0, 'MEMBER_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_dialed_number_call_type_member]
	TO [portalapp_role]
GO
