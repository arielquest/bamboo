...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_media_routing_domain_media_class_member] 
(
...
AS
    ...
    IF @XmlData IS NULL
    BEGIN 
        SELECT @XmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_MEDIA_ROUTING_DOMAIN_MEDIA_CLASS_MEMBER' AND MEMBER_TYPE_ID = '00000040-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    
    -- Get Custom Default XML
    IF @CustomXmlData IS NULL
    BEGIN 
        SELECT @CustomXmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_MEDIA_ROUTING_DOMAIN_MEDIA_CLASS_MEMBER' AND MEMBER_TYPE_ID = '00000040-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        IF @ParentItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_MEDIA_CLASS WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF @ChildItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_MEDIA_ROUTING_DOMAIN WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                ...
                BEGIN TRANSACTION PADD_mediaroutingdomTran;
                    ...
                    BEGIN 
                        ...
                        BEGIN TRY;
                            -- Add the base data
                            INSERT INTO dbo.TB_DIM_MEDIA_ROUTING_DOMAIN_MEDIA_CLASS_MEMBER 
                            (
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_INSERT_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 67, 68, DEFAULT, DEFAULT, 'S', 0, 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_media_routing_domain_media_class_member]
	TO [portalapp_role]
GO
