...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_dialed_number_pkey]
(    
    ...
    @Dialednumber            [NVARCHAR](32) = NULL, 
    @PermitApplicationRouting  [BIT] = 0, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldDialednumber                 [NVARCHAR](32), 
                @OldPermitApplicationRouting     [BIT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_dialed_number_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00002400-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldDialednumber = map.[DIALEDNUMBER],
                                @OldPermitApplicationRouting = map.[PERMIT_APPLICATION_ROUTING], 
                        ...
                        FROM dbo.TB_DIM_DIALED_NUMBER_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_DIALED_NUMBER AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_DIALED_NUMBER_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'DIALED_NUMBER_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.DIALED_NUMBER_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIALED_NUMBER_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIALED_NUMBER_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDialednumber IS NOT NULL AND @Dialednumber IS NOT NULL AND @OldDialednumber COLLATE Latin1_General_CS_AS <> @Dialednumber COLLATE Latin1_General_CS_AS)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldDialednumber AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Dialednumber AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIALED_NUMBER_DIALEDNUMBER', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldPermitApplicationRouting IS NOT NULL AND @PermitApplicationRouting IS NOT NULL AND @OldPermitApplicationRouting <> @PermitApplicationRouting)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldPermitApplicationRouting WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @PermitApplicationRouting WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIALED_NUMBER_PERMIT_APPLICATION_ROUTING', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_dialednPKEYTran;
                            BEGIN TRY;
                                ...
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [PERMIT_APPLICATION_ROUTING] = @PermitApplicationRouting, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 35, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_dialednPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_dialed_number_pkey]
	TO [portalapp_role]
GO
