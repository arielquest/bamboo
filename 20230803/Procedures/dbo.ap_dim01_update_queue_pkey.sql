...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_queue_pkey]
(    
    ...
    @State                   [SMALLINT] = 1, 
    @EmailRoutingType        [SMALLINT] = 0, 
    @ChatRoutingType         [SMALLINT] = 0, 
    @SocialRoutingType       [SMALLINT] = 0, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldState                        [SMALLINT], 
                @OldEmailRoutingType             [SMALLINT], 
                @OldChatRoutingType              [SMALLINT], 
                @OldSocialRoutingType            [SMALLINT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_queue_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '61a20821-ae86-4d50-b6a9-502d89453907';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldState = map.[STATE],
                                @OldEmailRoutingType = map.[EMAIL_ROUTING_TYPE],
                                @OldChatRoutingType = map.[CHAT_ROUTING_TYPE],
                                @OldSocialRoutingType = map.[SOCIAL_ROUTING_TYPE], 
                        ...
                        FROM dbo.TB_DIM_QUEUE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_QUEUE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_QUEUE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'QUEUE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.QUEUE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('QUEUE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('QUEUE_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldState IS NOT NULL AND @State IS NOT NULL AND @OldState <> @State)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldState AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@State AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('QUEUE_STATE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldEmailRoutingType IS NOT NULL AND @EmailRoutingType IS NOT NULL AND @OldEmailRoutingType <> @EmailRoutingType)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldEmailRoutingType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@EmailRoutingType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('QUEUE_EMAIL_ROUTING_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldChatRoutingType IS NOT NULL AND @ChatRoutingType IS NOT NULL AND @OldChatRoutingType <> @ChatRoutingType)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldChatRoutingType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ChatRoutingType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('QUEUE_CHAT_ROUTING_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldSocialRoutingType IS NOT NULL AND @SocialRoutingType IS NOT NULL AND @OldSocialRoutingType <> @SocialRoutingType)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldSocialRoutingType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@SocialRoutingType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('QUEUE_SOCIAL_ROUTING_TYPE', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRY;
                                ...
                                UPDATE dbo.TB_DIM_QUEUE_PKEY_MAP
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [STATE] = @State, 
                                    [EMAIL_ROUTING_TYPE] = @EmailRoutingType, 
                                    [CHAT_ROUTING_TYPE] = @ChatRoutingType, 
                                    [SOCIAL_ROUTING_TYPE] = @SocialRoutingType, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 113, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_queue_pkey]
	TO [portalapp_role]
GO
