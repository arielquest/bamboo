...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_code_group]
(
    ...
    @CodeConstraintTypeId            [UNIQUEIDENTIFIER] = '00002110-1000-0000-0000-000000000010', 
    @ConstraintDefinition            [NVARCHAR](max) = NULL, 
    @ConstraintMinValue              [NVARCHAR](100) = NULL, 
    @ConstraintMaxValue              [NVARCHAR](100) = NULL, 
...
AS
    ...
    @OldCodeConstraintTypeId         [UNIQUEIDENTIFIER],
    @OldConstraintDefinition         [NVARCHAR](max),
    @OldConstraintMinValue           [NVARCHAR](100),
    @OldConstraintMaxValue           [NVARCHAR](100),
    @OldDefaultValue                 [NVARCHAR](100),
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_CODE_GROUP' AND ITEM_TYPE_ID = '00010550-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_CODE_GROUP' AND ITEM_TYPE_ID = '00010550-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldCodeConstraintTypeId = item.[CODE_CONSTRAINT_TYPE_ID], 
        @OldConstraintDefinition = item.[CONSTRAINT_DEFINITION], 
        @OldConstraintMinValue = item.[CONSTRAINT_MIN_VALUE], 
        @OldConstraintMaxValue = item.[CONSTRAINT_MAX_VALUE], 
        @OldDefaultValue = item.[DEFAULT_VALUE], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_CODE_GROUP item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'CODE_GROUP_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_GROUP_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldCodeConstraintTypeId IS NOT NULL AND @CodeConstraintTypeId IS NOT NULL AND @OldCodeConstraintTypeId <> @CodeConstraintTypeId) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldCodeConstraintTypeId AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@CodeConstraintTypeId AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_GROUP_CODE_CONSTRAINT_TYPE_ID', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldConstraintDefinition IS NOT NULL AND @ConstraintDefinition IS NOT NULL AND @OldConstraintDefinition <> @ConstraintDefinition) 
        OR (@OldConstraintDefinition IS NULL AND @ConstraintDefinition IS NOT NULL)
        OR (@OldConstraintDefinition IS NOT NULL AND @ConstraintDefinition IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldConstraintDefinition AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ConstraintDefinition AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_GROUP_CONSTRAINT_DEFINITION', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldConstraintMinValue IS NOT NULL AND @ConstraintMinValue IS NOT NULL AND @OldConstraintMinValue <> @ConstraintMinValue) 
        OR (@OldConstraintMinValue IS NULL AND @ConstraintMinValue IS NOT NULL)
        OR (@OldConstraintMinValue IS NOT NULL AND @ConstraintMinValue IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldConstraintMinValue AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ConstraintMinValue AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_GROUP_CONSTRAINT_MIN_VALUE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldConstraintMaxValue IS NOT NULL AND @ConstraintMaxValue IS NOT NULL AND @OldConstraintMaxValue <> @ConstraintMaxValue) 
        OR (@OldConstraintMaxValue IS NULL AND @ConstraintMaxValue IS NOT NULL)
        OR (@OldConstraintMaxValue IS NOT NULL AND @ConstraintMaxValue IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldConstraintMaxValue AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ConstraintMaxValue AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_GROUP_CONSTRAINT_MAX_VALUE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldDefaultValue IS NOT NULL AND @DefaultValue IS NOT NULL AND @OldDefaultValue <> @DefaultValue) 
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_GROUP_DEFAULT_VALUE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_codeTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_code_group @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_code_group returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_code_group @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_code_group returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[CODE_CONSTRAINT_TYPE_ID] = @CodeConstraintTypeId        
,[CONSTRAINT_DEFINITION] = @ConstraintDefinition        
,[CONSTRAINT_MIN_VALUE] = @ConstraintMinValue        
,[CONSTRAINT_MAX_VALUE] = @ConstraintMaxValue        
,[DEFAULT_VALUE] = @DefaultValue        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 18, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_codeTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_code_group]
	TO [portalapp_role]
GO
