...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_query_rule_campaign_member_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_delete_query_rule_campaign_member_pkey');
        END;
        ...
        BEGIN   
            ...
            INNER JOIN dbo.TB_DIM_QUERY_RULE_CAMPAIGN_MEMBER AS m WITH(READUNCOMMITTED) ON pm.MEMBER_BIZ_URN = m.MEMBER_URN
            ...
            BEGIN TRANSACTION PDEL_queryrulecampaPKEYTran;
            -- Perform update
            IF @Purge = 1
            BEGIN 
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_delete_query_rule_campaign_member_pkey: Error purging member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_delete_query_rule_campaign_member_pkey: Concurrency conflict occured (or attempt to purge already purged entry) whilst attempting to purge member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
            ...
            ELSE BEGIN 
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_delete_query_rule_campaign_member_pkey: Error deleting member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_delete_query_rule_campaign_member_pkey: Concurrency conflict occured (or attempt to delete already deleted entry) whilst attempting to delete member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
            ...
            BEGIN 
                COMMIT TRANSACTION PDEL_queryrulecampaPKEYTran;
            END;
            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_query_rule_campaign_member_pkey]
	TO [portalapp_role]
GO
