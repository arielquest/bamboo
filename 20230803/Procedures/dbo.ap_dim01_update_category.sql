...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_category]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_CATEGORY' AND ITEM_TYPE_ID = '00001700-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_CATEGORY' AND ITEM_TYPE_ID = '00001700-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldRootCategoryBizUrn = item.[ROOT_CATEGORY_BIZ_URN], 
        @OldPath = item.[PATH], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_CATEGORY item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'CATEGORY_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldRootCategoryBizUrn IS NOT NULL AND @RootCategoryBizUrn IS NOT NULL AND @OldRootCategoryBizUrn <> @RootCategoryBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldRootCategoryBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@RootCategoryBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CATEGORY_ROOT_CATEGORY_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldPath IS NOT NULL AND @Path IS NOT NULL AND @OldPath <> @Path) 
        OR (@OldPath IS NULL AND @Path IS NOT NULL)
        OR (@OldPath IS NOT NULL AND @Path IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldPath AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@Path AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CATEGORY_PATH', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_catTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_category @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_category returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_category @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_category returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[ROOT_CATEGORY_BIZ_URN] = @RootCategoryBizUrn        
,[PATH] = @Path        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 14, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_catTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_category]
	TO [portalapp_role]
GO
