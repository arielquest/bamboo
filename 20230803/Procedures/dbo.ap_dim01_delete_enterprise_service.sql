...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_enterprise_service] (@SessionId UNIQUEIDENTIFIER, @ItemUrn INT, @ChangeStamp INT OUTPUT)
AS BEGIN 
    SET NOCOUNT ON;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_ENTERPRISE_SERVICE WITH (UPDLOCK)
        ...
        BEGIN 
            ...
            BEGIN
                BEGIN TRANSACTION PDEL_enterpriseserviTran;
                    BEGIN TRY;
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_DELETE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 41, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_DELETED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PDEL_enterpriseserviTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_enterprise_service]
	TO [portalapp_role]
GO
