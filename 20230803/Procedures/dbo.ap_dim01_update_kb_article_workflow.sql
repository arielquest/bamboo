...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_kb_article_workflow]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_KB_ARTICLE_WORKFLOW' AND ITEM_TYPE_ID = '140AE2B9-C3BA-42BB-8508-0240E45B95C1' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_KB_ARTICLE_WORKFLOW' AND ITEM_TYPE_ID = '140AE2B9-C3BA-42BB-8508-0240E45B95C1' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldIsDefault = item.[IS_DEFAULT], 
        @OldIsSuggestion = item.[IS_SUGGESTION], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_KB_ARTICLE_WORKFLOW item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'KB_ARTICLE_WORKFLOW_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldIsDefault IS NOT NULL AND @IsDefault IS NOT NULL AND @OldIsDefault <> @IsDefault) 
        BEGIN      
            SET @AuditTempOld = CASE @OldIsDefault WHEN 1 THEN 'Yes' ELSE 'No' END;
            SET @AuditTempNew = CASE @IsDefault WHEN 1 THEN 'Yes' ELSE 'No' END;
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('KB_ARTICLE_WORKFLOW_IS_DEFAULT', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIsSuggestion IS NOT NULL AND @IsSuggestion IS NOT NULL AND @OldIsSuggestion <> @IsSuggestion) 
        BEGIN      
            SET @AuditTempOld = CASE @OldIsSuggestion WHEN 1 THEN 'Yes' ELSE 'No' END;
            SET @AuditTempNew = CASE @IsSuggestion WHEN 1 THEN 'Yes' ELSE 'No' END;
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('KB_ARTICLE_WORKFLOW_IS_SUGGESTION', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_kbarticleworkflTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_kb_article_workflow @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_kb_article_workflow returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_kb_article_workflow @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_kb_article_workflow returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
                        ...
                        UPDATE dbo.TB_DIM_KB_ARTICLE_WORKFLOW
...
,[IS_SUGGESTION] = @IsSuggestion        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 158, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_kbarticleworkflTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_kb_article_workflow]
	TO [portalapp_role]
GO
