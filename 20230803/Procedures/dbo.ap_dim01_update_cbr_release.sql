...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_cbr_release]
(
    ...
    @OriginalCBRReleaseBizItemUrn    [INT] = NULL, 
...
AS
    ...
    @OldOriginalCBRReleaseBizItemUrn  [INT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_CBR_RELEASE' AND ITEM_TYPE_ID = '8AF98BA5-936D-40A0-B7AA-F98163F18C9C' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_CBR_RELEASE' AND ITEM_TYPE_ID = '8AF98BA5-936D-40A0-B7AA-F98163F18C9C' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldOriginalCBRReleaseBizItemUrn = item.[ORIGINAL_CBR_RELEASE_BIZ_ITEM_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_CBR_RELEASE item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'CBR_RELEASE_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldOriginalCBRReleaseBizItemUrn IS NOT NULL AND @OriginalCBRReleaseBizItemUrn IS NOT NULL AND @OldOriginalCBRReleaseBizItemUrn <> @OriginalCBRReleaseBizItemUrn) 
        OR (@OldOriginalCBRReleaseBizItemUrn IS NULL AND @OriginalCBRReleaseBizItemUrn IS NOT NULL)
        OR (@OldOriginalCBRReleaseBizItemUrn IS NOT NULL AND @OriginalCBRReleaseBizItemUrn IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldOriginalCBRReleaseBizItemUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@OriginalCBRReleaseBizItemUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CBR_RELEASE_ORIGINAL_CBR_RELEASE_BIZ_ITEM_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_cbrreTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_cbr_release @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_cbr_release returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_cbr_release @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_cbr_release returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[ORIGINAL_CBR_RELEASE_BIZ_ITEM_URN] = @OriginalCBRReleaseBizItemUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 127, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_cbrreTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_cbr_release]
	TO [portalapp_role]
GO
