...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_skillgroup_pkey]
(    
    ...
    @IPTA                    [CHAR](1) = 'N', 
    @ServiceLevelThreshold   [INT] = -1, 
    @ServiceLevelType        [SMALLINT] = 0, 
    @DefaultEntry            [SMALLINT] = 0, 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_skillgroup_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00007600-1000-0000-0000-000000000010';
                        ...
                        FROM dbo.TB_DIM_SKILLGROUP_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_SKILLGROUP AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_SKILLGROUP_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'SKILLGROUP_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.SKILLGROUP_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldPeripheralNumber IS NOT NULL AND @PeripheralNumber IS NOT NULL AND @OldPeripheralNumber <> @PeripheralNumber)  
                        OR (@OldPeripheralNumber IS NULL AND @PeripheralNumber IS NOT NULL)
                        OR (@OldPeripheralNumber IS NOT NULL AND @PeripheralNumber IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldPeripheralNumber AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@PeripheralNumber AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_PERIPHERAL_NUMBER', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_PERIPHERAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_AVAILABLE_HOLDOFF_DELAY', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldPriority IS NOT NULL AND @Priority IS NOT NULL AND @OldPriority <> @Priority)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldPriority AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Priority AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_PRIORITY', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldExtension IS NOT NULL AND @Extension IS NOT NULL AND @OldExtension COLLATE Latin1_General_CS_AS <> @Extension COLLATE Latin1_General_CS_AS)  
                        OR (@OldExtension IS NULL AND @Extension IS NOT NULL)
                        OR (@OldExtension IS NOT NULL AND @Extension IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldExtension AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Extension AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_EXTENSION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldIPTA IS NOT NULL AND @IPTA IS NOT NULL AND @OldIPTA <> @IPTA)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldIPTA AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@IPTA AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_IPTA', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_SERVICE_LEVEL_THRESHOLD', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_SERVICE_LEVEL_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDefaultEntry IS NOT NULL AND @DefaultEntry IS NOT NULL AND @OldDefaultEntry <> @DefaultEntry)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldDefaultEntry AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@DefaultEntry AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_DEFAULT_ENTRY', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_SUB_SKILLGROUP_MASK', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_USER_DELETABLE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldIsPRSkill IS NOT NULL AND @IsPRSkill IS NOT NULL AND @OldIsPRSkill <> @IsPRSkill)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldIsPRSkill WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @IsPRSkill WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SKILLGROUP_IS_PR_SKILL', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_skillPKEYTran;
                            BEGIN TRY;
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 95, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_skillPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_skillgroup_pkey]
	TO [portalapp_role]
GO
