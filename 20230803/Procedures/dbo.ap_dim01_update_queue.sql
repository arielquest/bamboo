...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_queue]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_QUEUE' AND ITEM_TYPE_ID = '61A20821-AE86-4D50-B6A9-502D89453907' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_QUEUE' AND ITEM_TYPE_ID = '61A20821-AE86-4D50-B6A9-502D89453907' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldDialedNumberBizUrn = item.[DIALED_NUMBER_BIZ_URN], 
        @OldSkillGroupBizUrn = item.[SKILLGROUP_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_QUEUE item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'QUEUE_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldDialedNumberBizUrn IS NOT NULL AND @DialedNumberBizUrn IS NOT NULL AND @OldDialedNumberBizUrn <> @DialedNumberBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldDialedNumberBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@DialedNumberBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('QUEUE_DIALED_NUMBER_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('QUEUE_SKILLGROUP_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_queue @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_queue returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_queue @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_queue returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[DIALED_NUMBER_BIZ_URN] = @DialedNumberBizUrn        
,[SKILLGROUP_BIZ_URN] = @SkillGroupBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 113, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
