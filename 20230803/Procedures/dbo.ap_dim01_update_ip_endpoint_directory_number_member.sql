...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ip_endpoint_directory_number_member] 
(
...
AS
    ...
    @OldEffectiveFrom DATETIME, @OldEffectiveTo DATETIME, @OldXmlData XML, @OldCustomXmlData AS XML,@OldResourceTemplateId          [UNIQUEIDENTIFIER],
    ...
    BEGIN 
        ...
        WHERE TABLE_NAME = 'TB_DIM_IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER' AND MEMBER_TYPE_ID = '00000032-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_ADM_XML_DEFAULTS 
        WHERE TABLE_NAME = 'TB_DIM_IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER' AND MEMBER_TYPE_ID = '00000032-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        @OldEffectiveTo = member.[EFFECTIVE_TO], @OldCustomXmlData = member.[CUSTOM_XML_DATA], @OldResourceTemplateId = member.[RESOURCE_TEMPLATE_ID],
        ...
        FROM dbo.TB_DIM_IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER member
        ...
        IF EXISTS(SELECT 1 FROM dbo.TB_DIM_DIRECTORY_NUMBER WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF EXISTS(SELECT 1 FROM dbo.TB_DIM_IP_ENDPOINT WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                ...
                INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                SELECT 'IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
                ...
                INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                SELECT 
                'custom.IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                ...
                OR (@OldResourceTemplateId IS NOT NULL AND @ResourceTemplateId IS NULL)                 BEGIN 
      
                    SET @AuditTempOld = CAST(@OldResourceTemplateId AS NVARCHAR(250));
                    SET @AuditTempNew = CAST(@ResourceTemplateId AS NVARCHAR(250));                     INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                    VALUES('IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_RESOURCE_TEMPLATE_ID', @AuditTempNew, @AuditTempOld);
                    SET @FieldChanged = 1;
                    SET @ProvisionableFieldChanged= 1;
                END;
                ...
                BEGIN 
                    -- Update the tables
                    BEGIN TRANSACTION PUPD_ipendpointdirecTran;
                        ...
                        BEGIN 
                            ...
                            BEGIN TRY;
                                ...
                                IF @DoType2 = 1
                                BEGIN 
                                    EXEC @ReturnCode = dbo.ap_dim01_do_type2_ip_endpoint_directory_number_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @ChangeStamp, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 1;
                                    IF @ReturnCode <> 0
                                    BEGIN 
                                        RAISERROR ('ap_dim01_do_type2_ip_endpoint_directory_number_member returned [%d] Error raised in TRY block.', 16, 1, @ReturnCode);
                                    END;
...
,[RESOURCE_TEMPLATE_ID] = @ResourceTemplateId
                                ...
                                ELSE BEGIN 
                                    ...
                                    BEGIN 
                                        COMMIT TRANSACTION PUPD_ipendpointdirecTran;
                                    END;
                                    ELSE BEGIN 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_UPDATE_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 38, 53, DEFAULT, DEFAULT, 'S', 0, 'MEMBER_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ip_endpoint_directory_number_member]
	TO [portalapp_role]
GO
