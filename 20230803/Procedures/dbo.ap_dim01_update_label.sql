...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_label]
(
    ...
    @NetworkVruBizUrn                [INT] = -1, 
    @SelfCareEnabled                 [BIT] = 0, 
...
AS
    ...
    @OldNetworkVruBizUrn             [INT],
    @OldSelfCareEnabled              [BIT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_LABEL' AND ITEM_TYPE_ID = '00004700-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_LABEL' AND ITEM_TYPE_ID = '00004700-1000-0000-0000-000000000010' AND CUSTOM = 1
                 ORDER BY [XML_DEFAULTS_URN]) c;

    DECLARE @FIRST_SURROGATE BIT;

    SELECT  @FIRST_SURROGATE = CASE WHEN @NetworkVruBizUrn IN ( -1, -2 ) THEN 1
                                    WHEN EXISTS ( SELECT    1
                                                  FROM      [dbo].[TB_DIM_NETWORK_VRU]
                                                  WHERE     [ITEM_URN] = [ITEM_BIZ_URN]
                                                            AND [ITEM_URN] = @NetworkVruBizUrn ) THEN 1
                                    ELSE 0
                               END;
    ...
    -- Can only link @NetworkVruBizUrn to the first surrogate
    AND @FIRST_SURROGATE = 1 
    BEGIN 
        ...
        @OldNetworkVruBizUrn = item.[NETWORK_VRU_BIZ_URN], 
        @OldSelfCareEnabled = item.[SELF_CARE_ENABLED], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_LABEL item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'LABEL_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldNetworkVruBizUrn IS NOT NULL AND @NetworkVruBizUrn IS NOT NULL AND @OldNetworkVruBizUrn <> @NetworkVruBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldNetworkVruBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@NetworkVruBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('LABEL_NETWORK_VRU_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldSelfCareEnabled IS NOT NULL AND @SelfCareEnabled IS NOT NULL AND @OldSelfCareEnabled <> @SelfCareEnabled) 
        BEGIN      
            SET @AuditTempOld = CASE @OldSelfCareEnabled WHEN 1 THEN 'Yes' ELSE 'No' END;
            SET @AuditTempNew = CASE @SelfCareEnabled WHEN 1 THEN 'Yes' ELSE 'No' END;
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('LABEL_SELF_CARE_ENABLED', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_label @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_label returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_label @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_label returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[NETWORK_VRU_BIZ_URN] = @NetworkVruBizUrn        
,[SELF_CARE_ENABLED] = @SelfCareEnabled        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 63, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_Tran;
                        END;
                        ELSE BEGIN 
        ...
        END;
    END; 
    ELSE IF @FIRST_SURROGATE <> 1
    BEGIN
        SET @ErrorMessage = OBJECT_NAME(@@PROCID)+': '+'The NETWORK_VRU_BIZ_URN must be the first surrogate from TB_DIM_NETWORK_VRU';
    END; 
    ELSE BEGIN 
