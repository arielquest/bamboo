...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_media_routing_domain_pkey]
(    
    ...
    @MaxCallsInQueuePerCalltype  [INT] = NULL, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldInterruptible                [CHAR](1), 
                @OldMaxCallsInQueue              [INT], 
                @OldMaxCallsInQueuePerCalltype   [INT], 
                @OldMaxTaskDuration              [INT], 
                @OldMaxTimeInqueue               [INT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_media_routing_domain_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00009600-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldInterruptible = map.[INTERRUPTIBLE],
                                @OldMaxCallsInQueue = map.[MAX_CALLS_IN_QUEUE],
                                @OldMaxCallsInQueuePerCalltype = map.[MAX_CALLS_IN_QUEUE_PER_CALLTYPE],
                                @OldMaxTaskDuration = map.[MAX_TASK_DURATION],
                                @OldMaxTimeInqueue = map.[MAX_TIME_INQUEUE],
                        ...
                        FROM dbo.TB_DIM_MEDIA_ROUTING_DOMAIN_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_MEDIA_ROUTING_DOMAIN AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_MEDIA_ROUTING_DOMAIN_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'MEDIA_ROUTING_DOMAIN_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.MEDIA_ROUTING_DOMAIN_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldInterruptible IS NOT NULL AND @Interruptible IS NOT NULL AND @OldInterruptible <> @Interruptible)  
                        OR (@OldInterruptible IS NULL AND @Interruptible IS NOT NULL)
                        OR (@OldInterruptible IS NOT NULL AND @Interruptible IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldInterruptible AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Interruptible AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_INTERRUPTIBLE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMaxCallsInQueue IS NOT NULL AND @MaxCallsInQueue IS NOT NULL AND @OldMaxCallsInQueue <> @MaxCallsInQueue)  
                        OR (@OldMaxCallsInQueue IS NULL AND @MaxCallsInQueue IS NOT NULL)
                        OR (@OldMaxCallsInQueue IS NOT NULL AND @MaxCallsInQueue IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMaxCallsInQueue AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MaxCallsInQueue AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_MAX_CALLS_IN_QUEUE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMaxCallsInQueuePerCalltype IS NOT NULL AND @MaxCallsInQueuePerCalltype IS NOT NULL AND @OldMaxCallsInQueuePerCalltype <> @MaxCallsInQueuePerCalltype)  
                        OR (@OldMaxCallsInQueuePerCalltype IS NULL AND @MaxCallsInQueuePerCalltype IS NOT NULL)
                        OR (@OldMaxCallsInQueuePerCalltype IS NOT NULL AND @MaxCallsInQueuePerCalltype IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMaxCallsInQueuePerCalltype AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MaxCallsInQueuePerCalltype AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_MAX_CALLS_IN_QUEUE_PER_CALLTYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMaxTaskDuration IS NOT NULL AND @MaxTaskDuration IS NOT NULL AND @OldMaxTaskDuration <> @MaxTaskDuration)  
                        OR (@OldMaxTaskDuration IS NULL AND @MaxTaskDuration IS NOT NULL)
                        OR (@OldMaxTaskDuration IS NOT NULL AND @MaxTaskDuration IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMaxTaskDuration AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MaxTaskDuration AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_MAX_TASK_DURATION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldMaxTimeInqueue IS NOT NULL AND @MaxTimeInqueue IS NOT NULL AND @OldMaxTimeInqueue <> @MaxTimeInqueue)  
                        OR (@OldMaxTimeInqueue IS NULL AND @MaxTimeInqueue IS NOT NULL)
                        OR (@OldMaxTimeInqueue IS NOT NULL AND @MaxTimeInqueue IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldMaxTimeInqueue AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@MaxTimeInqueue AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_MAX_TIME_INQUEUE', @AuditTempNew, @AuditTempOld);
                        ...
                        OR (@OldServiceLevelThreshold IS NULL AND @ServiceLevelThreshold IS NOT NULL)
                        OR (@OldServiceLevelThreshold IS NOT NULL AND @ServiceLevelThreshold IS NULL) 
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIA_ROUTING_DOMAIN_SERVICE_LEVEL_THRESHOLD', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_mediaroutingdoPKEYTran;
                            BEGIN TRY;
                                ...
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [MAX_CALLS_IN_QUEUE] = @MaxCallsInQueue, 
                                    [MAX_CALLS_IN_QUEUE_PER_CALLTYPE] = @MaxCallsInQueuePerCalltype, 
                                    [MAX_TASK_DURATION] = @MaxTaskDuration, 
                                    [MAX_TIME_INQUEUE] = @MaxTimeInqueue, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 68, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_mediaroutingdoPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_media_routing_domain_pkey]
	TO [portalapp_role]
GO
