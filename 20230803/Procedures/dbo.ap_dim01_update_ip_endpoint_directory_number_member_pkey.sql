...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ip_endpoint_directory_number_member_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_update_ip_endpoint_directory_number_member_pkey');
        END;
        ...
        BEGIN    
            ...
            SET @MemberTypeId = '00000032-1000-0000-0000-0000000000c0';
            SELECT @MemberTypeIsProvisionable = ISNULL((SELECT TOP 1 1 
                                                        FROM dbo.TE_DIM_ITEM_TYPE 
                                                        WHERE ITEM_TYPE_ID IN ('00011100-1000-0000-0000-000000000010', '00004500-1000-0000-0000-000000000010')
            ...
            FROM dbo.TB_DIM_IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_PKEY_MAP AS pm WITH(UPDLOCK)
            INNER JOIN dbo.TB_DIM_IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER AS m WITH(READUNCOMMITTED) ON pm.MEMBER_BIZ_URN = m.MEMBER_BIZ_URN AND m.LATEST = 1
            ...
            BEGIN 
                ...
                WHERE TABLE_NAME = 'TB_DIM_IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_PKEY_MAP' 
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
            ...
            BEGIN  
                ...
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_LINE_INDEX', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            ...
            BEGIN        
                ...
                BEGIN 
                    RAISERROR('ap_dim01_update_ip_endpoint_directory_number_member_pkey: Cannot perform provisionable update on deleted or deleting pkey map [%d]', 16, 1, @PkeyMapUrn) WITH LOG;
                END;
                ...
                BEGIN TRANSACTION PUPD_ipendpointdirePKEYTran;       
                -- Update the pkey  record
                UPDATE dbo.TB_DIM_IP_ENDPOINT_DIRECTORY_NUMBER_MEMBER_PKEY_MAP
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_update_ip_endpoint_directory_number_member_pkey: Error updating member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_update_ip_endpoint_directory_number_member_pkey: Concurrency conflict occured whilst attempting to update member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 38, 53, @ClusterResourceId, NULL, 'S', 0, 
                    'MEMBER_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
            
                IF @InitialTransactionCount = 0 
                BEGIN 
                    COMMIT TRANSACTION PUPD_ipendpointdirePKEYTran;
                END;
                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ip_endpoint_directory_number_member_pkey]
	TO [portalapp_role]
GO
