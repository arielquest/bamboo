...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_hierarchy_node]
(
...
AS
    ...
    @OldHierarchyBusinessUrn         [INT],
    @OldEditable                     [BIT],
    @OldNodeId                       [UNIQUEIDENTIFIER],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_HIERARCHY_NODE' AND ITEM_TYPE_ID = '00039552-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_HIERARCHY_NODE' AND ITEM_TYPE_ID = '00039552-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldHierarchyBusinessUrn = item.[HIERARCHY_BUSINESS_URN], 
        @OldEditable = item.[EDITABLE], 
        @OldNodeId = item.[NODE_ID], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_HIERARCHY_NODE item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'HIERARCHY_NODE_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldHierarchyBusinessUrn IS NOT NULL AND @HierarchyBusinessUrn IS NOT NULL AND @OldHierarchyBusinessUrn <> @HierarchyBusinessUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldHierarchyBusinessUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@HierarchyBusinessUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('HIERARCHY_NODE_HIERARCHY_BUSINESS_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldEditable IS NOT NULL AND @Editable IS NOT NULL AND @OldEditable <> @Editable) 
        BEGIN      
            SET @AuditTempOld = CASE @OldEditable WHEN 1 THEN 'Yes' ELSE 'No' END;
            SET @AuditTempNew = CASE @Editable WHEN 1 THEN 'Yes' ELSE 'No' END;
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('HIERARCHY_NODE_EDITABLE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldNodeId IS NOT NULL AND @NodeId IS NOT NULL AND @OldNodeId <> @NodeId) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldNodeId AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@NodeId AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('HIERARCHY_NODE_NODE_ID', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_hierarchTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_hierarchy_node @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_hierarchy_node returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_hierarchy_node @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_hierarchy_node returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[HIERARCHY_BUSINESS_URN] = @HierarchyBusinessUrn        
,[EDITABLE] = @Editable        
,[NODE_ID] = @NodeId        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 50, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_hierarchTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_hierarchy_node]
	TO [portalapp_role]
GO
