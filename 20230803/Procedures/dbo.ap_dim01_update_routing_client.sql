...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_routing_client]
(
    ...
    @LogicalInterfaceControllerBizUrn  [INT] = -1, 
...
AS
    ...
    @OldLogicalInterfaceControllerBizUrn  [INT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_ROUTING_CLIENT' AND ITEM_TYPE_ID = '00007300-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_ROUTING_CLIENT' AND ITEM_TYPE_ID = '00007300-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldLogicalInterfaceControllerBizUrn = item.[LOGICAL_INTERFACE_CONTROLLER_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_ROUTING_CLIENT item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'ROUTING_CLIENT_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldLogicalInterfaceControllerBizUrn IS NOT NULL AND @LogicalInterfaceControllerBizUrn IS NOT NULL AND @OldLogicalInterfaceControllerBizUrn <> @LogicalInterfaceControllerBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldLogicalInterfaceControllerBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@LogicalInterfaceControllerBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('ROUTING_CLIENT_LOGICAL_INTERFACE_CONTROLLER_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_routingcTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_routing_client @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_routing_client returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_routing_client @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_routing_client returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[LOGICAL_INTERFACE_CONTROLLER_BIZ_URN] = @LogicalInterfaceControllerBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 88, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_routingcTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_routing_client]
	TO [portalapp_role]
GO
