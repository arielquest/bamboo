...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_vxmlapplication_server_pkey]
(    
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldIPAddress                    [NVARCHAR](100), 
                @OldHostname                     [NVARCHAR](100), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_vxmlapplication_server_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = 'e8df41b0-854c-421d-9c6d-1eaa8fdc9dce';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldIPAddress = map.[IP_ADDRESS],
                                @OldHostname = map.[HOSTNAME], 
                        ...
                        FROM dbo.TB_DIM_VXMLAPPLICATION_SERVER_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_VXMLAPPLICATION_SERVER AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_VXMLAPPLICATION_SERVER_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'VXMLAPPLICATION_SERVER_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.VXMLAPPLICATION_SERVER_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        IF (@OldIPAddress IS NOT NULL AND @IPAddress IS NOT NULL AND @OldIPAddress COLLATE Latin1_General_CS_AS <> @IPAddress COLLATE Latin1_General_CS_AS)  
                        OR (@OldIPAddress IS NULL AND @IPAddress IS NOT NULL)
                        OR (@OldIPAddress IS NOT NULL AND @IPAddress IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldIPAddress AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@IPAddress AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('VXMLAPPLICATION_SERVER_IP_ADDRESS', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldHostname IS NOT NULL AND @Hostname IS NOT NULL AND @OldHostname COLLATE Latin1_General_CS_AS <> @Hostname COLLATE Latin1_General_CS_AS)  
                        OR (@OldHostname IS NULL AND @Hostname IS NOT NULL)
                        OR (@OldHostname IS NOT NULL AND @Hostname IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldHostname AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Hostname AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('VXMLAPPLICATION_SERVER_HOSTNAME', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_vxmlapplicatioPKEYTran;
                            BEGIN TRY;
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 110, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_vxmlapplicatioPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_vxmlapplication_server_pkey]
	TO [portalapp_role]
GO
