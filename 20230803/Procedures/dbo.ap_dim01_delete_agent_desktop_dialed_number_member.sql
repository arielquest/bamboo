...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_agent_desktop_dialed_number_member]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_delete_agent_desktop_dialed_number_member');
        END;
        ...
        BEGIN    
            ...
            SELECT  @ParentItemBusinessUrn = PARENT_ITEM_BIZ_URN,
                    ...
                    @Deleted = DELETED
            FROM dbo.TB_DIM_AGENT_DESKTOP_DIALED_NUMBER_MEMBER WITH (UPDLOCK)
            WHERE MEMBER_URN = @MemberUrn;
    
            -- Return if already deleted
            IF @Deleted <> 1
            BEGIN        
                BEGIN TRANSACTION PDEL_agentdesktopdiaTran;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_delete_agent_desktop_dialed_number_member: Error deleting member [%d], Error [%d] ', 16, 1, @MemberUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_delete_agent_desktop_dialed_number_member: Concurrency conflict occured (or attempt to delete already deleted member) whilst deleting member [%d], Error [%d]', 16, 1, @MemberUrn, @ReturnCode) WITH LOG;
                END;
                ...
                EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_DELETE_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 35, 2,  NULL, NULL, 'S', 0, 
                'MEMBER_DELETED', NULL, NULL, NULL, @AuditUrn OUTPUT;
        
                IF @InitialTransactionCount = 0 
                BEGIN 
                    COMMIT TRANSACTION PDEL_agentdesktopdiaTran;
                END;
                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_agent_desktop_dialed_number_member]
	TO [portalapp_role]
GO
