...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_network_vru_script_pkey]
(    
    ...
    @VruScriptName           [NVARCHAR](40) = NULL, 
    @Interruptible           [BIT] = NULL, 
    @Overridable             [BIT] = NULL, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldVruScriptName                [NVARCHAR](40), 
                @OldInterruptible                [BIT], 
                @OldOverridable                  [BIT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_network_vru_script_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = 'aaf3357b-5222-416d-887f-2e6678c73910';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldVruScriptName = map.[VRU_SCRIPT_NAME],
                                @OldInterruptible = map.[INTERRUPTIBLE],
                                @OldOverridable = map.[OVERRIDABLE],
                                @OldConfigParam = map.[CONFIG_PARAM],
                                @OldTimeout = map.[TIMEOUT], 
                        ...
                        FROM dbo.TB_DIM_NETWORK_VRU_SCRIPT_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_NETWORK_VRU_SCRIPT AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_NETWORK_VRU_SCRIPT_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'NETWORK_VRU_SCRIPT_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.NETWORK_VRU_SCRIPT_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('NETWORK_VRU_SCRIPT_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('NETWORK_VRU_SCRIPT_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldVruScriptName IS NOT NULL AND @VruScriptName IS NOT NULL AND @OldVruScriptName COLLATE Latin1_General_CS_AS <> @VruScriptName COLLATE Latin1_General_CS_AS)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldVruScriptName AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@VruScriptName AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('NETWORK_VRU_SCRIPT_VRU_SCRIPT_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            SET @AuditTempOld = CASE @OldInterruptible WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @Interruptible WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('NETWORK_VRU_SCRIPT_INTERRUPTIBLE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldOverridable IS NOT NULL AND @Overridable IS NOT NULL AND @OldOverridable <> @Overridable)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldOverridable WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @Overridable WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('NETWORK_VRU_SCRIPT_OVERRIDABLE', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('NETWORK_VRU_SCRIPT_CONFIG_PARAM', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldTimeout IS NOT NULL AND @Timeout IS NOT NULL AND @OldTimeout <> @Timeout)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldTimeout AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Timeout AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('NETWORK_VRU_SCRIPT_TIMEOUT', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_networkvruscriPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_NETWORK_VRU_SCRIPT_PKEY_MAP
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 71, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_networkvruscriPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_network_vru_script_pkey]
	TO [portalapp_role]
GO
