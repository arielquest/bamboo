...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_entry_point_pkey]
(    
    ...
    @StartPageUrl            [NVARCHAR](255) = NULL, 
    @FinishPageUrl           [NVARCHAR](255) = NULL, 
    @AgentAvailability       [SMALLINT] = NULL, 
    @ConsoleMode             [SMALLINT] = NULL, 
    @IsActive                [BIT] = 1, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldInternalName                 [NVARCHAR](255), 
                @OldDescription                  [NVARCHAR](2000), 
                @OldStartPageUrl                 [NVARCHAR](255), 
                @OldFinishPageUrl                [NVARCHAR](255), 
                @OldAgentAvailability            [SMALLINT], 
                @OldConsoleMode                  [SMALLINT], 
                @OldIsActive                     [BIT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_entry_point_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '9d660b17-9af3-43ec-a6b8-ec8e0d74c2a0';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldStartPageUrl = map.[START_PAGE_URL],
                                @OldFinishPageUrl = map.[FINISH_PAGE_URL],
                                @OldAgentAvailability = map.[AGENT_AVAILABILITY],
                                @OldConsoleMode = map.[CONSOLE_MODE],
                                @OldIsActive = map.[IS_ACTIVE], 
                        ...
                        FROM dbo.TB_DIM_ENTRY_POINT_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_ENTRY_POINT AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_ENTRY_POINT_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'ENTRY_POINT_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.ENTRY_POINT_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ENTRY_POINT_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ENTRY_POINT_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldStartPageUrl IS NOT NULL AND @StartPageUrl IS NOT NULL AND @OldStartPageUrl COLLATE Latin1_General_CS_AS <> @StartPageUrl COLLATE Latin1_General_CS_AS)  
                        OR (@OldStartPageUrl IS NULL AND @StartPageUrl IS NOT NULL)
                        OR (@OldStartPageUrl IS NOT NULL AND @StartPageUrl IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldStartPageUrl AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@StartPageUrl AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ENTRY_POINT_START_PAGE_URL', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldFinishPageUrl IS NOT NULL AND @FinishPageUrl IS NOT NULL AND @OldFinishPageUrl COLLATE Latin1_General_CS_AS <> @FinishPageUrl COLLATE Latin1_General_CS_AS)  
                        OR (@OldFinishPageUrl IS NULL AND @FinishPageUrl IS NOT NULL)
                        OR (@OldFinishPageUrl IS NOT NULL AND @FinishPageUrl IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldFinishPageUrl AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@FinishPageUrl AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ENTRY_POINT_FINISH_PAGE_URL', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldAgentAvailability IS NOT NULL AND @AgentAvailability IS NOT NULL AND @OldAgentAvailability <> @AgentAvailability)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldAgentAvailability AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@AgentAvailability AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ENTRY_POINT_AGENT_AVAILABILITY', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldConsoleMode IS NOT NULL AND @ConsoleMode IS NOT NULL AND @OldConsoleMode <> @ConsoleMode)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldConsoleMode AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ConsoleMode AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ENTRY_POINT_CONSOLE_MODE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldIsActive IS NOT NULL AND @IsActive IS NOT NULL AND @OldIsActive <> @IsActive)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldIsActive WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @IsActive WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ENTRY_POINT_IS_ACTIVE', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_entryPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_ENTRY_POINT_PKEY_MAP
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 115, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_entryPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_entry_point_pkey]
	TO [portalapp_role]
GO
