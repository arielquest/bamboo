...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_precision_queue_step_precision_queue_member] 
(
...
AS
    ...
    IF @XmlData IS NULL
    BEGIN 
        SELECT @XmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_STEP_PRECISION_QUEUE_MEMBER' AND MEMBER_TYPE_ID = '00039592-1000-0000-0000-000000000010' AND CUSTOM = 0;
    END;
    
    -- Get Custom Default XML
    IF @CustomXmlData IS NULL
    BEGIN 
        SELECT @CustomXmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_STEP_PRECISION_QUEUE_MEMBER' AND MEMBER_TYPE_ID = '00039592-1000-0000-0000-000000000010' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        IF @ParentItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_PRECISION_QUEUE WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            ...
            BEGIN 
                ...
                BEGIN TRANSACTION PADD_precisionqueuesTran;
                    ...
                    BEGIN 
                        ...
                        BEGIN TRY;
                            -- Add the base data
                            INSERT INTO dbo.TB_DIM_PRECISION_QUEUE_STEP_PRECISION_QUEUE_MEMBER 
                            (
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_INSERT_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 78, 79, DEFAULT, DEFAULT, 'S', 0, 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_precision_queue_step_precision_queue_member]
	TO [portalapp_role]
GO
