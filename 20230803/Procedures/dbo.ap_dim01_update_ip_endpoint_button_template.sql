...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ip_endpoint_button_template]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE' AND ITEM_TYPE_ID = '00011000-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE' AND ITEM_TYPE_ID = '00011000-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldModelCodeBizUrn = item.[MODEL_CODE_BIZ_URN], 
        @OldProtocolCodeBizUrn = item.[PROTOCOL_CODE_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'IP_ENDPOINT_BUTTON_TEMPLATE_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldModelCodeBizUrn IS NOT NULL AND @ModelCodeBizUrn IS NOT NULL AND @OldModelCodeBizUrn <> @ModelCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldModelCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ModelCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('IP_ENDPOINT_BUTTON_TEMPLATE_MODEL_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('IP_ENDPOINT_BUTTON_TEMPLATE_PROTOCOL_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_ipendpointbuttoTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_ip_endpoint_button_template @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_ip_endpoint_button_template returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_ip_endpoint_button_template @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_ip_endpoint_button_template returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[MODEL_CODE_BIZ_URN] = @ModelCodeBizUrn        
,[PROTOCOL_CODE_BIZ_URN] = @ProtocolCodeBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 54, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_ipendpointbuttoTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ip_endpoint_button_template]
	TO [portalapp_role]
GO
