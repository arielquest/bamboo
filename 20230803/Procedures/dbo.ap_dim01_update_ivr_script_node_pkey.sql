...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ivr_script_node_pkey]
(    
    ...
    @SpeechRecognition       [BIT] = 0, 
    @DeflectionPoint         [BIT] = 0, 
    @DeflectionCategory      [NVARCHAR](50) = NULL, 
    @DeflectionException     [BIT] = 0, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldSpeechRecognition            [BIT], 
                @OldDeflectionPoint              [BIT], 
                @OldDeflectionCategory           [NVARCHAR](50), 
                @OldDeflectionException          [BIT], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_ivr_script_node_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00004610-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldSpeechRecognition = map.[SPEECH_RECOGNITION],
                                @OldDeflectionPoint = map.[DEFLECTION_POINT],
                                @OldDeflectionCategory = map.[DEFLECTION_CATEGORY],
                                @OldDeflectionException = map.[DEFLECTION_EXCEPTION], 
                        ...
                        FROM dbo.TB_DIM_IVR_SCRIPT_NODE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_IVR_SCRIPT_NODE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_IVR_SCRIPT_NODE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'IVR_SCRIPT_NODE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.IVR_SCRIPT_NODE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_SCRIPT_NODE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_SCRIPT_NODE_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldSpeechRecognition IS NOT NULL AND @SpeechRecognition IS NOT NULL AND @OldSpeechRecognition <> @SpeechRecognition)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldSpeechRecognition WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @SpeechRecognition WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_SCRIPT_NODE_SPEECH_RECOGNITION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDeflectionPoint IS NOT NULL AND @DeflectionPoint IS NOT NULL AND @OldDeflectionPoint <> @DeflectionPoint)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldDeflectionPoint WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @DeflectionPoint WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_SCRIPT_NODE_DEFLECTION_POINT', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDeflectionCategory IS NOT NULL AND @DeflectionCategory IS NOT NULL AND @OldDeflectionCategory COLLATE Latin1_General_CS_AS <> @DeflectionCategory COLLATE Latin1_General_CS_AS)  
                        OR (@OldDeflectionCategory IS NULL AND @DeflectionCategory IS NOT NULL)
                        OR (@OldDeflectionCategory IS NOT NULL AND @DeflectionCategory IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldDeflectionCategory AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@DeflectionCategory AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_SCRIPT_NODE_DEFLECTION_CATEGORY', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDeflectionException IS NOT NULL AND @DeflectionException IS NOT NULL AND @OldDeflectionException <> @DeflectionException)  
                        BEGIN  
                            SET @AuditTempOld = CASE @OldDeflectionException WHEN 1 THEN 'Yes' ELSE 'No' END;
                            SET @AuditTempNew = CASE @DeflectionException WHEN 1 THEN 'Yes' ELSE 'No' END; 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_SCRIPT_NODE_DEFLECTION_EXCEPTION', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_ivrscripPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_IVR_SCRIPT_NODE_PKEY_MAP
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [DEFLECTION_POINT] = @DeflectionPoint, 
                                    [DEFLECTION_CATEGORY] = @DeflectionCategory, 
                                    [DEFLECTION_EXCEPTION] = @DeflectionException, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 60, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_ivrscripPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ivr_script_node_pkey]
	TO [portalapp_role]
GO
