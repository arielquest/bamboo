...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_dial_number_plan_pkey]
(    
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_dial_number_plan_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00009300-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldDialString = map.[DIAL_STRING],
                                @OldPostRoute = map.[POST_ROUTE], 
                        ...
                        FROM dbo.TB_DIM_DIAL_NUMBER_PLAN_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_DIAL_NUMBER_PLAN AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_DIAL_NUMBER_PLAN_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'DIAL_NUMBER_PLAN_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.DIAL_NUMBER_PLAN_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIAL_NUMBER_PLAN_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIAL_NUMBER_PLAN_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDialString IS NOT NULL AND @DialString IS NOT NULL AND @OldDialString COLLATE Latin1_General_CS_AS <> @DialString COLLATE Latin1_General_CS_AS)  
                        OR (@OldDialString IS NULL AND @DialString IS NOT NULL)
                        OR (@OldDialString IS NOT NULL AND @DialString IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldDialString AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@DialString AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIAL_NUMBER_PLAN_DIAL_STRING', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldPostRoute IS NOT NULL AND @PostRoute IS NOT NULL AND @OldPostRoute <> @PostRoute)  
                        OR (@OldPostRoute IS NULL AND @PostRoute IS NOT NULL)
                        OR (@OldPostRoute IS NOT NULL AND @PostRoute IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldPostRoute AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@PostRoute AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DIAL_NUMBER_PLAN_POST_ROUTE', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_dialnumbePKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_DIAL_NUMBER_PLAN_PKEY_MAP
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 37, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_dialnumbePKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_dial_number_plan_pkey]
	TO [portalapp_role]
GO
