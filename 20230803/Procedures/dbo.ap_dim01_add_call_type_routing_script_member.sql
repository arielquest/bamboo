...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_call_type_routing_script_member] 
(
...
AS
    ...
    IF @XmlData IS NULL
    BEGIN 
        SELECT @XmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_CALL_TYPE_ROUTING_SCRIPT_MEMBER' AND MEMBER_TYPE_ID = '00200000-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    
    -- Get Custom Default XML
    IF @CustomXmlData IS NULL
    BEGIN 
        SELECT @CustomXmlData = DEFAULT_XML FROM dbo.TB_ADM_XML_DEFAULTS WHERE TABLE_NAME = 'TB_DIM_CALL_TYPE_ROUTING_SCRIPT_MEMBER' AND MEMBER_TYPE_ID = '00200000-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        IF @ParentItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_ROUTING_SCRIPT WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            IF @ChildItemBusinessUrn IS NOT NULL AND EXISTS(SELECT 1 FROM dbo.TB_DIM_CALL_TYPE WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ChildItemBusinessUrn)
            BEGIN 
                -- Insert the base and derived table in a transaction to ensure an atomic insert.
                BEGIN TRANSACTION PADD_calltyperoutingTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Add the base data
                            INSERT INTO dbo.TB_DIM_CALL_TYPE_ROUTING_SCRIPT_MEMBER 
                            (
                             [PARENT_ITEM_BIZ_URN], [CHILD_ITEM_BIZ_URN], [EFFECTIVE_FROM], [EFFECTIVE_TO], [XML_DATA], [SYSTEM], [MODIFIED_BY_ID], [CREATED_BY_ID], [CUSTOM_XML_DATA], [ITEM] 
                            )
                            SELECT TOP 1 @ParentItemBusinessUrn, @ChildItemBusinessUrn, @EffectiveFrom, @EffectiveTo, @XmlData, @System, @SessionUserId, @SessionUserId, @CustomXmlData, @Item 
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_INSERT_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 89, 12, DEFAULT, DEFAULT, 'S', 0, 
                            'MEMBER_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                            IF @InitialTransactionCount = 0 
                            BEGIN 
                                COMMIT TRANSACTION PADD_calltyperoutingTran;
                            END;
                            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_call_type_routing_script_member]
	TO [portalapp_role]
GO
