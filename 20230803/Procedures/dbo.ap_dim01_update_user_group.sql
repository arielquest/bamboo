...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_user_group]
(
    ...
    @AgentTeamBizUrn                 [INT] = -1, 
    @SkillGroupBizUrn                [INT] = -1, 
...
AS
    ...
    @OldAgentTeamBizUrn              [INT],
    @OldSkillGroupBizUrn             [INT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_USER_GROUP' AND ITEM_TYPE_ID = 'C95A5F46-9DEC-4CF3-80BC-F6943F3CF0C0' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_USER_GROUP' AND ITEM_TYPE_ID = 'C95A5F46-9DEC-4CF3-80BC-F6943F3CF0C0' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldAgentTeamBizUrn = item.[AGENT_TEAM_BIZ_URN], 
        @OldSkillGroupBizUrn = item.[SKILLGROUP_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_USER_GROUP item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'USER_GROUP_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldAgentTeamBizUrn IS NOT NULL AND @AgentTeamBizUrn IS NOT NULL AND @OldAgentTeamBizUrn <> @AgentTeamBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldAgentTeamBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@AgentTeamBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('USER_GROUP_AGENT_TEAM_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
            SET @DoStandardType2 = 1;
        END;
            
        IF (@OldSkillGroupBizUrn IS NOT NULL AND @SkillGroupBizUrn IS NOT NULL AND @OldSkillGroupBizUrn <> @SkillGroupBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldSkillGroupBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@SkillGroupBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('USER_GROUP_SKILLGROUP_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
            SET @DoStandardType2 = 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_userTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_user_group @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_user_group returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_user_group @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_user_group returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[AGENT_TEAM_BIZ_URN] = @AgentTeamBizUrn        
,[SKILLGROUP_BIZ_URN] = @SkillGroupBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 114, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_userTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_user_group]
	TO [portalapp_role]
GO
