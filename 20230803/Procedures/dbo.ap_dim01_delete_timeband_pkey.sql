...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_timeband_pkey]
(
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    WHERE ITEM_TYPE_ID = '00007810-1000-0000-0000-000000000010';
                    ...
                    BEGIN TRANSACTION PDEL_timPKEYTran;
                    BEGIN TRY;
                        -- Perform update
                        IF @Purge = 1
                        BEGIN 
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, DEFAULT, 98, DEFAULT, @ClusterResourceId, DEFAULT, 'S', 0, 'DIMENSION_PURGED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                        END
                        ELSE BEGIN 
                            UPDATE dbo.TB_DIM_TIMEBAND_PKEY_MAP  
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, DEFAULT, 98, DEFAULT, @ClusterResourceId, DEFAULT, 'S', 0, 'DIMENSION_DELETED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                        END;
                    END TRY
                    BEGIN CATCH;
                    ...
                    BEGIN 
                        COMMIT TRANSACTION PDEL_timPKEYTran;
                    END;
                    ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_timeband_pkey]
	TO [portalapp_role]
GO
