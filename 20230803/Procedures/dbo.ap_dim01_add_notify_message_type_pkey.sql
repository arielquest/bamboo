...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_notify_message_type_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode AS [INTEGER] = 0,
                @Status AS [CHAR](1),
                @ItemTypeId AS [UNIQUEIDENTIFIER] = 'a1a245cd-acce-4d76-9839-7a589515601a',
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    IF NOT EXISTS(SELECT 1 
                                  FROM dbo.TB_DIM_NOTIFY_MESSAGE_TYPE WITH(READUNCOMMITTED) 
                    ...
                    ELSE BEGIN 
                        ...
                        IF EXISTS(SELECT 1 
                                  FROM [dbo].[TB_DIM_NOTIFY_MESSAGE_TYPE_PKEY_MAP] WITH(READUNCOMMITTED) 
                        ...
                        ELSE BEGIN 
                            ...
                            BEGIN TRANSACTION PADD_notifymessagetPKEYTran;
                            BEGIN TRY;
                                INSERT INTO [dbo].[TB_DIM_NOTIFY_MESSAGE_TYPE_PKEY_MAP](
                                ITEM_BIZ_URN, 
                                CLUSTER_RESOURCE_ID, 
                                STATUS, 
                                EFFECTIVE_FROM, 
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, DEFAULT, 128, DEFAULT, @ClusterResourceId, DEFAULT, 'S', 0, 'DIMENSION_ADDED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                
                            IF @InitialTransactionCount = 0 
                            BEGIN 
                                COMMIT TRANSACTION PADD_notifymessagetPKEYTran;
                            END;
                            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_notify_message_type_pkey]
	TO [portalapp_role]
GO
