...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_custom_attribute1] (@SessionId UNIQUEIDENTIFIER, @ItemUrn INT, @ChangeStamp INT OUTPUT)
AS BEGIN 
    SET NOCOUNT ON;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_CUSTOM_ATTRIBUTE1 WITH (UPDLOCK)
            ...
            BEGIN
                BEGIN TRANSACTION PDEL_customattributeTran;
                    BEGIN TRY;
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_DELETE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 133, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_DELETED', NULL, NULL, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_custom_attribute1]
	TO [portalapp_role]
GO
