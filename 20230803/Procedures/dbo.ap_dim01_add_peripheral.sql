...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_peripheral]
(
...
BEGIN 
    ...
    BEGIN 
        ...
        BEGIN
            ...
            BEGIN TRANSACTION PADD_peripTran;
                WHILE @deadlockRetry < 3
                BEGIN 
                    SAVE TRANSACTION deadlockSavePoint;
                    BEGIN TRY;
                        -- Add the base data
                        INSERT INTO dbo.TB_DIM_PERIPHERAL
                        (
                       ...
                       , [APPLICATION_USER_BIZ_URN]
                        ...
                        @XmlData, @TimeZoneUrn, @TenantBizUrn, @HierarchyParentUrn, @CustomXmlData, @SessionUserId, @SessionUserId
                         , @ApplicationUserBizUrn
                        );

                        SELECT @ItemUrn = SCOPE_IDENTITY();
            
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_INSERT_COMMITTED', DEFAULT, @ItemUrn, DEFAULT, 73, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PADD_peripTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_peripheral]
	TO [portalapp_role]
GO
