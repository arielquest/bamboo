...
GO
-------------------------------------------------------------------------------
-- This procedure checks the data processed within the cubes against 
-- the data imported in the fact and summary tables.
-------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[ap_imp01_check_olap_unprocessed_data] 
(
    @CubeName AS NVARCHAR(50) = NULL, 
    @MeasureGroupName AS NVARCHAR(128) = NULL, 
    @ServerName AS NVARCHAR(50) = NULL
) AS
BEGIN 
    SET NOCOUNT ON;

    DECLARE @ErrorMessage AS VARCHAR(2000), @v_Error INT, @v_ResourceInstanceID AS VARCHAR(50);

    --Get the RESOURCE_INSTANCE_ID of the machine.
    SELECT @v_ResourceInstanceID = RESOURCE_INSTANCE_ID
    ...
    BEGIN TRY;
        ...
        BEGIN 
            IF @MeasureGroupName IS NOT NULL
            BEGIN 
                ...
                WHERE UPPER([ol].[PARENT_OLAP_OBJECT_NAME]) = UPPER(@CubeName)
                    ...
                    AND [fp].[RESOURCE_INSTANCE_ID] = @v_ResourceInstanceID
                    AND [fp].[ONLINE_DATE] IS NOT NULL
                    AND [fp].[OFFLINE_DATE] IS NULL
                    AND [ol].[OLAP_OBJECT_URN] <> 123 --Case
        ...
        ELSE BEGIN 
            ...
            INNER JOIN [dbo].[TB_ADM_OLAP_OBJECT] AS ol ON [fpm].[CUBE_VIEW_NAME] = [ol].[PRIMARY_TABLE]
            INNER JOIN [dbo].[TB_ADM_OLAP_OBJECT_CLUSTER_MAP] AS mp ON [ol].[OLAP_OBJECT_URN] = [mp].[OLAP_OBJECT_URN]
            WHERE [fp].[RESOURCE_INSTANCE_ID] = @v_ResourceInstanceID
                    AND [fp].[ONLINE_DATE] IS NOT NULL
                    AND [fp].[OFFLINE_DATE] IS NULL
                    AND [ol].[OLAP_OBJECT_URN] <> 123 --Case
            GROUP BY [fp].[FACT_PARTITION_ID], [fpm].[CUBE_VIEW_NAME], [ol].[PARENT_OLAP_OBJECT_NAME], [ol].[OLAP_OBJECT_NAME], [mp].[ENABLED], [fp].[PARTITION_ID];
