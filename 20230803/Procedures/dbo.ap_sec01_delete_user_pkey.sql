...
GO
CREATE PROCEDURE [dbo].[ap_sec01_delete_user_pkey]
(
    ...
    BEGIN TRY;
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                -- If user id and cluster are not supplied return
                IF @PkeyMapUrn IS NOT NULL
                BEGIN 
                    SELECT  @UserId = map.[USER_ID],
                    ...
                    -- Deleting a user which is not yet provisioned means no provisioning is required to delete it
                    ...
                    BEGIN 
                        SET @Status = 'P';
                        SET @AuditTypeName = 'AT_USER_DELETE_REQUESTED_ON_EQUIPMENT'; -- Delete Request
                    END;
                    ELSE BEGIN 
                        -- Default to no provisioning required
                        SET @Status = 'D';
                        SET @AuditTypeName = 'AT_USER_DELETE_COMMITTED_ON_EQUIPMENT'; -- Purge Execution, Delete Execution or Expired?
                    END;
    
                    BEGIN TRANSACTION userPkDeleteTran;
                    BEGIN TRY;
                        -- Perform update
                        IF @Purge = 1
                        BEGIN 
                            UPDATE dbo.TB_SEC_USER_PKEY_MAP 
                            SET EFFECTIVE_TO = CASE WHEN STATUS = 'D' THEN EFFECTIVE_TO ELSE @UtcNow END, -- If the user is being deleted and purged then set effective date. If just purge then don't.
                            ...
                            BEGIN 
                                SET @ReturnCode = -2;
                                SET @ErrorMessage = OBJECT_NAME(@@PROCID) + ': Concurrency conflict occured (or attempt to purge already purged entry) whilst attempting to purge user pkey map [%d], Error [%d]';
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, @ClusterResourceId, @UserId, 'S', 0, 'USER_PURGED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                        END
                        ELSE BEGIN 
                            UPDATE dbo.TB_SEC_USER_PKEY_MAP  
                            ...
                            BEGIN 
                                SET @ReturnCode = -2;
                                SET @ErrorMessage = OBJECT_NAME(@@PROCID) + ': Concurrency conflict occured (or attempt to delete already deleted entry) whilst attempting to delete user pkey map [%d], Error [%d]';
                                RAISERROR(@ErrorMessage, 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                            END;

                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, @ClusterResourceId, @UserId, 'S', 0, 'USER_DELETED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                        END;
                    END TRY
                    BEGIN CATCH;
                    ...
                    BEGIN 
                        COMMIT TRANSACTION userPkDeleteTran;
                    END;
                    ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_sec01_delete_user_pkey]
	TO [portalapp_role]
GO
