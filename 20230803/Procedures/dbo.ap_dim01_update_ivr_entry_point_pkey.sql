...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ivr_entry_point_pkey]
(    
    ...
    @HiddenTollFreeNumber    [NVARCHAR](50) = NULL, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldHiddenTollFreeNumber         [NVARCHAR](50), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_ivr_entry_point_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00011200-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldHiddenTollFreeNumber = map.[HIDDEN_TOLL_FREE_NUMBER],
                                @OldRegion = map.[REGION],
                                @OldLanguage = map.[LANGUAGE], 
                        ...
                        FROM dbo.TB_DIM_IVR_ENTRY_POINT_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_IVR_ENTRY_POINT AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_IVR_ENTRY_POINT_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'IVR_ENTRY_POINT_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.IVR_ENTRY_POINT_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ENTRY_POINT_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ENTRY_POINT_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldHiddenTollFreeNumber IS NOT NULL AND @HiddenTollFreeNumber IS NOT NULL AND @OldHiddenTollFreeNumber COLLATE Latin1_General_CS_AS <> @HiddenTollFreeNumber COLLATE Latin1_General_CS_AS)  
                        OR (@OldHiddenTollFreeNumber IS NULL AND @HiddenTollFreeNumber IS NOT NULL)
                        OR (@OldHiddenTollFreeNumber IS NOT NULL AND @HiddenTollFreeNumber IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldHiddenTollFreeNumber AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@HiddenTollFreeNumber AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ENTRY_POINT_HIDDEN_TOLL_FREE_NUMBER', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldRegion IS NOT NULL AND @Region IS NOT NULL AND @OldRegion COLLATE Latin1_General_CS_AS <> @Region COLLATE Latin1_General_CS_AS)  
                        OR (@OldRegion IS NULL AND @Region IS NOT NULL)
                        OR (@OldRegion IS NOT NULL AND @Region IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldRegion AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Region AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ENTRY_POINT_REGION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldLanguage IS NOT NULL AND @Language IS NOT NULL AND @OldLanguage <> @Language)  
                        OR (@OldLanguage IS NULL AND @Language IS NOT NULL)
                        OR (@OldLanguage IS NOT NULL AND @Language IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldLanguage AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Language AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('IVR_ENTRY_POINT_LANGUAGE', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_ivrentryPKEYTran;
                            BEGIN TRY;
                                ...
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [HIDDEN_TOLL_FREE_NUMBER] = @HiddenTollFreeNumber, 
                                    [REGION] = @Region, 
                                    [LANGUAGE] = @Language, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 55, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_ivrentryPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ivr_entry_point_pkey]
	TO [portalapp_role]
GO
