...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_skillgroup_campaign_member] 
(
...
AS
    ...
    BEGIN 
        ...
        WHERE TABLE_NAME = 'TB_DIM_SKILLGROUP_CAMPAIGN_MEMBER' AND MEMBER_TYPE_ID = '00000034-1000-0000-0000-0000000000C0' AND CUSTOM = 0;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_ADM_XML_DEFAULTS 
        WHERE TABLE_NAME = 'TB_DIM_SKILLGROUP_CAMPAIGN_MEMBER' AND MEMBER_TYPE_ID = '00000034-1000-0000-0000-0000000000C0' AND CUSTOM = 1;
    END;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_SKILLGROUP_CAMPAIGN_MEMBER member
        ...
        IF EXISTS(SELECT 1 FROM dbo.TB_DIM_CAMPAIGN WITH (READUNCOMMITTED) WHERE ITEM_BIZ_URN = @ParentItemBusinessUrn)
        BEGIN 
            ...
            BEGIN 
                ...
                INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                SELECT 'SKILLGROUP_CAMPAIGN_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
                ...
                BEGIN 
                    -- Update the tables
                    BEGIN TRANSACTION PUPD_skillgroupcampaTran;
                        ...
                        BEGIN 
                            ...
                            BEGIN TRY;
                                ...
                                IF @DoType2 = 1
                                BEGIN 
                                    EXEC @ReturnCode = dbo.ap_dim01_do_type2_skillgroup_campaign_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @ChangeStamp, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 1;
                                    IF @ReturnCode <> 0
                                    BEGIN 
                                        RAISERROR ('ap_dim01_do_type2_skillgroup_campaign_member returned [%d] Error raised in TRY block.', 16, 1, @ReturnCode);
                                    END;
                                ...
                                ELSE BEGIN 
                                    ...
                                    BEGIN 
                                        COMMIT TRANSACTION PUPD_skillgroupcampaTran;
                                    END;
                                    ELSE BEGIN 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_MEMBER_UPDATE_COMMITTED', DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 13, 95, DEFAULT, DEFAULT, 'S', 0, 'MEMBER_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_skillgroup_campaign_member]
	TO [portalapp_role]
GO
