...
GO
CREATE PROCEDURE [dbo].[ap_dim01_prov_update_member_pkey_map]
    (
      -- optional fields allow setting of just the pkey1 and pkey2 values
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN TRY;
                ...
                BEGIN
                    SET @ReturnCode = 5000;
                    RAISERROR ('Invalid Status Change. @PkeyMap=%d @OldStatus=%s @NewStatus=%s, Type=%d', 16, 1, @PkeyMapUrn, @OldStatus, @Status, @TypeUrn);
                END;
                ...
                BEGIN
                    ...
                    EXEC [dbo].[ap_dim01_do_type2_member_pkey_map] @SessionId = @SessionId, @PkeyMapUrn = @PkeyMapUrn OUTPUT, @MemberTypeUrn = @TypeUrn,
                        @ChangeStamp = @ChangeStamp, @EffectiveFrom = @UtcNow, @IgnoreConsolidationPeriod = 1, @MaintainBusinessKey = 1;
                                
                    SET @ChangeStamp = 0;
                END;
            
                -- Set member pkey map fields
            ...
            BEGIN CATCH;
                SELECT  @ReturnCode = CASE WHEN @ReturnCode NOT IN ( -2, 5000 ) THEN ERROR_NUMBER()
                                           ELSE @ReturnCode
                                      END, @ErrorMessage = ERROR_MESSAGE();
                -- User defined error? if not get from ERROR_NUMBER

                -- Can't insert duplicate key!
                IF @ReturnCode = 2601
                BEGIN
                    -- DuplicateMemberEquipmentMapping
                    SET @ReturnCode = 50505;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_prov_update_member_pkey_map]
	TO [portalapp_role]
GO
