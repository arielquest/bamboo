...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_sub_code_pkey]
(    
    ...
    @ClusterVersion          [NVARCHAR](50) = NULL, 
    @ClusterResourceTypeId   [UNIQUEIDENTIFIER] = NULL, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldClusterVersion               [NVARCHAR](50), 
                @OldClusterResourceTypeId        [UNIQUEIDENTIFIER], 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_sub_code_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '24fb83b4-a581-4642-a847-0ab90f78be38';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldClusterVersion = map.[CLUSTER_VERSION],
                                @OldClusterResourceTypeId = map.[CLUSTER_RESOURCE_TYPE_ID], 
                        ...
                        FROM dbo.TB_DIM_SUB_CODE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_SUB_CODE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_SUB_CODE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'SUB_CODE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.SUB_CODE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        IF (@OldClusterVersion IS NOT NULL AND @ClusterVersion IS NOT NULL AND @OldClusterVersion COLLATE Latin1_General_CS_AS <> @ClusterVersion COLLATE Latin1_General_CS_AS)  
                        OR (@OldClusterVersion IS NULL AND @ClusterVersion IS NOT NULL)
                        OR (@OldClusterVersion IS NOT NULL AND @ClusterVersion IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldClusterVersion AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ClusterVersion AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SUB_CODE_CLUSTER_VERSION', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldClusterResourceTypeId IS NOT NULL AND @ClusterResourceTypeId IS NOT NULL AND @OldClusterResourceTypeId <> @ClusterResourceTypeId)  
                        OR (@OldClusterResourceTypeId IS NULL AND @ClusterResourceTypeId IS NOT NULL)
                        OR (@OldClusterResourceTypeId IS NOT NULL AND @ClusterResourceTypeId IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldClusterResourceTypeId AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ClusterResourceTypeId AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('SUB_CODE_CLUSTER_RESOURCE_TYPE_ID', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_suPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_SUB_CODE_PKEY_MAP
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [CLUSTER_RESOURCE_TYPE_ID] = @ClusterResourceTypeId, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 117, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_suPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_sub_code_pkey]
	TO [portalapp_role]
GO
