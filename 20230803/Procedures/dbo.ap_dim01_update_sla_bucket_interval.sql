...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_sla_bucket_interval]
(
    ...
    @ActivityTypeCodeBizUrn          [INT] = -1, 
    @IntervalUpperBound1             [INT] = 0, 
...
AS
    ...
    @OldActivityTypeCodeBizUrn       [INT],
    @OldIntervalUpperBound1          [INT],
    @OldIntervalUpperBound2          [INT],
    @OldIntervalUpperBound3          [INT],
    @OldIntervalUpperBound4          [INT],
    @OldIntervalUpperBound5          [INT],
    @OldIntervalUpperBound6          [INT],
    @OldIntervalUpperBound7          [INT],
    @OldIntervalUpperBound8          [INT],
    @OldIntervalUpperBound9          [INT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_SLA_BUCKET_INTERVAL' AND ITEM_TYPE_ID = '2BE66C38-5592-4DBA-BC11-81D3395163B0' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_SLA_BUCKET_INTERVAL' AND ITEM_TYPE_ID = '2BE66C38-5592-4DBA-BC11-81D3395163B0' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldActivityTypeCodeBizUrn = item.[ACTIVITY_TYPE_CODE_BIZ_URN], 
        @OldIntervalUpperBound1 = item.[INTERVAL_UPPER_BOUND_1], 
        @OldIntervalUpperBound2 = item.[INTERVAL_UPPER_BOUND_2], 
        @OldIntervalUpperBound3 = item.[INTERVAL_UPPER_BOUND_3], 
        @OldIntervalUpperBound4 = item.[INTERVAL_UPPER_BOUND_4], 
        @OldIntervalUpperBound5 = item.[INTERVAL_UPPER_BOUND_5], 
        @OldIntervalUpperBound6 = item.[INTERVAL_UPPER_BOUND_6], 
        @OldIntervalUpperBound7 = item.[INTERVAL_UPPER_BOUND_7], 
        @OldIntervalUpperBound8 = item.[INTERVAL_UPPER_BOUND_8], 
        @OldIntervalUpperBound9 = item.[INTERVAL_UPPER_BOUND_9], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_SLA_BUCKET_INTERVAL item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'SLA_BUCKET_INTERVAL_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldActivityTypeCodeBizUrn IS NOT NULL AND @ActivityTypeCodeBizUrn IS NOT NULL AND @OldActivityTypeCodeBizUrn <> @ActivityTypeCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldActivityTypeCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@ActivityTypeCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_ACTIVITY_TYPE_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound1 IS NOT NULL AND @IntervalUpperBound1 IS NOT NULL AND @OldIntervalUpperBound1 <> @IntervalUpperBound1) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound1 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound1 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_1', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound2 IS NOT NULL AND @IntervalUpperBound2 IS NOT NULL AND @OldIntervalUpperBound2 <> @IntervalUpperBound2) 
        OR (@OldIntervalUpperBound2 IS NULL AND @IntervalUpperBound2 IS NOT NULL)
        OR (@OldIntervalUpperBound2 IS NOT NULL AND @IntervalUpperBound2 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound2 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound2 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_2', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound3 IS NOT NULL AND @IntervalUpperBound3 IS NOT NULL AND @OldIntervalUpperBound3 <> @IntervalUpperBound3) 
        OR (@OldIntervalUpperBound3 IS NULL AND @IntervalUpperBound3 IS NOT NULL)
        OR (@OldIntervalUpperBound3 IS NOT NULL AND @IntervalUpperBound3 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound3 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound3 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_3', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound4 IS NOT NULL AND @IntervalUpperBound4 IS NOT NULL AND @OldIntervalUpperBound4 <> @IntervalUpperBound4) 
        OR (@OldIntervalUpperBound4 IS NULL AND @IntervalUpperBound4 IS NOT NULL)
        OR (@OldIntervalUpperBound4 IS NOT NULL AND @IntervalUpperBound4 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound4 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound4 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_4', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound5 IS NOT NULL AND @IntervalUpperBound5 IS NOT NULL AND @OldIntervalUpperBound5 <> @IntervalUpperBound5) 
        OR (@OldIntervalUpperBound5 IS NULL AND @IntervalUpperBound5 IS NOT NULL)
        OR (@OldIntervalUpperBound5 IS NOT NULL AND @IntervalUpperBound5 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound5 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound5 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_5', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound6 IS NOT NULL AND @IntervalUpperBound6 IS NOT NULL AND @OldIntervalUpperBound6 <> @IntervalUpperBound6) 
        OR (@OldIntervalUpperBound6 IS NULL AND @IntervalUpperBound6 IS NOT NULL)
        OR (@OldIntervalUpperBound6 IS NOT NULL AND @IntervalUpperBound6 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound6 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound6 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_6', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound7 IS NOT NULL AND @IntervalUpperBound7 IS NOT NULL AND @OldIntervalUpperBound7 <> @IntervalUpperBound7) 
        OR (@OldIntervalUpperBound7 IS NULL AND @IntervalUpperBound7 IS NOT NULL)
        OR (@OldIntervalUpperBound7 IS NOT NULL AND @IntervalUpperBound7 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound7 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound7 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_7', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound8 IS NOT NULL AND @IntervalUpperBound8 IS NOT NULL AND @OldIntervalUpperBound8 <> @IntervalUpperBound8) 
        OR (@OldIntervalUpperBound8 IS NULL AND @IntervalUpperBound8 IS NOT NULL)
        OR (@OldIntervalUpperBound8 IS NOT NULL AND @IntervalUpperBound8 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound8 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound8 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_8', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntervalUpperBound9 IS NOT NULL AND @IntervalUpperBound9 IS NOT NULL AND @OldIntervalUpperBound9 <> @IntervalUpperBound9) 
        OR (@OldIntervalUpperBound9 IS NULL AND @IntervalUpperBound9 IS NOT NULL)
        OR (@OldIntervalUpperBound9 IS NOT NULL AND @IntervalUpperBound9 IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntervalUpperBound9 AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntervalUpperBound9 AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SLA_BUCKET_INTERVAL_INTERVAL_UPPER_BOUND_9', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_slabucketintervTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_sla_bucket_interval @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_sla_bucket_interval returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_sla_bucket_interval @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_sla_bucket_interval returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
                        ...
                        UPDATE dbo.TB_DIM_SLA_BUCKET_INTERVAL
...
,[ACTIVITY_TYPE_CODE_BIZ_URN] = @ActivityTypeCodeBizUrn        
,[INTERVAL_UPPER_BOUND_1] = @IntervalUpperBound1        
,[INTERVAL_UPPER_BOUND_2] = @IntervalUpperBound2        
,[INTERVAL_UPPER_BOUND_3] = @IntervalUpperBound3        
,[INTERVAL_UPPER_BOUND_4] = @IntervalUpperBound4        
,[INTERVAL_UPPER_BOUND_5] = @IntervalUpperBound5        
,[INTERVAL_UPPER_BOUND_6] = @IntervalUpperBound6        
,[INTERVAL_UPPER_BOUND_7] = @IntervalUpperBound7        
,[INTERVAL_UPPER_BOUND_8] = @IntervalUpperBound8        
,[INTERVAL_UPPER_BOUND_9] = @IntervalUpperBound9        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 131, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_slabucketintervTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_sla_bucket_interval]
	TO [portalapp_role]
GO
