...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_call_type_pkey]
(    
    ...
    @CcaiConfigParameter     [NVARCHAR](100) = -1, 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldCcaiConfigParameter          [NVARCHAR](100), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_call_type_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00001500-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldCcaiConfigParameter = map.[CCAI_CONFIG_PARAMETER], 
                        ...
                        FROM dbo.TB_DIM_CALL_TYPE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_CALL_TYPE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_CALL_TYPE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'CALL_TYPE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.CALL_TYPE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('CALL_TYPE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('CALL_TYPE_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        OR (@OldServiceLevelType IS NULL AND @ServiceLevelType IS NOT NULL)
                        OR (@OldServiceLevelType IS NOT NULL AND @ServiceLevelType IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldServiceLevelType AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@ServiceLevelType AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('CALL_TYPE_SERVICE_LEVEL_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('CALL_TYPE_SERVICE_LEVEL_THRESHOLD', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldCcaiConfigParameter IS NOT NULL AND @CcaiConfigParameter IS NOT NULL AND @OldCcaiConfigParameter COLLATE Latin1_General_CS_AS <> @CcaiConfigParameter COLLATE Latin1_General_CS_AS)  
                        OR (@OldCcaiConfigParameter IS NULL AND @CcaiConfigParameter IS NOT NULL)
                        OR (@OldCcaiConfigParameter IS NOT NULL AND @CcaiConfigParameter IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldCcaiConfigParameter AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@CcaiConfigParameter AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('CALL_TYPE_CCAI_CONFIG_PARAMETER', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_calPKEYTran;
                            BEGIN TRY;
                                ...
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [CCAI_CONFIG_PARAMETER] = @CcaiConfigParameter, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 12, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_calPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_call_type_pkey]
	TO [portalapp_role]
GO
