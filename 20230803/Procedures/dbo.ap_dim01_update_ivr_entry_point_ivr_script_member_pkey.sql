...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_ivr_entry_point_ivr_script_member_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_update_ivr_entry_point_ivr_script_member_pkey');
        END;
        ...
        BEGIN    
            ...
            SET @MemberTypeId = '00000042-1000-0000-0000-0000000000c0';
            SELECT @MemberTypeIsProvisionable = ISNULL((SELECT TOP 1 1 
                                                        FROM dbo.TE_DIM_ITEM_TYPE 
                                                        WHERE ITEM_TYPE_ID IN ('00004600-1000-0000-0000-000000000010', '00011200-1000-0000-0000-000000000010')
            ...
            FROM dbo.TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER_PKEY_MAP AS pm WITH(UPDLOCK)
            INNER JOIN dbo.TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER AS m WITH(READUNCOMMITTED) ON pm.MEMBER_BIZ_URN = m.MEMBER_BIZ_URN AND m.LATEST = 1
            ...
            BEGIN 
                ...
                WHERE TABLE_NAME = 'TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER_PKEY_MAP' 
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'custom.IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
            SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
            SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
            ...
            BEGIN        
                ...
                BEGIN 
                    RAISERROR('ap_dim01_update_ivr_entry_point_ivr_script_member_pkey: Cannot perform provisionable update on deleted or deleting pkey map [%d]', 16, 1, @PkeyMapUrn) WITH LOG;
                END;
                ...
                BEGIN TRANSACTION PUPD_ivrentrypointiPKEYTran;       
                -- Update the pkey  record
                UPDATE dbo.TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER_PKEY_MAP
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_update_ivr_entry_point_ivr_script_member_pkey: Error updating member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_update_ivr_entry_point_ivr_script_member_pkey: Concurrency conflict occured whilst attempting to update member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 59, 55, @ClusterResourceId, NULL, 'S', 0, 
                    'MEMBER_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
            
                IF @InitialTransactionCount = 0 
                BEGIN 
                    COMMIT TRANSACTION PUPD_ivrentrypointiPKEYTran;
                END;
                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_ivr_entry_point_ivr_script_member_pkey]
	TO [portalapp_role]
GO
