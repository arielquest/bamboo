...
GO
CREATE PROCEDURE [dbo].[ap_dim01_do_type2_ip_endpoint_button_template]
...
AS
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE 
        ...
        BEGIN 
            ...
            BEGIN 
                ...
                BEGIN TRANSACTION PTYP_ipendpointbuttoTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Insert the new type-2 item record
                            INSERT INTO dbo.TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE ([ITEM_BIZ_URN], [SKIP_LEVEL], [FOLDER_ID], [OWNER_ID], [ITEM_SUBTYPE_ID], [NAME], [DESCRIPTION],
[MODEL_CODE_BIZ_URN],
...
[PROTOCOL_CODE_BIZ_URN],
                            ...
                            BEGIN            
                                ...
                                UPDATE dbo.TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE 
...
[LINE_COUNT], 
                                ...
                                UPDATE dbo.TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE_PKEY_MAP 
                                SET STATUS = 'D', 
                                ...
                                INSERT INTO dbo.TB_DIM_IP_ENDPOINT_BUTTON_TEMPLATE_PKEY_MAP  ([PKEY_MAP_BIZ_URN], [ITEM_BIZ_URN], [CLUSTER_RESOURCE_ID], [PKEY1], [PKEY2],                            
[INTERNAL_NAME], 
[LINE_COUNT], 
...
[INTERNAL_NAME], [LINE_COUNT],                                 [STATUS], [EFFECTIVE_FROM], [EFFECTIVE_TO], [CREATED_BY_ID], [MODIFIED_BY_ID], [MODIFIED_DATE], [CREATION_DATE],
                                [PURGED], [SYSTEM], [LATEST], [SOURCE_CHANGE_STAMP], [XML_DATA], [CUSTOM_XML_DATA]
                                FROM #PKEY_MAP_ROWS;
                           ...
                           BEGIN 
                               ...
                               VALUES('AT_DIMENSION_TYPE_2_PERFORMED', @ItemBusinessUrn, @NewItemUrn, 54, 'DIMENSION_TYPE_2_CHANGE');
                           END;
                           ELSE BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)                
                               VALUES('AT_DIMENSION_TYPE_2_TOMBSTONE_PERFORMED', @ItemBusinessUrn, @ItemUrn, 54, 'DIMENSION_TYPE_2_CHANGE');
                           END;
            
                           IF @MaintainBusinessKey = 0
                           BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)
                               VALUES('AT_DIMENSION_TYPE_2_REVIVAL_PERFORMED', @NewItemBusinessUrn, @ItemUrn, 54, 'DIMENSION_TYPE_2_CHANGE');
                           ...
                           BEGIN 
                               COMMIT TRANSACTION PTYP_ipendpointbuttoTran;
                           END;
                           ELSE BEGIN 
    ...
    ELSE BEGIN 
        ...
        BEGIN 
            ...
            END;
        END;
        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_TYPE_2_PERFORMED', DEFAULT, @ItemBusinessUrn, DEFAULT, 54, DEFAULT, DEFAULT, DEFAULT, 'F', 0, 'DIMENSION_TYPE_2_CHANGE', NULL, DEFAULT, DEFAULT, @AuditUrn out;
        RAISERROR(@ErrorMessage, 16, 1, @ReturnCode) WITH LOG;
    END;
    RETURN @ReturnCode;
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_do_type2_ip_endpoint_button_template]
	TO [portalapp_role]
GO
