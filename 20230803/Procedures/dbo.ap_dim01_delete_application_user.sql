...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_application_user] (@SessionId UNIQUEIDENTIFIER, @ItemUrn INT, @ChangeStamp INT OUTPUT)
AS BEGIN 
    SET NOCOUNT ON;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_APPLICATION_USER WITH (UPDLOCK)
        ...
        BEGIN 
            ...
            BEGIN
                BEGIN TRANSACTION PDEL_applicatioTran;
                    BEGIN TRY;
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_DELETE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 7, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_DELETED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PDEL_applicatioTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_application_user]
	TO [portalapp_role]
GO
