...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_kb_article_review_stage_pkey]
(
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    WHERE ITEM_TYPE_ID = 'a25e34b9-79e9-4601-9fd8-51a638940e4c';
                    ...
                    BEGIN TRANSACTION PDEL_kbarticlereviePKEYTran;
                    BEGIN TRY;
                        -- Perform update
                        IF @Purge = 1
                        BEGIN 
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, DEFAULT, 172, DEFAULT, @ClusterResourceId, DEFAULT, 'S', 0, 'DIMENSION_PURGED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                        END
                        ELSE BEGIN 
                            UPDATE dbo.TB_DIM_KB_ARTICLE_REVIEW_STAGE_PKEY_MAP  
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, DEFAULT, 172, DEFAULT, @ClusterResourceId, DEFAULT, 'S', 0, 'DIMENSION_DELETED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                        END;
                    END TRY
                    BEGIN CATCH;
                    ...
                    BEGIN 
                        COMMIT TRANSACTION PDEL_kbarticlereviePKEYTran;
                    END;
                    ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_kb_article_review_stage_pkey]
	TO [portalapp_role]
GO
