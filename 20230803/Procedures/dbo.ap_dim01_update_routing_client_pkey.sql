...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_routing_client_pkey]
(    
    ...
    @TimeoutThreshold        [SMALLINT] = NULL, 
    @TimeoutLimit            [SMALLINT] = NULL, 
    @LateThreshold           [SMALLINT] = NULL, 
    @DialedNumberLabelMapPresent  [CHAR](1) = 'N', 
    @NetworkRoutingClient    [NVARCHAR](32) = NULL, 
    @NetworkTransferPrefered  [CHAR](1) = 'N', 
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldTimeoutThreshold             [SMALLINT], 
                @OldTimeoutLimit                 [SMALLINT], 
                @OldLateThreshold                [SMALLINT], 
                @OldDialedNumberLabelMapPresent  [CHAR](1), 
                @OldNetworkRoutingClient         [NVARCHAR](32), 
                @OldNetworkTransferPrefered      [CHAR](1), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_routing_client_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = '00007300-1000-0000-0000-000000000010';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldTimeoutThreshold = map.[TIMEOUT_THRESHOLD],
                                @OldTimeoutLimit = map.[TIMEOUT_LIMIT],
                                @OldLateThreshold = map.[LATE_THRESHOLD],
                                @OldDialedNumberLabelMapPresent = map.[DIALED_NUMBER_LABEL_MAP_PRESENT],
                                @OldNetworkRoutingClient = map.[NETWORK_ROUTING_CLIENT],
                                @OldNetworkTransferPrefered = map.[NETWORK_TRANSFER_PREFERED], 
                        ...
                        FROM dbo.TB_DIM_ROUTING_CLIENT_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_ROUTING_CLIENT AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_ROUTING_CLIENT_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'ROUTING_CLIENT_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.ROUTING_CLIENT_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        BEGIN  
                            ...
                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_CLIENT_TYPE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldTimeoutThreshold IS NOT NULL AND @TimeoutThreshold IS NOT NULL AND @OldTimeoutThreshold <> @TimeoutThreshold)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldTimeoutThreshold AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@TimeoutThreshold AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_TIMEOUT_THRESHOLD', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldTimeoutLimit IS NOT NULL AND @TimeoutLimit IS NOT NULL AND @OldTimeoutLimit <> @TimeoutLimit)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldTimeoutLimit AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@TimeoutLimit AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_TIMEOUT_LIMIT', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldLateThreshold IS NOT NULL AND @LateThreshold IS NOT NULL AND @OldLateThreshold <> @LateThreshold)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldLateThreshold AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@LateThreshold AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_LATE_THRESHOLD', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldDialedNumberLabelMapPresent IS NOT NULL AND @DialedNumberLabelMapPresent IS NOT NULL AND @OldDialedNumberLabelMapPresent <> @DialedNumberLabelMapPresent)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldDialedNumberLabelMapPresent AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@DialedNumberLabelMapPresent AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_DIALED_NUMBER_LABEL_MAP_PRESENT', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldNetworkRoutingClient IS NOT NULL AND @NetworkRoutingClient IS NOT NULL AND @OldNetworkRoutingClient COLLATE Latin1_General_CS_AS <> @NetworkRoutingClient COLLATE Latin1_General_CS_AS)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldNetworkRoutingClient AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@NetworkRoutingClient AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_NETWORK_ROUTING_CLIENT', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldNetworkTransferPrefered IS NOT NULL AND @NetworkTransferPrefered IS NOT NULL AND @OldNetworkTransferPrefered <> @NetworkTransferPrefered)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldNetworkTransferPrefered AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@NetworkTransferPrefered AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROUTING_CLIENT_NETWORK_TRANSFER_PREFERED', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_routingcPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_ROUTING_CLIENT_PKEY_MAP
                                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                                    ...
                                    [DIALED_NUMBER_LABEL_MAP_PRESENT] = @DialedNumberLabelMapPresent, 
                                    [NETWORK_ROUTING_CLIENT] = @NetworkRoutingClient, 
                                    [NETWORK_TRANSFER_PREFERED] = @NetworkTransferPrefered, 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 88, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_routingcPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_routing_client_pkey]
	TO [portalapp_role]
GO
