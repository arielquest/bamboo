...
GO
CREATE PROCEDURE [dbo].[ap_dim01_do_type2_campaign]
    ...
    BEGIN 
        ...
        BEGIN 
            ...
            BEGIN 
                ...
                BEGIN TRANSACTION PTYP_camTran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Insert the new type-2 item record
                            INSERT INTO dbo.TB_DIM_CAMPAIGN ([ITEM_BIZ_URN], [SKIP_LEVEL], [FOLDER_ID], [OWNER_ID], [ITEM_SUBTYPE_ID], [NAME], [DESCRIPTION],
...
[ICM_ENABLED], 
                                ...
                                INSERT INTO dbo.TB_DIM_CAMPAIGN_PKEY_MAP  ([PKEY_MAP_BIZ_URN], [ITEM_BIZ_URN], [CLUSTER_RESOURCE_ID], [PKEY1], [PKEY2],                            
[INTERNAL_NAME], 
[DESCRIPTION], 
[ABANDON_ENABLED], 
[ABANDON_PERCENT], 
...
[INTERNAL_NAME], [DESCRIPTION], [ABANDON_ENABLED], [ABANDON_PERCENT], [NO_ANSWER_CALLBACK], [BUSY_CALLBACK], [NO_ANSWER_RING_LIMIT], [MAXIMUM_LINE_AGENT], [LINES_PER_AGENT], [LEAVE_MESSAGE_ENABLED], [MAXIMUM_ATTEMPTS], [WORK_START_HOURS], [WORK_START_MINUTES], [WORK_END_HOURS], [WORK_END_MINUTES], [HOME_START_HOURS], [HOME_START_MINUTES], [HOME_END_HOURS], [HOME_END_MINUTES], [HOME_ENABLED], [WORK_ENABLED], [BUSY_RETRY_ENABLED], [MAXIMUM_BUSY_ATTEMPTS], [ANSWER_DETECT_ENABLED], [CALLBACK_TIME_LIMIT], [MINIMUM_CALL_DURATION], [EXHAUSTED_CALLS_ENABLED], [EDGE_DETECT_ENABLED], [QUICK_DETECT_ENABLED], [SP_CLOSED_RECORD_ENABLED], [SP_CLOSED_RECORD_COUNT], [PERSONALIZED_CALLBACK_ENABLED], [RELEASE_CALLBACK_ENABLED], [RESCHEDULE_CALLBACK_MODE], [IP_TERMINATING_BEEP_DETECT], [CAMPAIGN_PURPOSE_TYPE], [IPAMD_ENABLED], [AMD_TREATMENT_MODE], [USE_GMT_FROM_REGION_PREFIX], [CONFIG_PARAM], [ABANDON_CUSTOMER_CALLBACK], [ABANDONED_DIALER_CALLBACK], [CUSTOMER_NOT_HOME_CALLBACK], [ANSWERING_MACHINE_CALLBACK], [PREFIX_DIGITS], [WAIT_FOR_BUSY_RETRY], [DISABLE_CPA], [DST_LOCATION], [ICM_ENABLED],                                 [STATUS], [EFFECTIVE_FROM], [EFFECTIVE_TO], [CREATED_BY_ID], [MODIFIED_BY_ID], [MODIFIED_DATE], [CREATION_DATE],
                                [PURGED], [SYSTEM], [LATEST], [SOURCE_CHANGE_STAMP], [XML_DATA], [CUSTOM_XML_DATA]
                                FROM #PKEY_MAP_ROWS;
                           ...
                           BEGIN 
                               ...
                               VALUES('AT_DIMENSION_TYPE_2_PERFORMED', @ItemBusinessUrn, @NewItemUrn, 13, 'DIMENSION_TYPE_2_CHANGE');
                           END;
                           ELSE BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)                
                               VALUES('AT_DIMENSION_TYPE_2_TOMBSTONE_PERFORMED', @ItemBusinessUrn, @ItemUrn, 13, 'DIMENSION_TYPE_2_CHANGE');
                           END;
            
                           IF @MaintainBusinessKey = 0
                           BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)
                               VALUES('AT_DIMENSION_TYPE_2_REVIVAL_PERFORMED', @NewItemBusinessUrn, @ItemUrn, 13, 'DIMENSION_TYPE_2_CHANGE');
            
                               -- Type-2 the correspondiing memberships
                               DECLARE @MemberUrn INT, @MemberBusinessUrn INT, @MemberChangeStamp INT;

                               DECLARE query_rule_campaign_member_cursor CURSOR FOR
                               ...
                               AND MEMBER_BIZ_URN IN (SELECT MEMBER_BIZ_URN FROM dbo.TB_DIM_QUERY_RULE_CAMPAIGN_MEMBER_PKEY_MAP WHERE STATUS IN ('E', 'S', 'P', 'R'));
                               OPEN query_rule_campaign_member_cursor;
                               FETCH NEXT FROM query_rule_campaign_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               WHILE @@FETCH_STATUS = 0
                               BEGIN 
                                   EXEC @ReturnCode = dbo.ap_dim01_do_type2_query_rule_campaign_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @MemberChangeStamp, @EffectiveFrom, NULL, NULL, @MaintainBusinessKey;
                    
                                   IF @ReturnCode <> 0
                                   BEGIN 
                                       RAISERROR ('dbo.ap_dim01_do_type2_query_rule_campaign_member returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                   END;
                    
                                   -- Update the new member to point at the new item
                                   UPDATE dbo.TB_DIM_QUERY_RULE_CAMPAIGN_MEMBER
                                   SET PARENT_ITEM_BIZ_URN = @NewItemUrn 
                                   WHERE MEMBER_URN = @MemberUrn;
                    
                                   FETCH NEXT FROM query_rule_campaign_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               END;
                               CLOSE query_rule_campaign_member_cursor;
                               DEALLOCATE query_rule_campaign_member_cursor;
                           ...
                           BEGIN 
                               COMMIT TRANSACTION PTYP_camTran;
                           END;
                           ELSE BEGIN 
    ...
    ELSE BEGIN 
        ...
        BEGIN 
            ...
            END;
        END;
        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_TYPE_2_PERFORMED', DEFAULT, @ItemBusinessUrn, DEFAULT, 13, DEFAULT, DEFAULT, DEFAULT, 'F', 0, 'DIMENSION_TYPE_2_CHANGE', NULL, DEFAULT, DEFAULT, @AuditUrn out;
        RAISERROR(@ErrorMessage, 16, 1, @ReturnCode) WITH LOG;
    END;
    RETURN @ReturnCode;
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_do_type2_campaign]
	TO [portalapp_role]
GO
