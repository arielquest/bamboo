...
GO
CREATE PROCEDURE [dbo].[ap_dim01_delete_ivr_routing_target] (@SessionId UNIQUEIDENTIFIER, @ItemUrn INT, @ChangeStamp INT OUTPUT)
AS BEGIN 
    SET NOCOUNT ON;
    ...
    BEGIN 
        ...
        FROM dbo.TB_DIM_IVR_ROUTING_TARGET WITH (UPDLOCK)
            ...
            BEGIN
                BEGIN TRANSACTION PDEL_ivrroutingtargeTran;
                    BEGIN TRY;
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_DELETE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 58, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_DELETED', NULL, NULL, NULL, @AuditUrn OUTPUT;
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_delete_ivr_routing_target]
	TO [portalapp_role]
GO
