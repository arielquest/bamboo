...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_scl_adapter]
(
    ...
    @AdapterTypeCodeBizUrn           [INT] = -1, 
    @AdapterTypeSubCodeBizUrn        [INT] = -1, 
...
AS
    ...
    @OldAdapterTypeCodeBizUrn        [INT],
    @OldAdapterTypeSubCodeBizUrn     [INT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_SCL_ADAPTER' AND ITEM_TYPE_ID = '9CE7C8C9-76F1-4FFC-B809-861517F609F2' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_SCL_ADAPTER' AND ITEM_TYPE_ID = '9CE7C8C9-76F1-4FFC-B809-861517F609F2' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldAdapterTypeCodeBizUrn = item.[ADAPTER_TYPE_CODE_BIZ_URN], 
        @OldAdapterTypeSubCodeBizUrn = item.[ADAPTER_TYPE_SUB_CODE_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_SCL_ADAPTER item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'SCL_ADAPTER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldAdapterTypeCodeBizUrn IS NOT NULL AND @AdapterTypeCodeBizUrn IS NOT NULL AND @OldAdapterTypeCodeBizUrn <> @AdapterTypeCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldAdapterTypeCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@AdapterTypeCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SCL_ADAPTER_ADAPTER_TYPE_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldAdapterTypeSubCodeBizUrn IS NOT NULL AND @AdapterTypeSubCodeBizUrn IS NOT NULL AND @OldAdapterTypeSubCodeBizUrn <> @AdapterTypeSubCodeBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldAdapterTypeSubCodeBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@AdapterTypeSubCodeBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SCL_ADAPTER_ADAPTER_TYPE_SUB_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_scladTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_scl_adapter @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_scl_adapter returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_scl_adapter @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_scl_adapter returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[ADAPTER_TYPE_CODE_BIZ_URN] = @AdapterTypeCodeBizUrn        
,[ADAPTER_TYPE_SUB_CODE_BIZ_URN] = @AdapterTypeSubCodeBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 142, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_scladTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_scl_adapter]
	TO [portalapp_role]
GO
