...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_precision_queue_step_precision_attribute_member_pkey]
(
    ...
    @ParenthesesCount        [INT] = NULL,
    @TermRelation            [INT] = NULL,
    @AttributeRelation       [INT] = NULL,
...
BEGIN 
    ...
    DECLARE @ReturnCode                     AS [INTEGER],
            ...
            @OldParenthesesCount             [INT], 
            @OldTermRelation                 [INT], 
            @OldAttributeRelation            [INT], 
            @OldValue1                       [VARCHAR](255), 
            @OldValue2                       [VARCHAR](255), 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_update_precision_queue_step_precision_attribute_member_pkey');
        END;
        ...
        BEGIN    
            ...
            SET @MemberTypeId = '00039612-1000-0000-0000-000000000010';
            SELECT @MemberTypeIsProvisionable = ISNULL((SELECT TOP 1 1 
                                                        FROM dbo.TE_DIM_ITEM_TYPE 
                                                        WHERE ITEM_TYPE_ID IN ('00039602-1000-0000-0000-000000000010', '00039572-1000-0000-0000-000000000010')
            ...
            SELECT  @OldEffectiveFrom = pm.[EFFECTIVE_FROM],
                    ...
                    @OldParenthesesCount = pm.[PARENTHESES_COUNT], 
                    @OldTermRelation = pm.[TERM_RELATION], 
                    @OldAttributeRelation = pm.[ATTRIBUTE_RELATION], 
                    @OldValue1 = pm.[VALUE_1], 
                    @OldValue2 = pm.[VALUE_2], 
            ...
            FROM dbo.TB_DIM_PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_PKEY_MAP AS pm WITH(UPDLOCK)
            INNER JOIN dbo.TB_DIM_PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER AS m WITH(READUNCOMMITTED) ON pm.MEMBER_BIZ_URN = m.MEMBER_BIZ_URN AND m.LATEST = 1
            ...
            BEGIN 
                ...
                WHERE TABLE_NAME = 'TB_DIM_PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_PKEY_MAP' 
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'custom.PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
            SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
            SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
            ...
            IF (@OldParenthesesCount IS NOT NULL AND @ParenthesesCount IS NOT NULL AND @OldParenthesesCount <> @ParenthesesCount) 
 
            BEGIN  
                SET @AuditTempOld = CAST(@OldParenthesesCount AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@ParenthesesCount AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_PARENTHESES_COUNT', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            
 
            IF (@OldTermRelation IS NOT NULL AND @TermRelation IS NOT NULL AND @OldTermRelation <> @TermRelation) 
 
            BEGIN  
                SET @AuditTempOld = CAST(@OldTermRelation AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@TermRelation AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_TERM_RELATION', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            
 
            IF (@OldAttributeRelation IS NOT NULL AND @AttributeRelation IS NOT NULL AND @OldAttributeRelation <> @AttributeRelation) 
 
            BEGIN  
                SET @AuditTempOld = CAST(@OldAttributeRelation AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@AttributeRelation AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_ATTRIBUTE_RELATION', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            
 
            IF (@OldValue1 IS NOT NULL AND @Value1 IS NOT NULL AND @OldValue1 <> @Value1) 
 
            OR (@OldValue1 IS NULL AND @Value1 IS NOT NULL)
            OR (@OldValue1 IS NOT NULL AND @Value1 IS NULL) 
            BEGIN  
                SET @AuditTempOld = CAST(@OldValue1 AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@Value1 AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_VALUE_1', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            
 
            IF (@OldValue2 IS NOT NULL AND @Value2 IS NOT NULL AND @OldValue2 <> @Value2) 
 
            OR (@OldValue2 IS NULL AND @Value2 IS NOT NULL)
            OR (@OldValue2 IS NOT NULL AND @Value2 IS NULL) 
            BEGIN  
                SET @AuditTempOld = CAST(@OldValue2 AS NVARCHAR(250));
                SET @AuditTempNew = CAST(@Value2 AS NVARCHAR(250)); 
                INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                VALUES('PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_VALUE_2', @AuditTempNew, @AuditTempOld); 
                SET @FieldsChanged = 1;
                SET @ProvisionableFieldsChanged= 1; 
            END;
            ...
            BEGIN        
                ...
                BEGIN 
                    RAISERROR('ap_dim01_update_precision_queue_step_precision_attribute_member_pkey: Cannot perform provisionable update on deleted or deleting pkey map [%d]', 16, 1, @PkeyMapUrn) WITH LOG;
                END;
                ...
                BEGIN TRANSACTION PUPD_precisionqueuePKEYTran;       
                -- Update the pkey  record
                UPDATE dbo.TB_DIM_PRECISION_QUEUE_STEP_PRECISION_ATTRIBUTE_MEMBER_PKEY_MAP
                SET [EFFECTIVE_FROM] = @EffectiveFrom,
                    ...
                    [PARENTHESES_COUNT] = @ParenthesesCount, 
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_update_precision_queue_step_precision_attribute_member_pkey: Error updating member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_update_precision_queue_step_precision_attribute_member_pkey: Concurrency conflict occured whilst attempting to update member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ParentItemBusinessUrn, @ChildItemBusinessUrn, 77, 79, @ClusterResourceId, NULL, 'S', 0, 
                    'MEMBER_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
            
                IF @InitialTransactionCount = 0 
                BEGIN 
                    COMMIT TRANSACTION PUPD_precisionqueuePKEYTran;
                END;
                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_precision_queue_step_precision_attribute_member_pkey]
	TO [portalapp_role]
GO
