...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_scl_search]
(
    ...
    @SearchPhrase                    [NVARCHAR](1000) = NULL, 
    @SCLAdapterBizUrn                [INT] = -1, 
...
AS
    ...
    @OldSearchPhrase                 [NVARCHAR](1000),
    @OldSCLAdapterBizUrn             [INT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_SCL_SEARCH' AND ITEM_TYPE_ID = 'B2A289A1-F613-4D0D-8F57-F83E04521155' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_SCL_SEARCH' AND ITEM_TYPE_ID = 'B2A289A1-F613-4D0D-8F57-F83E04521155' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldSearchPhrase = item.[SEARCH_PHRASE], 
        @OldSCLAdapterBizUrn = item.[SCL_ADAPTER_BIZ_URN], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_SCL_SEARCH item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'SCL_SEARCH_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldSearchPhrase IS NOT NULL AND @SearchPhrase IS NOT NULL AND @OldSearchPhrase <> @SearchPhrase) 
        OR (@OldSearchPhrase IS NULL AND @SearchPhrase IS NOT NULL)
        OR (@OldSearchPhrase IS NOT NULL AND @SearchPhrase IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldSearchPhrase AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@SearchPhrase AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SCL_SEARCH_SEARCH_PHRASE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldSCLAdapterBizUrn IS NOT NULL AND @SCLAdapterBizUrn IS NOT NULL AND @OldSCLAdapterBizUrn <> @SCLAdapterBizUrn) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldSCLAdapterBizUrn AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@SCLAdapterBizUrn AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('SCL_SEARCH_SCL_ADAPTER_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_sclsTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_scl_search @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_scl_search returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_scl_search @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_scl_search returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[SCL_ADAPTER_BIZ_URN] = @SCLAdapterBizUrn        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 143, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_sclsTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_scl_search]
	TO [portalapp_role]
GO
