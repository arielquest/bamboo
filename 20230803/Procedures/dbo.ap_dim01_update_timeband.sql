...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_timeband]
(
...
AS
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_TIMEBAND' AND ITEM_TYPE_ID = '00007810-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_TIMEBAND' AND ITEM_TYPE_ID = '00007810-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldPeriodStart = item.[PERIOD_START], 
        @OldPeriodEnd = item.[PERIOD_END], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_TIMEBAND item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'TIMEBAND_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('TIMEBAND_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldPeriodStart IS NOT NULL AND @PeriodStart IS NOT NULL AND @OldPeriodStart <> @PeriodStart) 
        OR (@OldPeriodStart IS NULL AND @PeriodStart IS NOT NULL)
        OR (@OldPeriodStart IS NOT NULL AND @PeriodStart IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldPeriodStart AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@PeriodStart AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('TIMEBAND_PERIOD_START', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldPeriodEnd IS NOT NULL AND @PeriodEnd IS NOT NULL AND @OldPeriodEnd <> @PeriodEnd) 
        OR (@OldPeriodEnd IS NULL AND @PeriodEnd IS NOT NULL)
        OR (@OldPeriodEnd IS NOT NULL AND @PeriodEnd IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldPeriodEnd AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@PeriodEnd AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('TIMEBAND_PERIOD_END', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_timTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_timeband @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_timeband returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_timeband @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_timeband returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[PERIOD_END] = @PeriodEnd        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 98, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_timTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_timeband]
	TO [portalapp_role]
GO
