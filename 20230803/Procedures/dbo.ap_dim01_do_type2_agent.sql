...
GO
CREATE PROCEDURE [dbo].[ap_dim01_do_type2_agent]
    ...
    BEGIN 
        ...
        BEGIN 
            ...
            BEGIN 
                ...
                BEGIN TRANSACTION PTYP_Tran;
                    WHILE @deadlockRetry < 3
                    BEGIN 
                        SAVE TRANSACTION deadlockSavePoint;
                        BEGIN TRY;
                            -- Insert the new type-2 item record
                            INSERT INTO dbo.TB_DIM_AGENT ([ITEM_BIZ_URN], [SKIP_LEVEL], [FOLDER_ID], [OWNER_ID], [ITEM_SUBTYPE_ID], [NAME], [DESCRIPTION],
[DEPARTMENT_BIZ_URN],
[MANAGER_AGENT_BIZ_URN],
[USER_ID],
...
[ENABLED_ASSIST_SERVICE_TYPES], 
                                ...
                                INSERT INTO dbo.TB_DIM_AGENT_PKEY_MAP  ([PKEY_MAP_BIZ_URN], [ITEM_BIZ_URN], [CLUSTER_RESOURCE_ID], [PKEY1], [PKEY2],                            
[INTERNAL_NAME], 
[DESCRIPTION], 
[PERIPHERAL_NUMBER], 
[PERIPHERAL_NAME], 
...
[INTERNAL_NAME], [DESCRIPTION], [PERIPHERAL_NUMBER], [PERIPHERAL_NAME], [SUPERVISOR], [AGENT_STATE_TRACE], [DOMAIN_NAME], [DOMAIN_LOGIN_NAME], [DOMAIN_USER_NAME], [DOMAIN_USER_GUID], [USER_DELETABLE], [USER_GROUP_ID], [ENABLED_ASSIST_SERVICE_TYPES],                                 [STATUS], [EFFECTIVE_FROM], [EFFECTIVE_TO], [CREATED_BY_ID], [MODIFIED_BY_ID], [MODIFIED_DATE], [CREATION_DATE],
                                [PURGED], [SYSTEM], [LATEST], [SOURCE_CHANGE_STAMP], [XML_DATA], [CUSTOM_XML_DATA]
                                FROM #PKEY_MAP_ROWS;
                           ...
                           BEGIN 
                               ...
                               VALUES('AT_DIMENSION_TYPE_2_PERFORMED', @ItemBusinessUrn, @NewItemUrn, 1, 'DIMENSION_TYPE_2_CHANGE');
                           END;
                           ELSE BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)                
                               VALUES('AT_DIMENSION_TYPE_2_TOMBSTONE_PERFORMED', @ItemBusinessUrn, @ItemUrn, 1, 'DIMENSION_TYPE_2_CHANGE');
                           END;
            
                           IF @MaintainBusinessKey = 0
                           BEGIN 
                               INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, EVENT_DESCRIPTION)
                               VALUES('AT_DIMENSION_TYPE_2_REVIVAL_PERFORMED', @NewItemBusinessUrn, @ItemUrn, 1, 'DIMENSION_TYPE_2_CHANGE');
            
                               -- Type-2 the correspondiing memberships
                               DECLARE @MemberUrn INT, @MemberBusinessUrn INT, @MemberChangeStamp INT;

                               DECLARE agent_agent_desktop_member_cursor CURSOR FOR
                               ...
                               AND MEMBER_BIZ_URN IN (SELECT MEMBER_BIZ_URN FROM dbo.TB_DIM_AGENT_AGENT_DESKTOP_MEMBER_PKEY_MAP WHERE STATUS IN ('E', 'S', 'P', 'R'));
                               OPEN agent_agent_desktop_member_cursor;
                               FETCH NEXT FROM agent_agent_desktop_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               WHILE @@FETCH_STATUS = 0
                               BEGIN 
                                   EXEC @ReturnCode = dbo.ap_dim01_do_type2_agent_agent_desktop_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @MemberChangeStamp, @EffectiveFrom, NULL, NULL, @MaintainBusinessKey;
                    
                                   IF @ReturnCode <> 0
                                   BEGIN 
                                       RAISERROR ('dbo.ap_dim01_do_type2_agent_agent_desktop_member returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                   END;
                    
                                   -- Update the new member to point at the new item
                                   UPDATE dbo.TB_DIM_AGENT_AGENT_DESKTOP_MEMBER
                                   SET CHILD_ITEM_BIZ_URN = @NewItemUrn 
                                   WHERE MEMBER_URN = @MemberUrn;
                    
                                   FETCH NEXT FROM agent_agent_desktop_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               END;
                               CLOSE agent_agent_desktop_member_cursor;
                               DEALLOCATE agent_agent_desktop_member_cursor;
                               DECLARE agent_agent_team_member_cursor CURSOR FOR
                               SELECT MEMBER_URN, MEMBER_BIZ_URN, CHANGE_STAMP
                               FROM dbo.TB_DIM_AGENT_AGENT_TEAM_MEMBER        
                               WHERE CHILD_ITEM_BIZ_URN = @ItemBusinessUrn 
                               AND MEMBER_BIZ_URN IN (SELECT MEMBER_BIZ_URN FROM dbo.TB_DIM_AGENT_AGENT_TEAM_MEMBER_PKEY_MAP WHERE STATUS IN ('E', 'S', 'P', 'R'));
                               OPEN agent_agent_team_member_cursor;
                               FETCH NEXT FROM agent_agent_team_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               WHILE @@FETCH_STATUS = 0
                               BEGIN 
                                   EXEC @ReturnCode = dbo.ap_dim01_do_type2_agent_agent_team_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @MemberChangeStamp, @EffectiveFrom, NULL, NULL, @MaintainBusinessKey;
                    
                                   IF @ReturnCode <> 0
                                   BEGIN 
                                       RAISERROR ('dbo.ap_dim01_do_type2_agent_agent_team_member returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                   END;
                    
                                   -- Update the new member to point at the new item
                                   UPDATE dbo.TB_DIM_AGENT_AGENT_TEAM_MEMBER
                                   SET CHILD_ITEM_BIZ_URN = @NewItemUrn 
                                   WHERE MEMBER_URN = @MemberUrn;
                    
                                   FETCH NEXT FROM agent_agent_team_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               END;
                               CLOSE agent_agent_team_member_cursor;
                               DEALLOCATE agent_agent_team_member_cursor;
                               ...
                               DECLARE agent_person_member_cursor CURSOR FOR
                               SELECT MEMBER_URN, MEMBER_BIZ_URN, CHANGE_STAMP
                               FROM dbo.TB_DIM_AGENT_PERSON_MEMBER        
                               WHERE CHILD_ITEM_BIZ_URN = @ItemBusinessUrn 
                               AND MEMBER_BIZ_URN IN (SELECT MEMBER_BIZ_URN FROM dbo.TB_DIM_AGENT_PERSON_MEMBER_PKEY_MAP WHERE STATUS IN ('E', 'S', 'P', 'R'));
                               OPEN agent_person_member_cursor;
                               FETCH NEXT FROM agent_person_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               WHILE @@FETCH_STATUS = 0
                               BEGIN 
                                   EXEC @ReturnCode = dbo.ap_dim01_do_type2_agent_person_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @MemberChangeStamp, @EffectiveFrom, NULL, NULL, @MaintainBusinessKey;
                    
                                   IF @ReturnCode <> 0
                                   BEGIN 
                                       RAISERROR ('dbo.ap_dim01_do_type2_agent_person_member returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                   END;
                                   ...
                                   FETCH NEXT FROM agent_person_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               END;
                               CLOSE agent_person_member_cursor;
                               DEALLOCATE agent_person_member_cursor;
                               DECLARE agent_precision_attribute_member_cursor CURSOR FOR
                               SELECT MEMBER_URN, MEMBER_BIZ_URN, CHANGE_STAMP
                               FROM dbo.TB_DIM_AGENT_PRECISION_ATTRIBUTE_MEMBER        
                               WHERE CHILD_ITEM_BIZ_URN = @ItemBusinessUrn 
                               AND MEMBER_BIZ_URN IN (SELECT MEMBER_BIZ_URN FROM dbo.TB_DIM_AGENT_PRECISION_ATTRIBUTE_MEMBER_PKEY_MAP WHERE STATUS IN ('E', 'S', 'P', 'R'));
                               OPEN agent_precision_attribute_member_cursor;
                               FETCH NEXT FROM agent_precision_attribute_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               WHILE @@FETCH_STATUS = 0
                               BEGIN 
                                   EXEC @ReturnCode = dbo.ap_dim01_do_type2_agent_precision_attribute_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @MemberChangeStamp, @EffectiveFrom, NULL, NULL, @MaintainBusinessKey;
                    
                                   IF @ReturnCode <> 0
                                   BEGIN 
                                       RAISERROR ('dbo.ap_dim01_do_type2_agent_precision_attribute_member returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                   END;
                    
                                   -- Update the new member to point at the new item
                                   UPDATE dbo.TB_DIM_AGENT_PRECISION_ATTRIBUTE_MEMBER
                                   SET CHILD_ITEM_BIZ_URN = @NewItemUrn 
                                   WHERE MEMBER_URN = @MemberUrn;
                    
                                   FETCH NEXT FROM agent_precision_attribute_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               END;
                               CLOSE agent_precision_attribute_member_cursor;
                               DEALLOCATE agent_precision_attribute_member_cursor;
                               ...
                               DECLARE agent_user_group_member_cursor CURSOR FOR
                               SELECT MEMBER_URN, MEMBER_BIZ_URN, CHANGE_STAMP
                               FROM dbo.TB_DIM_AGENT_USER_GROUP_MEMBER        
                               WHERE CHILD_ITEM_BIZ_URN = @ItemBusinessUrn 
                               AND MEMBER_BIZ_URN IN (SELECT MEMBER_BIZ_URN FROM dbo.TB_DIM_AGENT_USER_GROUP_MEMBER_PKEY_MAP WHERE STATUS IN ('E', 'S', 'P', 'R'));
                               OPEN agent_user_group_member_cursor;
                               FETCH NEXT FROM agent_user_group_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               WHILE @@FETCH_STATUS = 0
                               BEGIN 
                                   EXEC @ReturnCode = dbo.ap_dim01_do_type2_agent_user_group_member @SessionId, @MemberUrn OUT, @MemberBusinessUrn OUT, @MemberChangeStamp, @EffectiveFrom, NULL, NULL, @MaintainBusinessKey;
                    
                                   IF @ReturnCode <> 0
                                   BEGIN 
                                       RAISERROR ('dbo.ap_dim01_do_type2_agent_user_group_member returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                   END;
                    
                                   -- Update the new member to point at the new item
                                   UPDATE dbo.TB_DIM_AGENT_USER_GROUP_MEMBER
                                   SET CHILD_ITEM_BIZ_URN = @NewItemUrn 
                                   WHERE MEMBER_URN = @MemberUrn;
                    
                                   FETCH NEXT FROM agent_user_group_member_cursor INTO @MemberUrn, @MemberBusinessUrn, @MemberChangeStamp;
                               END;
                               CLOSE agent_user_group_member_cursor;
                               DEALLOCATE agent_user_group_member_cursor;
                           END;
    ...
    ELSE BEGIN 
        ...
        BEGIN 
            ...
            END;
        END;
        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_TYPE_2_PERFORMED', DEFAULT, @ItemBusinessUrn, DEFAULT, 1, DEFAULT, DEFAULT, DEFAULT, 'F', 0, 'DIMENSION_TYPE_2_CHANGE', NULL, DEFAULT, DEFAULT, @AuditUrn out;
        RAISERROR(@ErrorMessage, 16, 1, @ReturnCode) WITH LOG;
    END;
    RETURN @ReturnCode;
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_do_type2_agent]
	TO [portalapp_role]
GO
