...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_ivr_entry_point_ivr_script_member_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_add_ivr_entry_point_ivr_script_member_pkey');
        END;
        ...
        BEGIN    
            IF NOT EXISTS(SELECT 1 
                          FROM dbo.TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER WITH(READUNCOMMITTED) 
                          WHERE MEMBER_BIZ_URN = @MemberBusinessUrn
                          )
            BEGIN 
                SET @ReturnCode = -1;
                RAISERROR('ap_dim01_add_ivr_entry_point_ivr_script_member_pkey: The specified input business urn [%d] does not exist.', 16, 1, @MemberBusinessUrn);
            END;  
            ...
            IF EXISTS(SELECT 1 
                      FROM [dbo].[TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER_PKEY_MAP] WITH(READUNCOMMITTED) 
                      WHERE MEMBER_BIZ_URN = @MemberBusinessUrn AND CLUSTER_RESOURCE_ID = @ClusterResourceId AND STATUS <> 'D'
                      )
            BEGIN 
                SET @ReturnCode = -1;
                RAISERROR('ap_dim01_add_ivr_entry_point_ivr_script_member_pkey: Effective mapping already exists for member [%d] on cluster [%s]', 16, 1, @MemberBusinessUrn, @ClusterResourceName) WITH LOG;
            END;
            ...
            FROM dbo.TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER AS m WITH(READUNCOMMITTED)
            ...
            BEGIN TRANSACTION PADD_ivrentrypointiPKEYTran;
       
            INSERT INTO dbo.TB_DIM_IVR_ENTRY_POINT_IVR_SCRIPT_MEMBER_PKEY_MAP(MEMBER_BIZ_URN, 
                                                                              CLUSTER_RESOURCE_ID, 
                                                                              STATUS, 
                                                                              EFFECTIVE_FROM, 
                                                                              EFFECTIVE_TO, 
            ...
            IF @ReturnCode <> 0
            BEGIN 
                RAISERROR('ap_dim01_add_ivr_entry_point_ivr_script_member_pkey: Error linking member [%d] to cluster resource [%s], Error [%d] ', 16, 1, @MemberBusinessUrn, @ClusterResourceName, @ReturnCode) WITH LOG;
            END;
            ...
            BEGIN 
               COMMIT TRANSACTION PADD_ivrentrypointiPKEYTran;
            END;
            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_ivr_entry_point_ivr_script_member_pkey]
	TO [portalapp_role]
GO
