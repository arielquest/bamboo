...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_sub_code_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode AS [INTEGER] = 0,
                @Status AS [CHAR](1),
                @ItemTypeId AS [UNIQUEIDENTIFIER] = '24fb83b4-a581-4642-a847-0ab90f78be38',
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                IF @ItemBusinessUrn IS NOT NULL
                BEGIN 
                    IF NOT EXISTS(SELECT 1 
                                  FROM dbo.TB_DIM_SUB_CODE WITH(READUNCOMMITTED) 
                    ...
                    ELSE BEGIN 
                        ...
                        IF @ClusterResourceId IS NOT NULL
                        BEGIN
                        ...
                        IF EXISTS(SELECT 1 
                                  FROM [dbo].[TB_DIM_SUB_CODE_PKEY_MAP] WITH(READUNCOMMITTED) 
                        ...
                        ELSE BEGIN 
                            ...
                            BEGIN TRANSACTION PADD_suPKEYTran;
                            BEGIN TRY;
                                INSERT INTO [dbo].[TB_DIM_SUB_CODE_PKEY_MAP](
                                ITEM_BIZ_URN, 
                                CLUSTER_RESOURCE_ID, 
                                STATUS, 
                                EFFECTIVE_FROM, 
                            ...
                            -- Audit success
                            EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, DEFAULT, 117, DEFAULT, @ClusterResourceId, DEFAULT, 'S', 0, 'DIMENSION_ADDED_ON_EQUIPMENT', NULL, NULL, NULL, @AuditUrn OUTPUT;
                
                            IF @InitialTransactionCount = 0 
                            BEGIN 
                                COMMIT TRANSACTION PADD_suPKEYTran;
                            END;
                            ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_sub_code_pkey]
	TO [portalapp_role]
GO
