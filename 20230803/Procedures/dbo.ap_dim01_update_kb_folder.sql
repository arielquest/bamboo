...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_kb_folder]
(
    ...
    @KB_FOLDER_TYPE_CODE_BIZ_URN     [INT] = -1, 
    @KBFolderType                    [SMALLINT] = 0, 
...
AS
    ...
    @OldKB_FOLDER_TYPE_CODE_BIZ_URN  [INT],
    @OldKBFolderType                 [SMALLINT],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_KB_FOLDER' AND ITEM_TYPE_ID = '1EFC44CF-D313-4BF5-9BE7-A4A893D68D2C' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_KB_FOLDER' AND ITEM_TYPE_ID = '1EFC44CF-D313-4BF5-9BE7-A4A893D68D2C' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldKB_FOLDER_TYPE_CODE_BIZ_URN = item.[KB_FOLDER_TYPE_CODE_BIZ_URN], 
        @OldKBFolderType = item.[KB_FOLDER_TYPE], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_KB_FOLDER item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'KB_FOLDER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        IF (@OldKB_FOLDER_TYPE_CODE_BIZ_URN IS NOT NULL AND @KB_FOLDER_TYPE_CODE_BIZ_URN IS NOT NULL AND @OldKB_FOLDER_TYPE_CODE_BIZ_URN <> @KB_FOLDER_TYPE_CODE_BIZ_URN) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldKB_FOLDER_TYPE_CODE_BIZ_URN AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@KB_FOLDER_TYPE_CODE_BIZ_URN AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('KB_FOLDER_KB_FOLDER_TYPE_CODE_BIZ_URN', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldKBFolderType IS NOT NULL AND @KBFolderType IS NOT NULL AND @OldKBFolderType <> @KBFolderType) 
        BEGIN      
            SET @AuditTempOld = CAST(@OldKBFolderType AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@KBFolderType AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('KB_FOLDER_KB_FOLDER_TYPE', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
        ...
        BEGIN 
            BEGIN TRANSACTION PUPD_kbfTran;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_kb_folder @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_kb_folder returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_kb_folder @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_kb_folder returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[KB_FOLDER_TYPE_CODE_BIZ_URN] = @KB_FOLDER_TYPE_CODE_BIZ_URN        
,[KB_FOLDER_TYPE] = @KBFolderType        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 124, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPD_kbfTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_kb_folder]
	TO [portalapp_role]
GO
