...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_date_time_setting]
(
...
BEGIN 
    ...
    BEGIN 
        ...
        BEGIN
            ...
            BEGIN TRANSACTION PADD_datetimeseTran;
                WHILE @deadlockRetry < 3
                BEGIN 
                    SAVE TRANSACTION deadlockSavePoint;
                    BEGIN TRY;
                        -- Add the base data
                        INSERT INTO dbo.TB_DIM_DATE_TIME_SETTING
                        (
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_INSERT_COMMITTED', DEFAULT, @ItemUrn, DEFAULT, 31, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PADD_datetimeseTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_date_time_setting]
	TO [portalapp_role]
GO
