...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_route_enterprise_route_member_pkey]
(
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        BEGIN 
            ...
            RAISERROR(50157, 16, 5, 'ap_dim01_update_route_enterprise_route_member_pkey');
        END;
        ...
        BEGIN    
            ...
            SET @MemberTypeId = '0000000a-1000-0000-0000-0000000000c0';
            ...
            INNER JOIN dbo.TB_DIM_ROUTE_ENTERPRISE_ROUTE_MEMBER AS m WITH(READUNCOMMITTED) ON pm.MEMBER_BIZ_URN = m.MEMBER_BIZ_URN AND m.LATEST = 1
            ...
            BEGIN 
                ...
                WHERE TABLE_NAME = 'TB_DIM_ROUTE_ENTERPRISE_ROUTE_MEMBER_PKEY_MAP' 
            ...
            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            SELECT 
            'ROUTE_ENTERPRISE_ROUTE_MEMBER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
            ...
            BEGIN        
                ...
                BEGIN 
                    RAISERROR('ap_dim01_update_route_enterprise_route_member_pkey: Cannot perform provisionable update on deleted or deleting pkey map [%d]', 16, 1, @PkeyMapUrn) WITH LOG;
                END;
                ...
                BEGIN TRANSACTION PUPD_routeenterprisPKEYTran;       
                ...
                IF @ReturnCode <> 0
                BEGIN 
                    RAISERROR('ap_dim01_update_route_enterprise_route_member_pkey: Error updating member pkey map [%d], Error [%d] ', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    SET @ReturnCode = -2;
                    RAISERROR('ap_dim01_update_route_enterprise_route_member_pkey: Concurrency conflict occured whilst attempting to update member pkey map [%d], Error [%d]', 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                END;
                ...
                BEGIN 
                    COMMIT TRANSACTION PUPD_routeenterprisPKEYTran;
                END;
                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_route_enterprise_route_member_pkey]
	TO [portalapp_role]
GO
