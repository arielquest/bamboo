...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_mediafile_pkey]
(    
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldSize                         [INT], 
                @OldChecksum                     [NVARCHAR](100), 
                @OldPath                         [NVARCHAR](100), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_dim01_update_mediafile_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        ...
                        SET @ItemTypeId = 'd500e079-8a45-44b3-8068-f5dfe64297d5';
                        ...
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @OldFilename = map.[FILENAME],
                                @OldSize = map.[SIZE],
                                @OldChecksum = map.[CHECKSUM],
                                @OldPath = map.[PATH], 
                        ...
                        FROM dbo.TB_DIM_MEDIAFILE_PKEY_MAP AS map WITH(UPDLOCK)
                        INNER JOIN dbo.TB_DIM_MEDIAFILE AS item WITH(READUNCOMMITTED) ON map.ITEM_BIZ_URN = item.ITEM_BIZ_URN AND item.LATEST = 1
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_DIM_MEDIAFILE_PKEY_MAP' 
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'MEDIAFILE_' + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],
                        ...
                        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                        SELECT 
                        'custom.MEDIAFILE_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                        SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                        SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        IF (@OldFilename IS NOT NULL AND @Filename IS NOT NULL AND @OldFilename COLLATE Latin1_General_CS_AS <> @Filename COLLATE Latin1_General_CS_AS)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldFilename AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Filename AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIAFILE_FILENAME', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldSize IS NOT NULL AND @Size IS NOT NULL AND @OldSize <> @Size)  
                        OR (@OldSize IS NULL AND @Size IS NOT NULL)
                        OR (@OldSize IS NOT NULL AND @Size IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldSize AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Size AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIAFILE_SIZE', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldChecksum IS NOT NULL AND @Checksum IS NOT NULL AND @OldChecksum COLLATE Latin1_General_CS_AS <> @Checksum COLLATE Latin1_General_CS_AS)  
                        OR (@OldChecksum IS NULL AND @Checksum IS NOT NULL)
                        OR (@OldChecksum IS NOT NULL AND @Checksum IS NULL) 
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldChecksum AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Checksum AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIAFILE_CHECKSUM', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (@OldPath IS NOT NULL AND @Path IS NOT NULL AND @OldPath COLLATE Latin1_General_CS_AS <> @Path COLLATE Latin1_General_CS_AS)  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldPath AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Path AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('MEDIAFILE_PATH', @AuditTempNew, @AuditTempOld);
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            BEGIN TRANSACTION PUPD_mediPKEYTran;
                            BEGIN TRY;
                                -- Update the pkey record
                                UPDATE dbo.TB_DIM_MEDIAFILE_PKEY_MAP
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, @ItemBusinessUrn, NULL, 108, DEFAULT, @ClusterResourceId, NULL, 'S', 0, 'DIMENSION_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_mediPKEYTran;
                                END;
                                ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_update_mediafile_pkey]
	TO [portalapp_role]
GO
