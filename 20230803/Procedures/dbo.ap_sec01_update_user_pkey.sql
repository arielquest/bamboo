...
GO
CREATE PROCEDURE [dbo].[ap_sec01_update_user_pkey]
(    
...
BEGIN 
    ...
    BEGIN TRY;
        ...
        DECLARE @ReturnCode                             AS [INTEGER] = 0,
                ...
                @OldInternalName                        AS [NVARCHAR](100), 
                @OldDescription                            AS [NVARCHAR](500), 
                @OldRole                                AS [NVARCHAR](50), 
        ...
        ELSE BEGIN 
            ...
            ELSE BEGIN 
                ...
                BEGIN 
                    ...
                    RAISERROR('ap_sec01_update_user_pkey: Cannot force status change on non provisionable update', 16, 1) WITH LOG
                END;
                ELSE BEGIN 
                    ...
                    BEGIN 
                        SELECT  @OldEffectiveFrom = map.[EFFECTIVE_FROM],
                                ...
                                @ItemTypeId = 'EEE2E0E2-C672-4F3C-A84F-59800D8565E4'
                        FROM dbo.TB_SEC_USER_PKEY_MAP AS map WITH(UPDLOCK)
                        ...
                        BEGIN 
                            ...
                            FROM (SELECT 
                                  ...
                                  WHERE TABLE_NAME = 'TB_SEC_USER_PKEY_MAP' 
                        ...
                        IF (@OldXmlData IS NULL AND @XmlData IS NOT NULL)
                        OR (@OldXmlData IS NOT NULL AND @XmlData IS NULL)
                        BEGIN 
                            SET @FieldsChanged = 1;
                            SET @ProvisionableFieldsChanged = 1;
                        END;
    
                        IF (@XmlData IS NOT NULL AND @OldXmlData IS NOT NULL)        
                        BEGIN 
                            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            SELECT 
                            'USER_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],
                            ...
                            INNER JOIN @XmlData.nodes('Columns/Column') AS NewAttributes(Attribute)
                        ...
                        IF (@OldCustomXmlData IS NULL AND @CustomXmlData IS NOT NULL)
                        OR (@OldCustomXmlData IS NOT NULL AND @CustomXmlData IS NULL)
                        BEGIN 
                            SET @FieldsChanged = 1;
                        END;

                        IF (@CustomXmlData IS NOT NULL AND @OldCustomXmlData IS NOT NULL)        
                        BEGIN 
                            INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            SELECT 
                            'custom.USER_'  + CASE WHEN OldAttributes.Attribute IS NULL THEN NewAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') ELSE OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') END AS [ATTRIBUTE_FIELD],        
                            SUBSTRING(CAST(NewAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [NEW_VALUE],
                            SUBSTRING(CAST(OldAttributes.Attribute.query('text()') AS NVARCHAR(MAX)), 1, 500) AS [OLD_VALUE]
                        ...
                        IF (ISNULL(@OldInternalName, '12345678-9012-3456-7890-123456789012') <> ISNULL(@InternalName, '12345678-9012-3456-7890-123456789012'))  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldInternalName AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@InternalName AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
                        ...
                        END; 
                        IF (ISNULL(@OldDescription, '12345678-9012-3456-7890-123456789012') <> ISNULL(@Description, '12345678-9012-3456-7890-123456789012'))  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldDescription AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Description AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('DESCRIPTION', @AuditTempNew, @AuditTempOld);
                        ...
                        END;             
                        IF (ISNULL(@OldRole, '12345678-9012-3456-7890-123456789012') <> ISNULL(@Role, '12345678-9012-3456-7890-123456789012'))  
                        BEGIN  
                            SET @AuditTempOld = CAST(@OldRole AS NVARCHAR(250));
                            SET @AuditTempNew = CAST(@Role AS NVARCHAR(250)); 

                            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
                            VALUES('ROLE', @AuditTempNew, @AuditTempOld); 
                    ...
                    BEGIN 
                        ...
                        ELSE BEGIN      
                            ...
                            SET @AuditTypeName = CASE WHEN @Status = 'S' THEN 'AT_USER_UPDATE_REQUESTED_ON_EQUIPMENT' 
                                                                         ELSE 'AT_USER_UPDATE_COMMITTED_ON_EQUIPMENT' END;

                            BEGIN TRANSACTION PUPD_agenPKEYTran;
                            BEGIN TRY;
                                ...
                                BEGIN 
                                    SET @ReturnCode = -2;
                                    SET @ErrorMessage = OBJECT_NAME(@@PROCID)+': Concurrency conflict occured whilst attempting to update user pkey map [%d], Error [%d]';
                                ...
                                SET @FieldChangeXmlData = (SELECT ATTRIBUTE_FIELD, OLD_VALUE, NEW_VALUE 
                                                           FROM @AuditAttributeTable FOR XML RAW, TYPE).query(
                                                                '<FieldChanges>{
                                                                                for $node in /row
                                                                                   return 
                                ...
                                EXEC dbo.ap_adm01_audit_event @SessionId, @AuditTypeName, DEFAULT, DEFAULT, DEFAULT, DEFAULT, DEFAULT, @ClusterResourceId, @UserId, 'S', 0, 'USER_UPDATED_ON_EQUIPMENT', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
                                
                                IF @InitialTransactionCount = 0 
                                BEGIN 
                                    COMMIT TRANSACTION PUPD_agenPKEYTran;
                                END;
                                ELSE BEGIN 
                            ...
                            BEGIN CATCH;
                                ...
                                SELECT @ErrorMessage = OBJECT_NAME(@@PROCID)+': Error updating user pkey map [%d]. '+
                                                      'SQLError['+CASE WHEN @ErrorMessage IS NULL OR LEN(@ErrorMessage) < 1 THEN @ErrorHolder ELSE @ErrorMessage END+']. '+
                                                      'ErrorCode [%d]';
                                RAISERROR(@ErrorMessage, 16, 1, @PkeyMapUrn, @ReturnCode) WITH LOG;
                            END CATCH;
                        END;
    ...
    END TRY
    BEGIN CATCH;
        SET @ErrorMessage = OBJECT_NAME(@@PROCID)+': '+ERROR_MESSAGE();
...
GO
GRANT EXECUTE
	ON [dbo].[ap_sec01_update_user_pkey]
	TO [portalapp_role]
GO
