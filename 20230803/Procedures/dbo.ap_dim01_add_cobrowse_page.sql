...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_cobrowse_page]
(
...
BEGIN 
    ...
    BEGIN 
        ...
        BEGIN
            ...
            BEGIN TRANSACTION PADD_cobrowsTran;
                WHILE @deadlockRetry < 3
                BEGIN 
                    SAVE TRANSACTION deadlockSavePoint;
                    BEGIN TRY;
                        -- Add the base data
                        INSERT INTO dbo.TB_DIM_COBROWSE_PAGE
                        (
                        ...
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_INSERT_COMMITTED', DEFAULT, @ItemUrn, DEFAULT, 161, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PADD_cobrowsTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_cobrowse_page]
	TO [portalapp_role]
GO
