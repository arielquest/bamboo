...
GO
CREATE PROCEDURE [dbo].[ap_dim01_do_type2_agent_user_group_member]
...
AS
    ...
    BEGIN 
        ...
        BEGIN 
            ...
            BEGIN TRANSACTION PTYP_agentusergroupmTran;
                WHILE @deadlockRetry < 3
                BEGIN 
                    SAVE TRANSACTION deadlockSavePoint;
                    BEGIN TRY;
                        -- Copy base data
                        INSERT INTO dbo.TB_DIM_AGENT_USER_GROUP_MEMBER (MEMBER_BIZ_URN, PARENT_ITEM_BIZ_URN, CHILD_ITEM_BIZ_URN, 
                        ...
                        BEGIN 
                            ...
                            INTO #PKEY_MAP_ROWS
                            FROM dbo.TB_DIM_AGENT_USER_GROUP_MEMBER_PKEY_MAP
                            ...
                            INSERT INTO dbo.TB_DIM_AGENT_USER_GROUP_MEMBER_PKEY_MAP (PKEY_MAP_BIZ_URN, MEMBER_BIZ_URN, CLUSTER_RESOURCE_ID, CHANGE_STAMP, 
                            ...
                            BEGIN 
                                ...
                                SELECT 'AT_MEMBER_DELETE_COMMITTED_ON_EQUIPMENT', @OldParentItemBusinessUrn, @OldChildItemBusinessUrn, 114, 1,
                        ...
                        BEGIN 
                            ...
                            VALUES('AT_MEMBER_DELETE_COMMITTED', @OldParentItemBusinessUrn, @OldChildItemBusinessUrn,  114, 1, 'MEMBER_DELETED');

                            -- Audit add of new membership
                            INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, INT_VAL_4, EVENT_DESCRIPTION)
                            VALUES('AT_MEMBER_INSERT_COMMITTED', @NewParentItemBusinessUrn, @NewChildItemBusinessUrn,  114, 1, 'MEMBER_ADDED');

                            IF @MaintainBusinessKey = 0
                            BEGIN 
                                -- Audit insert of pkey entries against new parent/child
                                INSERT INTO @AuditTable(AUDIT_TYPE_INTERNAL_NAME, INT_VAL_1, INT_VAL_2, INT_VAL_3, INT_VAL_4, CLUSTER_RESOURCE_ID, EVENT_DESCRIPTION)
                                SELECT 'AT_MEMBER_INSERT_COMMITTED_ON_EQUIPMENT', @NewParentItemBusinessUrn, @NewChildItemBusinessUrn,  114, 1,
                        ...
                        BEGIN 
                            COMMIT TRANSACTION PTYP_agentusergroupmTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_do_type2_agent_user_group_member]
	TO [portalapp_role]
GO
