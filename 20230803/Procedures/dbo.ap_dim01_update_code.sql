...
GO
CREATE PROCEDURE [dbo].[ap_dim01_update_code]
(
...
AS
    ...
    @OldStringVal                    [NVARCHAR](4000),
    @OldIntVal                       [INT],
    @OldBigintVal                    [BIGINT],
    @OldDatetimeVal                  [DATETIME],
    ...
    OUTER APPLY (SELECT TOP 1 
                 ...
                 WHERE TABLE_NAME = 'TB_DIM_CODE' AND ITEM_TYPE_ID = '00002110-1000-0000-0000-000000000010' AND CUSTOM = 0
                 ORDER BY [XML_DEFAULTS_URN]) b
    OUTER APPLY (SELECT TOP 1 
                 DEFAULT_XML 
                 FROM dbo.TB_ADM_XML_DEFAULTS 
                 WHERE TABLE_NAME = 'TB_DIM_CODE' AND ITEM_TYPE_ID = '00002110-1000-0000-0000-000000000010' AND CUSTOM = 1
    ...
    BEGIN 
        ...
        @OldStringVal = item.[STRING_VAL], 
        @OldIntVal = item.[INT_VAL], 
        @OldBigintVal = item.[BIGINT_VAL], 
        @OldDatetimeVal = item.[DATETIME_VAL], 
        @OldFolderId = item.[FOLDER_ID]
        FROM dbo.TB_DIM_CODE item WITH(UPDLOCK)
        ...
        INSERT INTO @AuditAttributeTable(ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
        SELECT 
        'CODE_' + OldAttributes.Attribute.value('string(@Name)', 'NVARCHAR(250)') AS [ATTRIBUTE_FIELD],        
        ...
        BEGIN      
            ...
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_INTERNAL_NAME', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldStringVal IS NOT NULL AND @StringVal IS NOT NULL AND @OldStringVal <> @StringVal) 
        OR (@OldStringVal IS NULL AND @StringVal IS NOT NULL)
        OR (@OldStringVal IS NOT NULL AND @StringVal IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldStringVal AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@StringVal AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_STRING_VAL', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldIntVal IS NOT NULL AND @IntVal IS NOT NULL AND @OldIntVal <> @IntVal) 
        OR (@OldIntVal IS NULL AND @IntVal IS NOT NULL)
        OR (@OldIntVal IS NOT NULL AND @IntVal IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldIntVal AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@IntVal AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_INT_VAL', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldBigintVal IS NOT NULL AND @BigintVal IS NOT NULL AND @OldBigintVal <> @BigintVal) 
        OR (@OldBigintVal IS NULL AND @BigintVal IS NOT NULL)
        OR (@OldBigintVal IS NOT NULL AND @BigintVal IS NULL)
        BEGIN      
            SET @AuditTempOld = CAST(@OldBigintVal AS NVARCHAR(250));
            SET @AuditTempNew = CAST(@BigintVal AS NVARCHAR(250));
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_BIGINT_VAL', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
            
        IF (@OldDatetimeVal IS NOT NULL AND @DatetimeVal IS NOT NULL AND @OldDatetimeVal <> @DatetimeVal) 
        OR (@OldDatetimeVal IS NULL AND @DatetimeVal IS NOT NULL)
        OR (@OldDatetimeVal IS NOT NULL AND @DatetimeVal IS NULL)
        BEGIN      
            SET @AuditTempOld = dbo.fn_adm_datetime_to_string(@OldDatetimeVal);
            SET @AuditTempNew = dbo.fn_adm_datetime_to_string(@DatetimeVal);
            INSERT INTO @AuditAttributeTable (ATTRIBUTE_FIELD, NEW_VALUE, OLD_VALUE)
            VALUES('CODE_DATETIME_VAL', @AuditTempNew, @AuditTempOld);
            SET @FieldChanged = 1;
            SET @ProvisionableFieldChanged= 1;
        END;
                ...
                BEGIN 
                    ...
                    BEGIN TRY;
                        ...
                        BEGIN 
                            IF @DoOwnershipType2 = 1
                            BEGIN 
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_code @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, 1, 0;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_code returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;
                            END;
                            ELSE BEGIN               
                                EXEC @ReturnCode = dbo.ap_dim01_do_type2_code @SessionId, @ItemUrn OUT, @ItemBusinessUrn OUT, @ChangeStamp, DEFAULT, @IgnoreConsolidationPeriod, 1;
                                IF @ReturnCode <> 0
                                BEGIN 
                                    RAISERROR ('ap_dim01_do_type2_code returned [%d] in TRY block.', 16, 1, @ReturnCode);
                                END;       
...
,[DATETIME_VAL] = @DatetimeVal        
                        ...
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_UPDATE_COMMITTED', DEFAULT, @ItemBusinessUrn, DEFAULT, 17, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_UPDATED', NULL, @FieldChangeXmlData, NULL, @AuditUrn OUTPUT;
    
                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PUPDTran;
                        END;
                        ELSE BEGIN 
