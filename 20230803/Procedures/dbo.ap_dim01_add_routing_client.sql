...
GO
CREATE PROCEDURE [dbo].[ap_dim01_add_routing_client]
(
...
BEGIN 
    ...
    BEGIN 
        ...
        BEGIN
            ...
            BEGIN TRANSACTION PADD_routingcTran;
                WHILE @deadlockRetry < 3
                BEGIN 
                    SAVE TRANSACTION deadlockSavePoint;
                    BEGIN TRY;
                        -- Add the base data
                        INSERT INTO dbo.TB_DIM_ROUTING_CLIENT
                        (
                       ...
                       , [LOGICAL_INTERFACE_CONTROLLER_BIZ_URN]
                        ...
                        @XmlData, @TimeZoneUrn, @TenantBizUrn, @HierarchyParentUrn, @CustomXmlData, @SessionUserId, @SessionUserId
                         , @LogicalInterfaceControllerBizUrn
                        );

                        SELECT @ItemUrn = SCOPE_IDENTITY();
            
                        -- Audit success
                        EXEC dbo.ap_adm01_audit_event @SessionId, 'AT_DIMENSION_INSERT_COMMITTED', DEFAULT, @ItemUrn, DEFAULT, 88, DEFAULT, DEFAULT, DEFAULT, 'S', 0, 'DIMENSION_ADDED', NULL, NULL, NULL, @AuditUrn OUTPUT;

                        IF @InitialTransactionCount = 0 
                        BEGIN 
                            COMMIT TRANSACTION PADD_routingcTran;
                        END;
                        ELSE BEGIN 
...
GO
GRANT EXECUTE
	ON [dbo].[ap_dim01_add_routing_client]
	TO [portalapp_role]
GO
