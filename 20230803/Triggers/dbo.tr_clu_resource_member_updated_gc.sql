...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_member_updated_gc] ON [dbo].[TB_CLU_RESOURCE_MEMBER]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_CLU_RESOURCE_MEMBER';
    ...
    JOIN    inserted AS d ON d.RESOURCE_MEMBER_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.RESOURCE_MEMBER_ID,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.RESOURCE_MEMBER_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.RESOURCE_MEMBER_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.RESOURCE_MEMBER_ID = i.RESOURCE_MEMBER_ID );
