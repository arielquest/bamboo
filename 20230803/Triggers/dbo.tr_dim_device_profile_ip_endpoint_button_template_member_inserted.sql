...
GO
CREATE TRIGGER [dbo].[tr_dim_device_profile_ip_endpoint_button_template_member_inserted] ON [dbo].[TB_DIM_DEVICE_PROFILE_IP_ENDPOINT_BUTTON_TEMPLATE_MEMBER] FOR INSERT NOT FOR REPLICATION AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    /*
    -- Test
    OR (NOT UPDATE(MEMBER_BIZ_URN) = 0 AND COLUMNS_UPDATED() ^ 2 <> 0)
    */
    BEGIN 
        RETURN;
    END;
    ...
    BEGIN 
        ...
        BEGIN TRY;
            ...
            BEGIN 
                ...
                SET MEMBER_BIZ_URN = ISNULL(NULLIF(MEMBER_BIZ_URN,-2),MEMBER_URN)
                WHERE MEMBER_URN IN (SELECT MEMBER_URN FROM inserted);
            END;
