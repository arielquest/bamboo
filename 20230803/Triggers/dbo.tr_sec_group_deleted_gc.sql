...
GO
CREATE TRIGGER [dbo].[tr_sec_group_deleted_gc] ON [dbo].[TB_SEC_USER] FOR DELETE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    DECLARE @ErrorMessage AS NVARCHAR(500), @ErrorCode AS [INTEGER], @InitialTransactionCount AS [INTEGER], @TableGenerationCount AS BIGINT, @TableGenerationCountUrn AS INT, @GroupRecordCount AS INT;

    SET @InitialTransactionCount = @@TRANCOUNT;
    SET @ErrorMessage = '';
    SET @ErrorCode = 0;

    -- Check if any groups have been deleted
    SELECT @GroupRecordCount = COUNT(*)
    FROM deleted
    WHERE USER_TYPE = 'G';

    IF @GroupRecordCount > 0
    BEGIN 
        BEGIN TRANSACTION;
        ...
        INNER JOIN TS_ADM_TABLE_GENERATION_COUNT AS tablegc ON tablegc.TABLE_GENERATION_COUNT_URN = datagc.TABLE_GENERATION_COUNT_URN
        INNER JOIN deleted AS d ON d.USER_ID = datagc.GUID_KEY
