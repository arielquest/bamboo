...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_folder_mapping_updated_gc] ON [dbo].[TB_CLU_RESOURCE_FOLDER_MAPPING]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_CLU_RESOURCE_FOLDER_MAPPING';
    ...
    JOIN    inserted AS d ON d.MAPPING_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.MAPPING_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.MAPPING_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.MAPPING_ID = i.MAPPING_ID );
