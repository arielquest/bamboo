...
GO
CREATE TRIGGER [dbo].[tr_rep_dashboard_layout_updated_gc] ON [dbo].[TB_REP_DASHBOARD_LAYOUT]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_REP_DASHBOARD_LAYOUT';
    ...
    JOIN    inserted AS d ON d.DASHBOARD_LAYOUT_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.DASHBOARD_LAYOUT_ID,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.DASHBOARD_LAYOUT_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.DASHBOARD_LAYOUT_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.DASHBOARD_LAYOUT_ID = i.DASHBOARD_LAYOUT_ID );
