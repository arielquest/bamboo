...
GO
CREATE TRIGGER [dbo].[tr_rep_parameter_set_updated_gc] ON [dbo].[TB_REP_PARAMETER_SET]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_REP_PARAMETER_SET';
    ...
    JOIN    inserted AS d ON d.PARAMETER_SET_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.PARAMETER_SET_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.PARAMETER_SET_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.PARAMETER_SET_ID = i.PARAMETER_SET_ID );
