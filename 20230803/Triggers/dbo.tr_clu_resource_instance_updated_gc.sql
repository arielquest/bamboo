...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_instance_updated_gc] ON [dbo].[TB_CLU_RESOURCE_INSTANCE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_CLU_RESOURCE_INSTANCE';
    ...
    JOIN    inserted AS d ON d.RESOURCE_INSTANCE_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.RESOURCE_INSTANCE_ID,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.RESOURCE_INSTANCE_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.RESOURCE_INSTANCE_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.RESOURCE_INSTANCE_ID = i.RESOURCE_INSTANCE_ID );
