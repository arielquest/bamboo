...
GO
CREATE TRIGGER [dbo].[tr_dim_nng_updated_gc] ON [dbo].[TB_DIM_NNG]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    JOIN    inserted AS d ON d.NNG_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.NNG_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.NNG_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.NNG_URN = i.NNG_URN );
