...
GO
CREATE TRIGGER [dbo].[tr_adm_gadget_set_mapping_inserted_updated_gc] ON [dbo].[TB_ADM_GADGET_SET_MAPPING] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
            ...
            EXEC dbo.ap_adm01_increment_table_generation_count @TableName = 'TB_ADM_GADGET_SET_MAPPING', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT, @TableGenerationCountUrn = @TableGenerationCountUrn OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_ADM_DATA_GENERATION_COUNT dgc
            USING (SELECT GADGET_SET_MAPPING_ID, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END FROM inserted) correctValues(GADGET_SET_MAPPING_ID, LAST_OPERATION)
            ON (dgc.GUID_KEY = correctValues.GADGET_SET_MAPPING_ID AND dgc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn)
            WHEN MATCHED THEN
            ...
            WHEN NOT MATCHED THEN 
                ...
                VALUES (@TableGenerationCountUrn, correctValues.GADGET_SET_MAPPING_ID, @NewTableGenerationCount, correctValues.LAST_OPERATION);
