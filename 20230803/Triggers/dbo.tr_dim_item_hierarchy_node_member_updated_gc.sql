...
GO
CREATE TRIGGER [dbo].[tr_dim_item_hierarchy_node_member_updated_gc] ON [dbo].[TB_DIM_ITEM_HIERARCHY_NODE_MEMBER]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_DIM_ITEM_HIERARCHY_NODE_MEMBER';
    ...
    JOIN    inserted AS i ON i.MEMBER_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i1
        WHERE   NOT EXISTS ( SELECT i2.MEMBER_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS i2 ON i2.MEMBER_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND i2.MEMBER_URN = i1.MEMBER_URN );
