...
GO
CREATE TRIGGER [dbo].[tr_dim_date_updated_gc] ON [dbo].[TB_DIM_DATE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    JOIN    inserted AS d ON d.DATE_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.DATE_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.DATE_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.DATE_URN = i.DATE_URN );
