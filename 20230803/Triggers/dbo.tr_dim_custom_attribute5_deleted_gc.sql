...
GO
CREATE TRIGGER [dbo].[tr_dim_custom_attribute5_deleted_gc] ON [dbo].[TB_DIM_CUSTOM_ATTRIBUTE5] FOR DELETE
AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN 
        ...
        BEGIN TRY;
            ...
            EXEC [dbo].[ap_adm01_increment_table_generation_count] @TableName = 'TB_DIM_CUSTOM_ATTRIBUTE5', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT;

            -- Update Data Generation Counts
            UPDATE dbo.TS_DIM_CUSTOM_ATTRIBUTE5_GC
            SET LAST_OPERATION = 'D', GENERATION_COUNT = @NewTableGenerationCount
            FROM dbo.TS_DIM_CUSTOM_ATTRIBUTE5_GC AS datagc
