...
GO
CREATE TRIGGER [dbo].[tr_dim_hierarchy_level_updated_gc] ON [dbo].[TB_DIM_HIERARCHY_LEVEL]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_DIM_HIERARCHY_LEVEL';
    ...
    JOIN    inserted AS i ON i.HIERARCHY_LEVEL_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i1.HIERARCHY_LEVEL_URN,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i1
        WHERE   NOT EXISTS ( SELECT i2.HIERARCHY_LEVEL_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS i2 ON i2.HIERARCHY_LEVEL_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND i2.HIERARCHY_LEVEL_URN = i1.HIERARCHY_LEVEL_URN );
