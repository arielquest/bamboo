...
GO
CREATE TRIGGER [dbo].[tr_sec_information_notice_updated_gc] ON [dbo].[TB_SEC_INFORMATION_NOTICE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    JOIN    inserted AS d ON d.INFORMATION_NOTICE_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.INFORMATION_NOTICE_ID,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.INFORMATION_NOTICE_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.INFORMATION_NOTICE_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.INFORMATION_NOTICE_ID = i.INFORMATION_NOTICE_ID );
