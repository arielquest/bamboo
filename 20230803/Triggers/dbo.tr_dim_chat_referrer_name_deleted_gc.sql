...
GO
CREATE TRIGGER [dbo].[tr_dim_chat_referrer_name_deleted_gc] ON [dbo].[TB_DIM_CHAT_REFERRER_NAME] FOR DELETE
AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN TRANSACTION TR_chatreferrernTran;
    WHILE @deadlockRetry < 3
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
        BEGIN TRY;
            -- Update and Get Table Generation Count
            EXEC [dbo].[ap_adm01_increment_table_generation_count] @TableName = 'TB_DIM_CHAT_REFERRER_NAME', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT;

            -- Update Data Generation Counts
            UPDATE dbo.TS_DIM_CHAT_REFERRER_NAME_GC
            SET LAST_OPERATION = 'D', GENERATION_COUNT = @NewTableGenerationCount
            FROM dbo.TS_DIM_CHAT_REFERRER_NAME_GC AS datagc
            JOIN deleted AS d ON d.ITEM_URN = datagc.ITEM_URN;

            IF @InitialTransactionCount = 0 
            BEGIN 
               COMMIT TRANSACTION TR_chatreferrernTran;
            END;
            ELSE BEGIN 
