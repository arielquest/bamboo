...
GO
CREATE TRIGGER [dbo].[tr_dim_item_type_inserted_updated_gc] ON [dbo].[TE_DIM_ITEM_TYPE] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
            ...
            EXEC dbo.ap_adm01_increment_table_generation_count @TableName = 'TE_DIM_ITEM_TYPE', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT, @TableGenerationCountUrn = @TableGenerationCountUrn OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_ADM_DATA_GENERATION_COUNT dgc
            USING (SELECT ITEM_TYPE_ID, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END 
                   FROM inserted) correctValues(ITEM_TYPE_ID, LAST_OPERATION)
            ON (dgc.GUID_KEY = correctValues.ITEM_TYPE_ID AND dgc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn)
            WHEN MATCHED THEN
            ...
            WHEN NOT MATCHED THEN 
                ...
                VALUES (@TableGenerationCountUrn, correctValues.ITEM_TYPE_ID, @NewTableGenerationCount, correctValues.LAST_OPERATION);
