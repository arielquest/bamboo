...
GO
CREATE TRIGGER [dbo].[tr_adm_gadget_version_file_inserted_updated_gc] ON [dbo].[TB_ADM_GADGET_VERSION_FILE]
    FOR INSERT, UPDATE
AS
    BEGIN 
        SET XACT_ABORT OFF;
    ...
    -- Return early if no rows or if it is an update is by replication on the msrepl_tran_version column
        ...
        BEGIN 
            ...
            -- Update and Get Table Generation Count
                EXEC [dbo].[ap_adm01_increment_table_generation_count] @TableName = 'TB_ADM_GADGET_VERSION_FILE',
                    @NewTableGenerationCount = @NewTableGenerationCount OUTPUT, @TableGenerationCountUrn = @TableGenerationCountUrn OUTPUT;
            ...
            -- Update Data Generation Counts
                ...
                USING
                    ( SELECT    [Inserted].[ID],
                                ...
                                END
                      FROM      [inserted]
                    ) [correctValues] ( [ID], [LAST_OPERATION] )
                ON ( [dgc].[GUID_KEY] = [correctValues].[ID]
                     AND [dgc].[TABLE_GENERATION_COUNT_URN] = @TableGenerationCountUrn
                   )
                WHEN MATCHED THEN
                ...
                WHEN NOT MATCHED THEN
                    ...
                    VALUES ( @TableGenerationCountUrn,
                             [correctValues].[ID],
                             @NewTableGenerationCount,
                             [correctValues].[LAST_OPERATION]
                           );
