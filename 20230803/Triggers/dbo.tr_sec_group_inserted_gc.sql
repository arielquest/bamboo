...
GO
CREATE TRIGGER [dbo].[tr_sec_group_inserted_gc] ON [dbo].[TB_SEC_USER]
    FOR INSERT
AS
BEGIN 
    SET XACT_ABORT OFF; 
    ...
    DECLARE @ErrorMessage AS NVARCHAR(500),
        ...
        @GroupRecordCount AS INTEGER;
            
    SET @InitialTransactionCount = @@TRANCOUNT;
    SET @ErrorMessage = '';
    SET @ErrorCode = 0;
    
    -- Check if any groups have been inserted/updated
    ...
    BEGIN 
        ...
        USING
            ( SELECT    @TableGenerationCountUrn,
              ...
              FROM      inserted AS i
              WHERE     i.USER_TYPE = 'G'
            ) AS source ( TABLE_GENERATION_COUNT_URN, GUID_KEY,
                          GENERATION_COUNT, LAST_OPERATION )
        ON (
