...
GO
CREATE TRIGGER [dbo].[tr_dim_category_map_updated_gc] ON [dbo].[TB_DIM_CATEGORY_MAP]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_DIM_CATEGORY_MAP';
    ...
    JOIN    inserted AS d ON d.CATEGORY_MAP_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.CATEGORY_MAP_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.CATEGORY_MAP_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.CATEGORY_MAP_URN = i.CATEGORY_MAP_URN );
