...
GO
CREATE TRIGGER [dbo].[tr_clu_connection_type_updated_gc] ON [dbo].[TE_CLU_CONNECTION_TYPE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TE_CLU_CONNECTION_TYPE';
    ...
    JOIN    inserted AS d ON d.CONNECTION_TYPE_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.CONNECTION_TYPE_ID,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.CONNECTION_TYPE_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.CONNECTION_TYPE_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.CONNECTION_TYPE_ID = i.CONNECTION_TYPE_ID );
