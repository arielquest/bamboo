...
GO
CREATE TRIGGER [dbo].[tr_clu_connection_inserted_updated_gc] ON [dbo].[TB_CLU_CONNECTION] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
            ...
            USING (SELECT CONNECTION_ID, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END FROM inserted) correctValues(CONNECTION_ID, LAST_OPERATION)
            ...
            WHEN NOT MATCHED THEN 
                INSERT (CONNECTION_ID, GENERATION_COUNT, LAST_OPERATION)
                VALUES (correctValues.CONNECTION_ID, @NewTableGenerationCount, correctValues.LAST_OPERATION);
