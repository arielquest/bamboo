...
GO
CREATE TRIGGER [dbo].[tr_rep_threshold_updated_gc] ON [dbo].[TB_REP_THRESHOLD]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_REP_THRESHOLD';
    ...
    JOIN    inserted AS d ON d.THRESHOLD_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.THRESHOLD_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.THRESHOLD_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.THRESHOLD_ID = i.THRESHOLD_ID );
