...
GO
CREATE TRIGGER [dbo].[tr_dim_person_deleted_gc] ON [dbo].[TB_DIM_PERSON] FOR DELETE
AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN TRANSACTION TR_perTran;
    WHILE @deadlockRetry < 3
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
        BEGIN TRY;
            -- Update and Get Table Generation Count
            EXEC [dbo].[ap_adm01_increment_table_generation_count] @TableName = 'TB_DIM_PERSON', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT;

            -- Update Data Generation Counts
            UPDATE dbo.TS_DIM_PERSON_GC
            SET LAST_OPERATION = 'D', GENERATION_COUNT = @NewTableGenerationCount
            FROM dbo.TS_DIM_PERSON_GC AS datagc
            JOIN deleted AS d ON d.ITEM_URN = datagc.ITEM_URN;

            IF @InitialTransactionCount = 0 
            BEGIN 
               COMMIT TRANSACTION TR_perTran;
            END;
            ELSE BEGIN 
