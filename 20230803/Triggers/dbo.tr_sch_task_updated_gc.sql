...
GO
CREATE TRIGGER [dbo].[tr_sch_task_updated_gc] ON [dbo].[TB_SCH_TASK]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    IF NOT EXISTS ( SELECT  1
        ...
        OR (
           ...
           OR NOT EXISTS (
            SELECT 1 FROM inserted WHERE type = 'P')
    BEGIN 
        RETURN;
    END;
    ...
    BEGIN 
        ...
        BEGIN TRY;
            ...
            USING (SELECT TASK_ID, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END FROM inserted) correctValues(ID, LAST_OPERATION)
