...
GO
CREATE TRIGGER [dbo].[tr_dim_device_profile_ip_endpoint_button_template_member_inserted_updated_gc] ON [dbo].[TB_DIM_DEVICE_PROFILE_IP_ENDPOINT_BUTTON_TEMPLATE_MEMBER] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN TRANSACTION TR_deviceprofileipendpoinTran;
    ...
    BEGIN 
        ...
        BEGIN TRY;
            ...
            EXEC [dbo].[ap_adm01_increment_table_generation_count] @TableName = 'TB_DIM_DEVICE_PROFILE_IP_ENDPOINT_BUTTON_TEMPLATE_MEMBER', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_DIM_DEVICE_PROFILE_IP_ENDPOINT_BUTTON_TEMPLATE_MEMBER_GC dgc
            USING (SELECT MEMBER_URN, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END FROM inserted) correctValues(MEMBER_URN, LAST_OPERATION)
            ON (dgc.MEMBER_URN = correctValues.MEMBER_URN)
            WHEN MATCHED THEN
                UPDATE SET dgc.GENERATION_COUNT = @NewTableGenerationCount,
                           dgc.LAST_OPERATION = correctValues.LAST_OPERATION
            WHEN NOT MATCHED THEN 
                INSERT (MEMBER_URN, GENERATION_COUNT, CREATE_GENERATION_COUNT, LAST_OPERATION)
                VALUES (correctValues.MEMBER_URN, @NewTableGenerationCount, @NewTableGenerationCount, correctValues.LAST_OPERATION);

            IF @InitialTransactionCount = 0 
            BEGIN 
               COMMIT TRANSACTION TR_deviceprofileipendpoinTran;
            END;
            ELSE BEGIN 
