...
GO
CREATE TRIGGER [dbo].[tr_dim_member_type_inserted_updated_gc] ON [dbo].[TE_DIM_MEMBER_TYPE] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
            ...
            EXEC dbo.ap_adm01_increment_table_generation_count @TableName = 'TE_DIM_MEMBER_TYPE', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT, @TableGenerationCountUrn = @TableGenerationCountUrn OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_ADM_DATA_GENERATION_COUNT dgc
            USING (SELECT MEMBER_TYPE_ID, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END 
                   FROM inserted) correctValues(MEMBER_TYPE_ID, LAST_OPERATION)
            ON (dgc.GUID_KEY = correctValues.MEMBER_TYPE_ID AND dgc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn)
            WHEN MATCHED THEN
            ...
            WHEN NOT MATCHED THEN 
                ...
                VALUES (@TableGenerationCountUrn, correctValues.MEMBER_TYPE_ID, @NewTableGenerationCount, correctValues.LAST_OPERATION);
