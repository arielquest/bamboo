...
GO
CREATE TRIGGER [dbo].[tr_dim_fiscal_month_updated_gc] ON [dbo].[TB_DIM_FISCAL_MONTH]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    JOIN    inserted AS d ON d.FISCAL_MONTH_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.FISCAL_MONTH_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.FISCAL_MONTH_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.FISCAL_MONTH_URN = i.FISCAL_MONTH_URN );
