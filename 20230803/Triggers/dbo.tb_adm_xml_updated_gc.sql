...
GO
CREATE TRIGGER [dbo].[tb_adm_xml_updated_gc] ON [dbo].[TB_ADM_XML]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'VW_ADM_XML_H_GC';
    ...
    JOIN    inserted AS d ON d.XML_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.XML_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.XML_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.XML_ID = i.XML_ID )
                AND i.TYPE = 'H';
