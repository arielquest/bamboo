...
GO
CREATE TRIGGER [dbo].[tr_rep_dashboard_updated_gc] ON [dbo].[TB_REP_DASHBOARD]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_REP_DASHBOARD';
    ...
    JOIN    inserted AS d ON d.DASHBOARD_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.DASHBOARD_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.DASHBOARD_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.DASHBOARD_ID = i.DASHBOARD_ID );
