...
GO
CREATE TRIGGER [dbo].[tr_sec_policy_updated_gc] ON [dbo].[TB_SEC_POLICY]
    FOR UPDATE
AS
    BEGIN 
        SET XACT_ABORT OFF;
        ...
        BEGIN TRY
            ...
            WHERE   [TABLE_NAME] = 'TB_SEC_POLICY';
            ...
            JOIN    [Inserted] AS [d] ON [d].[POLICY_ID] = [datagc].[GUID_KEY]
            ...
            BEGIN 
                ...
                SELECT  @TableGenerationCountUrn,
                        ...
                        'U'
                FROM    [Inserted] AS [i]
                WHERE   NOT EXISTS ( SELECT [d].[POLICY_ID]
                                     FROM   [dbo].[TS_ADM_DATA_GENERATION_COUNT] AS [datagc]
                                     JOIN   [Inserted] AS [d] ON [d].[POLICY_ID] = [datagc].[GUID_KEY]
                                     WHERE  [datagc].[TABLE_GENERATION_COUNT_URN] = @TableGenerationCountUrn
                                            AND [d].[POLICY_ID] = [i].[POLICY_ID] );
