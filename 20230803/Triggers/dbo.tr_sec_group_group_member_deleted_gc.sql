...
GO
CREATE TRIGGER [dbo].[tr_sec_group_group_member_deleted_gc] ON [dbo].[TB_SEC_GROUP_MEMBER] FOR DELETE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    IF EXISTS (
            SELECT MEMBER_ID
            FROM deleted
            INNER JOIN TB_SEC_USER secUser ON secUser.USER_ID = deleted.CHILD_ID
            WHERE secUser.USER_TYPE = 'G'
            )
    BEGIN 
        BEGIN TRANSACTION;
        ...
        WHERE TABLE_NAME = 'TB_SEC_GROUP_GROUP_MEMBER';
