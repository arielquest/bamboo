...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_instance_component_updated_gc] ON [dbo].[TB_CLU_RESOURCE_INSTANCE_COMPONENT]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_CLU_RESOURCE_INSTANCE_COMPONENT';
    ...
    JOIN    inserted AS d ON d.COMPONENT_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.COMPONENT_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.COMPONENT_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
