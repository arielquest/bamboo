...
GO
CREATE TRIGGER [dbo].[tr_dim_item_subtype_updated_gc] ON [dbo].[TE_DIM_ITEM_SUBTYPE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TE_DIM_ITEM_SUBTYPE';
    ...
    JOIN    inserted AS d ON d.ITEM_SUBTYPE_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.ITEM_SUBTYPE_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.ITEM_SUBTYPE_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.ITEM_SUBTYPE_ID = i.ITEM_SUBTYPE_ID );
