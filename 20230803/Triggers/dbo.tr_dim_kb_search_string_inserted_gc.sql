...
GO
CREATE   TRIGGER [dbo].[tr_dim_kb_search_string_inserted_gc]
    ON [dbo].[TB_DIM_KB_SEARCH_STRING] FOR INSERT
AS
BEGIN
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN TRANSACTION;
        ...
        USING (    SELECT @TableGenerationCountUrn, i.ITEM_URN, @TableGenerationCount, 'I'
                FROM inserted AS i) AS source (TABLE_GENERATION_COUNT_URN, INT_KEY, GENERATION_COUNT, LAST_OPERATION)
        ON (target.TABLE_GENERATION_COUNT_URN = source.TABLE_GENERATION_COUNT_URN
            AND target.INT_KEY = source.INT_KEY)
        WHEN MATCHED THEN 
            UPDATE SET GENERATION_COUNT = source.GENERATION_COUNT, LAST_OPERATION = 'I', MODIFIED_DATE = GETUTCDATE()
        WHEN NOT MATCHED THEN    
            INSERT (TABLE_GENERATION_COUNT_URN, INT_KEY, GENERATION_COUNT, LAST_OPERATION)
            VALUES (source.TABLE_GENERATION_COUNT_URN, source.INT_KEY, source.GENERATION_COUNT, source.LAST_OPERATION);
