...
GO
CREATE TRIGGER [dbo].[tr_rep_report_chart_inserted_updated_gc] ON [dbo].[TB_REP_REPORT_CHART] FOR INSERT, UPDATE AS
BEGIN
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN
        SAVE TRANSACTION deadlockSavePoint;
            ...
            EXEC dbo.ap_adm01_increment_table_generation_count @TableName = 'TB_REP_REPORT_CHART', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT, @TableGenerationCountUrn = @TableGenerationCountUrn OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_ADM_DATA_GENERATION_COUNT dgc
            USING (SELECT REPORT_CHART_ID, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END FROM inserted) correctValues(REPORT_CHART_ID, LAST_OPERATION)
            ON (dgc.INT_KEY = correctValues.REPORT_CHART_ID AND dgc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn)
            WHEN MATCHED THEN
            ...
            WHEN NOT MATCHED THEN 
                ...
                VALUES (@TableGenerationCountUrn, correctValues.REPORT_CHART_ID, @NewTableGenerationCount, correctValues.LAST_OPERATION);
