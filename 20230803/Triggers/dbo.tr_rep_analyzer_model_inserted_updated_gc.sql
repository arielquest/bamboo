...
GO
CREATE TRIGGER [dbo].[tr_rep_analyzer_model_inserted_updated_gc] ON [dbo].[TB_REP_ANALYZER_MODEL] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
            ...
            EXEC dbo.ap_adm01_increment_table_generation_count @TableName = 'TB_REP_ANALYZER_MODEL', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT, @TableGenerationCountUrn = @TableGenerationCountUrn OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_ADM_DATA_GENERATION_COUNT dgc
            USING (SELECT ANALYZER_MODEL_URN, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END FROM inserted) correctValues(ANALYZER_MODEL_URN, LAST_OPERATION)
            ON (dgc.INT_KEY = correctValues.ANALYZER_MODEL_URN AND dgc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn)
            WHEN MATCHED THEN
            ...
            WHEN NOT MATCHED THEN 
                ...
                VALUES (@TableGenerationCountUrn, correctValues.ANALYZER_MODEL_URN, @NewTableGenerationCount, correctValues.LAST_OPERATION);
