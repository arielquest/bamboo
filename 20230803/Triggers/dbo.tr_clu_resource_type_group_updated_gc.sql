...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_type_group_updated_gc] ON [dbo].[TE_CLU_RESOURCE_TYPE_GROUP]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TE_CLU_RESOURCE_TYPE_GROUP';
    ...
    JOIN    inserted AS d ON d.RESOURCE_TYPE_GROUP_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.RESOURCE_TYPE_GROUP_ID,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.RESOURCE_TYPE_GROUP_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.RESOURCE_TYPE_GROUP_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.RESOURCE_TYPE_GROUP_ID = i.RESOURCE_TYPE_GROUP_ID );
