...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_instance_component_type_updated_gc] ON [dbo].[TE_CLU_RESOURCE_INSTANCE_COMPONENT_TYPE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TE_CLU_RESOURCE_INSTANCE_COMPONENT_TYPE';
    ...
    JOIN    inserted AS d ON d.COMPONENT_TYPE_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.COMPONENT_TYPE_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.COMPONENT_TYPE_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.COMPONENT_TYPE_ID = i.COMPONENT_TYPE_ID );
