...
GO
CREATE TRIGGER [dbo].[tr_clu_connection_status_updated_gc] ON [dbo].[TB_CLU_CONNECTION_STATUS]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    BEGIN 
        ...
        BEGIN TRY;
            ...
            EXEC dbo.ap_adm01_increment_table_generation_count @TableName = 'TB_CLU_CONNECTION',
                @NewTableGenerationCount = @NewTableGenerationCount OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_CLU_CONNECTION_GC dgc
            USING
                ( SELECT    CONNECTION_ID,
                            ...
                            END
                  FROM      inserted
                ) correctValues ( CONNECTION_ID, LAST_OPERATION )
            ON ( dgc.CONNECTION_ID = correctValues.CONNECTION_ID )
            WHEN MATCHED THEN
                UPDATE SET
                         dgc.GENERATION_COUNT = @NewTableGenerationCount,
