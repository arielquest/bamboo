...
GO
CREATE TRIGGER [dbo].[tr_sec_search_folder_updated_gc] ON [dbo].[TB_SEC_SEARCH_FOLDER]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_SEC_SEARCH_FOLDER';
    ...
    JOIN    inserted AS d ON d.SEARCH_FOLDER_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.SEARCH_FOLDER_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.SEARCH_FOLDER_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.SEARCH_FOLDER_ID = i.SEARCH_FOLDER_ID );
