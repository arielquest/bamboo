...
GO
CREATE TRIGGER [dbo].[tr_adm_custom_attribute_localization_inserted_updated_gc] ON [dbo].[TB_ADM_CUSTOM_ATTRIBUTE_LOCALIZATION] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
            ...
            EXEC dbo.ap_adm01_increment_table_generation_count @TableName = 'TB_ADM_CUSTOM_ATTRIBUTE_LOCALIZATION', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT, @TableGenerationCountUrn = @TableGenerationCountUrn OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_ADM_DATA_GENERATION_COUNT dgc
            USING (SELECT CUSTOM_ATTRIBUTE_LOCALIZATION_URN, CASE @IsUpdate WHEN 1 THEN 'U' ELSE 'I' END FROM inserted) correctValues(CUSTOM_ATTRIBUTE_LOCALIZATION_URN, LAST_OPERATION)
            ON (dgc.INT_KEY = correctValues.CUSTOM_ATTRIBUTE_LOCALIZATION_URN AND dgc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn)
            WHEN MATCHED THEN
            ...
            WHEN NOT MATCHED THEN 
                INSERT (TABLE_GENERATION_COUNT_URN, INT_KEY, GENERATION_COUNT, LAST_OPERATION)
                VALUES (@TableGenerationCountUrn, correctValues.CUSTOM_ATTRIBUTE_LOCALIZATION_URN, @NewTableGenerationCount, correctValues.LAST_OPERATION);
