...
GO
CREATE TRIGGER [dbo].[tr_sec_information_notice_deleted_gc] ON [dbo].[TB_SEC_INFORMATION_NOTICE] FOR DELETE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    DECLARE @ErrorMessage AS NVARCHAR(500), @ErrorCode AS [INTEGER], @InitialTransactionCount AS [INTEGER], @TableGenerationCount AS BIGINT, @TableGenerationCountUrn AS INT, @InformationNoticeId AS UNIQUEIDENTIFIER;
    ...
    WHERE TABLE_NAME = 'TB_SEC_INFORMATION_NOTICE';
    ...
    INNER JOIN deleted AS d ON d.INFORMATION_NOTICE_ID = datagc.GUID_KEY
