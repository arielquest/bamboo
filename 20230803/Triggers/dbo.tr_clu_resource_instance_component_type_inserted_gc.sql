...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_instance_component_type_inserted_gc] ON [dbo].[TE_CLU_RESOURCE_INSTANCE_COMPONENT_TYPE]
    FOR INSERT
AS
BEGIN 
    SET XACT_ABORT OFF; 
    ...
    USING
        ( SELECT    @TableGenerationCountUrn,
                    ...
                    'I'
          FROM      inserted AS i
        ) AS source ( TABLE_GENERATION_COUNT_URN, GUID_KEY, GENERATION_COUNT,
                      LAST_OPERATION )
    ON (
