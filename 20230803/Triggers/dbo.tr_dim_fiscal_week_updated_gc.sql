...
GO
CREATE TRIGGER [dbo].[tr_dim_fiscal_week_updated_gc] ON [dbo].[TB_DIM_FISCAL_WEEK]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_DIM_FISCAL_WEEK';
    ...
    JOIN    inserted AS d ON d.FISCAL_WEEK_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.FISCAL_WEEK_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.FISCAL_WEEK_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.FISCAL_WEEK_URN = i.FISCAL_WEEK_URN );
