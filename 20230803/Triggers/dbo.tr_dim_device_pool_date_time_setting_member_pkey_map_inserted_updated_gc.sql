...
GO
CREATE TRIGGER [dbo].[tr_dim_device_pool_date_time_setting_member_pkey_map_inserted_updated_gc] ON [dbo].[TB_DIM_DEVICE_POOL_DATE_TIME_SETTING_MEMBER_PKEY_MAP] FOR INSERT, UPDATE AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN TRANSACTION TR_devicepooldatetimesetPKEYTran;
    ...
    BEGIN 
        ...
        BEGIN TRY;
            ...
            EXEC [dbo].[ap_adm01_increment_table_generation_count] @TableName = 'TB_DIM_DEVICE_POOL_DATE_TIME_SETTING_MEMBER_PKEY_MAP', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT;

            -- Update Data Generation Counts
            MERGE INTO dbo.TS_DIM_DEVICE_POOL_DATE_TIME_SETTING_MEMBER_PKEY_MAP_GC dgc
            ...
            BEGIN 
                ...
                AND TS_ADM_PROVISIONING_GENERATION_COUNT.MEMBER_TYPE_ID = '00000058-1000-0000-0000-0000000000c0'
                AND map.STATUS IN ('S', 'P');
            END;

            IF @InitialTransactionCount = 0 
            BEGIN 
               COMMIT TRANSACTION TR_devicepooldatetimesetPKEYTran;
            END;
            ELSE BEGIN 
