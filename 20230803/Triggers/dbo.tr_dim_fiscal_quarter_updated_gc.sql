...
GO
CREATE TRIGGER [dbo].[tr_dim_fiscal_quarter_updated_gc] ON [dbo].[TB_DIM_FISCAL_QUARTER]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_DIM_FISCAL_QUARTER';
    ...
    JOIN    inserted AS d ON d.FISCAL_QUARTER_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.FISCAL_QUARTER_URN,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.FISCAL_QUARTER_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.FISCAL_QUARTER_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.FISCAL_QUARTER_URN = i.FISCAL_QUARTER_URN );
