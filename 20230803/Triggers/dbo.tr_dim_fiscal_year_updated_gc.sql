...
GO
CREATE TRIGGER [dbo].[tr_dim_fiscal_year_updated_gc] ON [dbo].[TB_DIM_FISCAL_YEAR]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_DIM_FISCAL_YEAR';
    ...
    JOIN    inserted AS d ON d.FISCAL_YEAR_URN = datagc.INT_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.FISCAL_YEAR_URN
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.FISCAL_YEAR_URN = datagc.INT_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.FISCAL_YEAR_URN = i.FISCAL_YEAR_URN );
