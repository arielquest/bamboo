...
GO
CREATE TRIGGER [dbo].[tr_dim_bucket_interval_deleted_gc] ON [dbo].[TB_DIM_BUCKET_INTERVAL] FOR DELETE
AS
BEGIN 
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;
    ...
    BEGIN TRANSACTION TR_bucketinterTran;
    WHILE @deadlockRetry < 3
    BEGIN 
        SAVE TRANSACTION deadlockSavePoint;
        BEGIN TRY;
            -- Update and Get Table Generation Count
            EXEC [dbo].[ap_adm01_increment_table_generation_count] @TableName = 'TB_DIM_BUCKET_INTERVAL', @NewTableGenerationCount = @NewTableGenerationCount OUTPUT;

            -- Update Data Generation Counts
            UPDATE dbo.TS_DIM_BUCKET_INTERVAL_GC
            SET LAST_OPERATION = 'D', GENERATION_COUNT = @NewTableGenerationCount
            FROM dbo.TS_DIM_BUCKET_INTERVAL_GC AS datagc
            JOIN deleted AS d ON d.ITEM_URN = datagc.ITEM_URN;

            IF @InitialTransactionCount = 0 
            BEGIN 
               COMMIT TRANSACTION TR_bucketinterTran;
            END;
            ELSE BEGIN 
