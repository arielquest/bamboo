...
GO
CREATE   TRIGGER [dbo].[tr_dim_kb_search_string_updated_gc]
    ON [dbo].[TB_DIM_KB_SEARCH_STRING] FOR UPDATE
AS
BEGIN
    SET XACT_ABORT OFF;
    SET NOCOUNT ON;

    IF NOT EXISTS(SELECT 1 FROM inserted) OR (UPDATE(msrepl_tran_version) AND SESSIONPROPERTY('REPLICATION_AGENT') = 0) RETURN;
    ...
    BEGIN TRANSACTION;
        ...
        SELECT @TableGenerationCount = GENERATION_COUNT, @TableGenerationCountUrn =  TABLE_GENERATION_COUNT_URN FROM dbo.TS_ADM_TABLE_GENERATION_COUNT WITH (UPDLOCK) WHERE TABLE_NAME = 'TB_DIM_KB_SEARCH_STRING';
        ...
        SELECT @UpdatedRowCount = COUNT(*) FROM inserted;
        ...
        SELECT @ErrorCode = @@ERROR, @DataGenUpdateCount = @@ROWCOUNT;
        IF @ErrorCode <> 0
        BEGIN
            SET @ErrorMessage = 'An error occured while attempting to update item. Error = [%d]';
            GOTO EXIT_WITH_ROLLBACK;
        ...
        BEGIN
            ...
            INSERT INTO TS_ADM_DATA_GENERATION_COUNT (TABLE_GENERATION_COUNT_URN, INT_KEY, GENERATION_COUNT, LAST_OPERATION)
            SELECT @TableGenerationCountUrn, i.ITEM_URN, @TableGenerationCount, 'U'
            FROM inserted AS i
            WHERE NOT EXISTS (SELECT d.ITEM_URN
                FROM dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                JOIN inserted AS d ON d.ITEM_URN = datagc.INT_KEY
                WHERE datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn AND d.ITEM_URN = i.ITEM_URN);        
