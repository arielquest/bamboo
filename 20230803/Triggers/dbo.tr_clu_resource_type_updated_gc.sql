...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_type_updated_gc] ON [dbo].[TE_ADM_CLUSTER_RESOURCE_TYPE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TE_ADM_CLUSTER_RESOURCE_TYPE';
    ...
    JOIN    inserted AS d ON d.CLUSTER_RESOURCE_TYPE_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                i.CLUSTER_RESOURCE_TYPE_ID,
                @TableGenerationCount,
                @TableGenerationCount,
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.CLUSTER_RESOURCE_TYPE_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.CLUSTER_RESOURCE_TYPE_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.CLUSTER_RESOURCE_TYPE_ID = i.CLUSTER_RESOURCE_TYPE_ID );
