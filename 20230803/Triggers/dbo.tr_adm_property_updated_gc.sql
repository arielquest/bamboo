...
GO
CREATE TRIGGER [dbo].[tr_adm_property_updated_gc] ON [dbo].[TB_ADM_PROPERTY]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    WHERE   TABLE_NAME = 'TB_ADM_PROPERTY';
    ...
    JOIN    inserted AS d ON d.PROPERTY_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.PROPERTY_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.PROPERTY_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.PROPERTY_ID = i.PROPERTY_ID );
