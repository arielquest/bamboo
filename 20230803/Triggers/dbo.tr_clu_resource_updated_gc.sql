...
GO
CREATE TRIGGER [dbo].[tr_clu_resource_updated_gc] ON [dbo].[TB_CLU_RESOURCE]
    FOR UPDATE
AS
BEGIN 
    SET XACT_ABORT OFF;
    ...
    JOIN    inserted AS d ON d.RESOURCE_ID = datagc.GUID_KEY
    ...
    BEGIN 
        ...
        SELECT  @TableGenerationCountUrn,
                ...
                'U'
        FROM    inserted AS i
        WHERE   NOT EXISTS ( SELECT d.RESOURCE_ID
                             FROM   dbo.TS_ADM_DATA_GENERATION_COUNT AS datagc
                             JOIN   inserted AS d ON d.RESOURCE_ID = datagc.GUID_KEY
                             WHERE  datagc.TABLE_GENERATION_COUNT_URN = @TableGenerationCountUrn
                                    AND d.RESOURCE_ID = i.RESOURCE_ID );
